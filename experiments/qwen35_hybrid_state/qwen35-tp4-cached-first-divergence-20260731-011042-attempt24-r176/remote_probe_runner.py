...
def main():
    ...
    try:
        ...
        def wrapped_official_chunk(*args, **kwargs):
            ...
            traced, _ = trace_chunk_core(
                ...
                selected_heads=linear.num_v_heads // 4,
            )
            official_rows["layer1_trace_replay_output"] = (
        ...
        def save_official_projection_chunk(name, kind):
            def hook(_module, _args, value):
                chunk = value[:, -64:]
                if kind == "qkv":
                    query, key, projected_value = chunk.split(
                        (
                            linear.key_dim,
                            linear.key_dim,
                            linear.value_dim,
                    ...
                    chunk = chunk[:, :, : linear.value_dim // 4]
                elif kind == "heads":
                    chunk = chunk[:, :, : linear.num_v_heads // 4]
                else:
                    raise RuntimeError(
                        f"unsupported projection chunk kind: {kind}"
                    )
                official_rows[name] = (
                    chunk.squeeze(0).detach().float().cpu().clone()
                )
                return value
            return hook
        ...
        hooks.extend((
            linear.in_proj_qkv.register_forward_hook(
                save_official_projection_chunk(
                    "layer1_trace_projected_qkv",
                    "qkv",
                )
            ),
            linear.in_proj_z.register_forward_hook(
                save_official_projection_chunk(
                    "layer1_trace_projected_z",
                    "value",
                )
            ),
            linear.in_proj_b.register_forward_hook(
                save_official_projection_chunk(
                    "layer1_trace_projected_b",
                    "heads",
                )
            ),
            linear.in_proj_a.register_forward_hook(
                save_official_projection_chunk(
                    "layer1_trace_projected_a",
                    "heads",
                )
            ),
        ))
    ...
    try:
        ...
        hooks.append(
            layer.input_layernorm.register_forward_hook(
                lambda _module, _args, value: (
                    engine_rows[phase["name"]].__setitem__(
                        "layer1_trace_input_norm_chunk",
                        value[-64:].detach().float().cpu().clone(),
                    )
                    if phase["name"] in engine_rows
                    else None
                )
                or value
            )
        ...
        def save_engine_projection_chunk(name):
            def hook(_module, _args, value):
                name_phase = phase["name"]
                if name_phase in engine_rows:
                    engine_rows[name_phase][name] = (
                        value[-64:].detach().float().cpu().clone()
                    )
                return value
            return hook
    ...
    names = [
        ...
        "layer1_conv_local",
        "layer1_core_local",
        "layer1_trace_raw_query",
        "layer1_trace_raw_key",
        "layer1_trace_raw_value",
        "layer1_trace_raw_g",
        "layer1_trace_raw_beta",
        "layer1_trace_state_before",
        "layer1_trace_query_chunk",
        "layer1_trace_key_chunk",
        "layer1_trace_value_chunk",
        "layer1_trace_key_cumulative_decay",
        "layer1_trace_decay_mask",
        "layer1_trace_intra",
        "layer1_trace_previous",
        "layer1_trace_value_new",
        "layer1_trace_inter",
        "layer1_trace_intra_value",
        "layer1_trace_output_chunk",
        "layer1_trace_state_after",
        "layer1_trace_replay_output",
        "layer1_gated_local",
        "layer1_out_proj",
    ]
