...
def main():
    ...
    try:
        ...
        def wrapped_official_recurrent8(*args, **kwargs):
            ...
            log_decay = kwargs["g"]
            history_row = {}
            history_row["sequence_length"] = int(query.shape[1])

            def history_store(name, tensor):
                history_row[name] = (
            ...
            prepared_query = prepared_query * (
                1 / (prepared_query.shape[-1] ** 0.5)
            )
            prepared_initial_state = initial_state.to(
                prepared_value
            )
            prepared_decay_factor = (
                prepared_log_decay[:, :, 0]
                .exp()
            )
            decayed_state = (
                prepared_initial_state
                * prepared_decay_factor.unsqueeze(
                    -1
                ).unsqueeze(-1)
            )
            memory = (
                decayed_state
                * prepared_key[:, :, 0].unsqueeze(-1)
            ).sum(dim=-2)
            delta = (
                prepared_value[:, :, 0] - memory
            ) * prepared_beta[:, :, 0].unsqueeze(-1)
            updated_state = (
                decayed_state
                + prepared_key[:, :, 0].unsqueeze(-1)
                * delta.unsqueeze(-2)
            ...
            replay_core = (
                updated_state
                * prepared_query[:, :, 0].unsqueeze(-1)
            ).sum(dim=-2)
            for name, tensor in (
                (
                    "normalized_query",
                    normalized_query[:, -1, 12:16],
                ),
                (
                    "normalized_key",
                    normalized_key[:, -1, 12:16],
                ),
                (
                    "prepared_query",
                    prepared_query[:, 12:16, 0],
                ),
                (
                    "prepared_key",
                    prepared_key[:, 12:16, 0],
                ),
                (
                    "prepared_value",
                    prepared_value[:, 12:16, 0],
                ),
                (
                    "prepared_beta",
                    prepared_beta[:, 12:16, 0],
                ),
                (
                    "prepared_log_decay",
                    prepared_log_decay[:, 12:16, 0],
                ),
                (
                    "prepared_decay_factor",
                    prepared_decay_factor[:, 12:16],
                ),
                (
                    "prepared_initial_state_k_v",
                    prepared_initial_state[:, 12:16],
                ),
                (
                    "decayed_state_k_v",
                    decayed_state[:, 12:16],
                ...
                ("replay_core", replay_core[:, 12:16]),
            ):
                history_store(name, tensor)
            ...
            official_rank3_history.append(history_row)
            return result
    ...
    rank3_recompute_history = [
        torch.load(
            capture_root / f"rank3-decode-{index:02d}.pt",
            map_location="cpu",
            weights_only=True,
        )
    ...
    result = {
        ...
        "official_vs_engine_rank3": [
            ...
            },
            {
                "name": "initial_state",
                **compare(
                    official_rank3["initial_state_k_v"].float(),
                    rank3_recompute[
            ...
            },
            {
                "name": "final_state",
                **compare(
                    official_rank3["final_state_k_v"].float(),
                    rank3_recompute[
        ...
        "official_vs_engine_rank3_state_history": [
            {
                "decode_index": decode_index,
                "official_history_index": (
                    len(official_rank3_history) - 4 + decode_index
                ),
                ...
                "official_initial_dtype": str(
                    official_rank3_history[
                        len(official_rank3_history) - 4 + decode_index
                    ]["initial_state_k_v"].dtype
                ),
                "engine_initial_dtype": str(
                    rank3_recompute_history[
                        decode_index
                    ]["initial_state_physical"].dtype
                ),
                "initial_state": compare(
                    official_rank3_history[
                        len(official_rank3_history) - 4 + decode_index
                    ]["initial_state_k_v"].float(),
                    rank3_recompute_history[
                        decode_index
                    ]["initial_state_physical"].transpose(
                        -1, -2
                    ).unsqueeze(0).float(),
                ),
                "initial_state_after_official_dtype_cast": compare(
                    official_rank3_history[
                        len(official_rank3_history) - 4 + decode_index
                    ]["initial_state_k_v"].float(),
                    rank3_recompute_history[
                ...
                "final_state": compare(
                    official_rank3_history[
                        len(official_rank3_history) - 4 + decode_index
                    ]["final_state_k_v"].float(),
                    rank3_recompute_history[
                        decode_index
                    ]["final_state_physical"].transpose(
                        -1, -2
                    ).unsqueeze(0).float(),
                ),
                "intermediates": [
        ...
        "official_rank3_history_count": len(
            official_rank3_history
        ),
        ...
        "schema_version": "qwen35.tp4-cached-decode-step4-layer8-rank3-fixture.v16",
