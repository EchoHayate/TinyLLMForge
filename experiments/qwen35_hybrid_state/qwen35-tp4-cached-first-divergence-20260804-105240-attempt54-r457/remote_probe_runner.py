...
def main():
    ...
    prompt = backend._request_prompt(payload, 1)
    source_prompt = (
    ...
    try:
        ...
        official_layer20 = text.layers[20]
        official_layer20_components = {}
        official_linear_components = {}
        official_linear = official_layer20.linear_attn
        ...
        def save_official_component(name):
            def hook(_module, _args, value):
                official_layer20_components[name] = tail(value)
                return value
            return hook

        def save_official_input(name):
            def hook(_module, args):
                official_layer20_components[name] = tail(args)
            return hook

        hooks.append(
            official_layer20.input_layernorm.register_forward_pre_hook(
                save_official_input("input")
            )
        )
        hooks.append(
            official_layer20.input_layernorm.register_forward_hook(
                save_official_component("input_norm")
            )
        )
        hooks.append(
            official_layer20.post_attention_layernorm
            .register_forward_pre_hook(
                save_official_input("attention_residual")
            )
        )
        hooks.append(
            official_layer20.post_attention_layernorm.register_forward_hook(
                save_official_component("post_norm")
            )
        )
        hooks.append(
            official_layer20.mlp.register_forward_hook(
    ...
    try:
        ...
        engine_layer20 = model.layer_stack.layers[20]
        engine_layer20_components = {
            "recompute": {},
            "restore": {},
        }
        ...
        norm_fixture_tensors = {}
        engine_linear = engine_layer20.linear_attention
        ...
        def wrapped_engine_norm(*args, **kwargs):
            ...
            if (
                ...
                global_value = original_engine_norm(
                    global_core,
                    global_gate,
                    args[2],
                    **kwargs,
                ...
                engine_linear_components[name_phase][
                    "gated_rmsnorm_global_replay"
                ] = canonical_linear_component(
                    "gated_rmsnorm",
                    global_value[:core.shape[0]],
                    official_side=False,
                )
                if name_phase == "recompute":
                    norm_fixture_tensors.update({
                        "core": core.detach().cpu().clone(),
                        "gate": gate.detach().cpu().clone(),
                        "weight": args[2].detach().cpu().clone(),
                    })
            return value
        ...
        def save_engine_component(name):
            def hook(_module, _args, value):
                name_phase = phase["name"]
                if name_phase in engine_layer20_components:
                    engine_layer20_components[name_phase][name] = tail(
                        value
                    )
                return value
            return hook
        ...
        hooks.append(
            engine_layer20.input_layernorm.register_forward_pre_hook(
                save_engine_input("input")
            )
        )
        hooks.append(
            engine_layer20.input_layernorm.register_forward_hook(
                save_engine_component("input_norm")
            )
        )
        hooks.append(
            engine_layer20.post_attention_layernorm
            .register_forward_pre_hook(
                save_engine_input("attention_residual")
            )
        )
        hooks.append(
            engine_layer20.post_attention_layernorm.register_forward_hook(
                save_engine_component("post_norm")
            )
        )
        hooks.append(
            engine_layer20.mlp.register_forward_hook(
        ...
        probe._run_request(
            engine, prompt, 39, timeout_s=configuration.timeout_s
        )
        phase["name"] = None
        probe._run_request(
            engine,
    ...
    for name in linear_component_names:
        if (
            ...
            raise RuntimeError(
                f"missing layer20 linear component capture: {name}"
            )
    ...
    for name_phase in ("recompute", "restore"):
        if (
            "gated_rmsnorm_global_replay"
            not in engine_linear_components[name_phase]
        ):
            raise RuntimeError(
                "missing global gated RMSNorm replay capture: "
                f"{name_phase}"
            )
    official_vs_global_replay_gated_rmsnorm = compare(
        official_linear_components["gated_rmsnorm"],
        engine_linear_components["recompute"][
            "gated_rmsnorm_global_replay"
        ],
    )
    recompute_vs_restore_global_replay_gated_rmsnorm = compare(
        engine_linear_components["recompute"][
            "gated_rmsnorm_global_replay"
        ],
    ...
    official_vs_recompute_layer20_components = []
    recompute_vs_restore_layer20_components = []
    for name in component_names:
        if (
            name not in official_layer20_components
            or name not in engine_layer20_components["recompute"]
            or name not in engine_layer20_components["restore"]
        ):
            raise RuntimeError(
                f"missing layer20 component capture: {name}"
            )
        official_vs_recompute_layer20_components.append({
            "name": name,
            **compare(
                official_layer20_components[name],
                engine_layer20_components["recompute"][name],
            ),
        })
        recompute_vs_restore_layer20_components.append({
            "name": name,
            **compare(
                engine_layer20_components["recompute"][name],
                engine_layer20_components["restore"][name],
            ),
        })
    ...
    required_fixture_tensors = ("core", "gate", "weight")
    missing_fixture_tensors = [
        name for name in required_fixture_tensors
        if name not in norm_fixture_tensors
    ]
    if missing_fixture_tensors:
        raise RuntimeError(
            "missing norm fixture tensors: "
            + ", ".join(missing_fixture_tensors)
        )
    fixture_path = output / "qwen35_layer20_step38_norm_geometry_r457.pt"
    torch.save({
        **norm_fixture_tensors,
        "expected": official_linear_components[
            "gated_rmsnorm"
        ].to(dtype=norm_fixture_tensors["core"].dtype),
        "tensor_parallel_size": int(
            engine_linear.in_proj_qkv.tp_size
        ),
        "norm_eps": float(engine_linear.norm_eps),
        "schema_version": (
            "qwen35.layer20.step38.norm-geometry-fixture.v1"
        ),
    }, fixture_path)

    result = {
        "fixture_path": str(fixture_path),
        "official_vs_global_replay_gated_rmsnorm": (
            official_vs_global_replay_gated_rmsnorm
        ),
        "recompute_vs_restore_global_replay_gated_rmsnorm": (
            recompute_vs_restore_global_replay_gated_rmsnorm
        ),
        "official_vs_recompute_linear_components": (
            official_vs_recompute_linear_components
        ...
        "schema_version": "qwen35.tp4-cached-request1-step38-layer20-norm-fixture.v14",
