...
def _install_model_capture(model, rank, receipt_dir):
    if getattr(model, "_r144_capture_installed", False):
        return
    ...
    def wrapped_gated(core, gate, weight, **kwargs):
        ...
        if weight.data_ptr() == linear.norm_weight.data_ptr():
            if not pending:
                raise RuntimeError("r144 gated capture lacks matching core")
    ...
    model._r144_capture_installed = True
...
def main():
    ...
    sitecustomize.write_text(
        "\n".join([
            ...
            ")",
            "if path:",
            "    name = '_tinyvllm_r144_rank_receipt_runner'",
