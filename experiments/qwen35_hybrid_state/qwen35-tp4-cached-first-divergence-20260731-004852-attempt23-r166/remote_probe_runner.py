...
def main():
    ...
    try:
        ...
        engine_original_chunk_local = engine_original_chunk

        def official_expression_engine_chunk(
            query,
            key,
            value,
            a,
            ...
            *,
            ...
            real_chunks = total_sequence_length // chunk_size
            target_chunks = 17
            if real_chunks > target_chunks:
                raise RuntimeError(
                    "diagnostic chunk-slot target is too small"
                )
            chunk_offset = 0 if real_chunks > 1 else target_chunks - 1
            trailing_chunks = (
                target_chunks - chunk_offset - real_chunks
            )

            def align_chunk_slots(tensor):
                pieces = []
                if chunk_offset:
            ...
            state = recurrent_state.transpose(-1, -2).unsqueeze(0)
            state = state.to(value)
            output = torch.zeros_like(value)
            for index in range(
                chunk_offset,
                chunk_offset + real_chunks,
            ):
                ...
                previous = (
                    key_cumulative_decay[:, :, index] @ state
                )
            ...
            output = output[
                :,
                :,
                chunk_offset:chunk_offset + real_chunks,
            ]
        ...
        engine_original_chunk = official_expression_engine_chunk
