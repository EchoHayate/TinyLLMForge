...
def main():
    ...
    try:
        ...
        def official_local_conv(value):
            row = value[:, :, -64:].transpose(1, 2).squeeze(0).float().cpu()
            query, key, projected_value = row.split(
                (
                    linear.key_dim,
                    linear.key_dim,
            ...
            linear.norm.register_forward_hook(
                lambda _module, _args, value: (
                    official_rows.__setitem__(
                        ...
                        ].reshape(64, -1).detach().float().cpu().clone(),
                    )
                    or value
                )
            )
        ...
        def capture_engine_core(function):
            def wrapped(
                ...
                **kwargs,
                ...
                context_rows = len(prompt)
                if token_count > 1 and context_rows > token_count:
                    padding = context_rows - token_count
                    query = torch.cat((
                        query.new_zeros(
                            padding,
                            query.shape[1],
                ...
                if result[0].shape[0] != token_count:
                    result = (
