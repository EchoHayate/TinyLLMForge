...
def main():
    ...
    try:
        ...
        def save_official_q_projection(_module, _args, value):
            ...
            official_query_gate["gate"] = (
                paired_all[..., official_attention.head_dim:]
        ...
        def save_official_output_input(_module, args):
            ...
            official_query_gate["post_gate"] = (
                value_all.detach().float().cpu().clone()
            )
            store_official(
                "output_input",
            ...
            **kwargs,
            ...
            if official_rotary_scope["active"] and query.shape[2] == 1:
                ...
                official_attention_core.update({
                    ...
                    "full_output": (
                        attention_output.detach().float().cpu().clone()
                    ),
                    "replay_output": (
    ...
    try:
        ...
        def wrapped_engine_query_gate(attention_output, query_gate):
            post_gate = original_engine_query_gate(
                attention_output,
                query_gate,
            )
            name_phase = phase["name"]
            if (
                engine_attention_scope["active"]
                and attention_output.shape[0] == 1
                and name_phase in engine_query_gate
            ):
                global_pre_gate = gather_feature_rows(attention_output)
                global_gate = gather_feature_rows(query_gate)
                global_post_gate = gather_feature_rows(post_gate)
                engine_query_gate[name_phase] = {
                    "pre_gate": (
                        global_pre_gate.detach().float().cpu().clone()
                    ),
                    "gate": global_gate.detach().float().cpu().clone(),
                    "post_gate": (
                        global_post_gate.detach().float().cpu().clone()
                    ),
                }
            return post_gate
    ...
    if any(
        name not in official_query_gate
        for name in ("gate", "post_gate")
    ):
        raise RuntimeError("official full query gate was not captured")
    for name_phase in ("recompute", "restore"):
        if any(
            name not in engine_query_gate[name_phase]
            for name in ("pre_gate", "gate", "post_gate")
        ):
            raise RuntimeError(
                f"Engine {name_phase} query gate was not captured"
            )
    ...
    result = {
        ...
        "official_vs_recompute_query_gate_pre": compare(
            official_attention_core["full_output"].reshape(8, 256),
            engine_query_gate["recompute"]["pre_gate"].reshape(8, 256),
        ),
        "official_vs_recompute_query_gate_gate": compare(
            official_query_gate["gate"],
            engine_query_gate["recompute"]["gate"].reshape(8, 256),
        ),
        "official_vs_recompute_query_gate_post": compare(
            official_query_gate["post_gate"],
            engine_query_gate["recompute"]["post_gate"].reshape(8, 256),
        ),
        "official_vs_recompute_query_gate_post_segments": [
            {
                "tp_rank": head_index // 2,
                "head_index": head_index,
                **compare(
                    official_query_gate["post_gate"][head_index],
                    engine_query_gate["recompute"][
                        "post_gate"
        ...
        "recompute_vs_restore_query_gate_post": compare(
            engine_query_gate["recompute"]["post_gate"],
            engine_query_gate["restore"]["post_gate"],
        ),
        ...
        "schema_version": "qwen35.tp4-cached-decode-step19-layer7-query-gate.v6",
