...
def main():
    ...
    try:
        ...
        original_official_chunk = (
            official_linear.chunk_gated_delta_rule
        )
        ...
        def canonical_official_recurrent_state(value):
            value = value.squeeze(0)
            local_heads = (
                value.shape[0] // len(configuration.gpu_indices)
            )
            value = value.narrow(0, 0, local_heads)
            return (
                value.transpose(-1, -2)
                .detach()
                .float()
        ...
        def canonical_official_prefill_heads(value):
            local_heads = (
                value.shape[2] // len(configuration.gpu_indices)
            )
            return (
                value.narrow(2, 0, local_heads)
                .squeeze(0)
                .detach()
                .float()
                .cpu()
        ...
        def canonical_official_prefill_state(value):
            local_heads = (
                value.shape[1] // len(configuration.gpu_indices)
            )
            return (
                value.narrow(1, 0, local_heads)
                .squeeze(0)
                .transpose(-1, -2)
                .detach()
                .float()
        ...
        def wrapped_official_chunk(*args, **kwargs):
            ...
            capture_this_call = (
                query.shape[1] == len(prompt)
                and "query" not in official_prefill_chunk
            )
            if capture_this_call:
                official_prefill_chunk["query"] = (
                    canonical_official_prefill_heads(query)
                )
                official_prefill_chunk["key"] = (
                    canonical_official_prefill_heads(key)
                )
                official_prefill_chunk["value"] = (
                    canonical_official_prefill_heads(value_input)
                )
                official_prefill_chunk["decay"] = (
                    canonical_official_prefill_heads(
                        decay.unsqueeze(-1)
                    ).squeeze(-1)
                )
                official_prefill_chunk["beta"] = (
                    canonical_official_prefill_heads(
                        beta.unsqueeze(-1)
                    ).squeeze(-1)
                )
                if initial_state is None:
                    official_prefill_chunk["initial_state"] = (
                        torch.zeros(
                            query.shape[2]
                            // len(configuration.gpu_indices),
                            value_input.shape[-1],
                            query.shape[-1],
                            dtype=torch.float32,
                        )
                    ...
                    official_prefill_chunk["initial_state"] = (
                        canonical_official_prefill_state(
                            initial_state
                        )
                    )
            output = original_official_chunk(*args, **kwargs)
            if capture_this_call:
                official_prefill_chunk["core"] = (
                    canonical_official_prefill_heads(output[0])
                )
                official_prefill_chunk["final_state"] = (
                    canonical_official_prefill_state(output[1])
                )
            return output

        official_linear.chunk_gated_delta_rule = (
            wrapped_official_chunk
        )
        ...
        def wrapped_official_recurrent(*args, **kwargs):
            ...
            is_decode = query.shape[1] == 1
            if is_decode:
                official_linear_components[
                    "recurrent_state_before"
                ] = canonical_official_recurrent_state(state)
            value = original_official_recurrent(*args, **kwargs)
            if is_decode:
                official_linear_components[
                    "recurrent_state_after"
                ] = canonical_official_recurrent_state(value[1])
        ...
        with torch.inference_mode():
            ...
            official_prefill_chunk["cached_state"] = (
                canonical_official_prefill_state(
                    prefill.past_key_values.layers[0]
                    .recurrent_states[0]
                )
            )
            official_prefill_chunk["cached_state_dtype"] = str(
                prefill.past_key_values.layers[0]
                .recurrent_states[0].dtype
            )
    ...
    try:
        ...
        original_engine_chunk = engine_linear_globals[
        ...
        def wrapped_engine_chunk(*args, **kwargs):
            query, key, value_input, a, b = args[:5]
            name_phase = phase["name"]
            expected_prefill_tokens = {
                "recompute": len(prompt),
                "restore": len(
                    payload["continuations"][0]["suffix_token_ids"]
                ),
            }
            is_layer0_prefill = (
                name_phase in expected_prefill_tokens
                and query.shape[0] == expected_prefill_tokens[name_phase]
                and "query" not in engine_prefill_chunk[name_phase]
                and args[5] is engine_linear.A_log
                and args[6] is engine_linear.dt_bias
            )
            if is_layer0_prefill:
                capture = engine_prefill_chunk[name_phase]
                capture["query"] = (
                ...
                capture["value"] = (
                    value_input.detach().float().cpu().clone()
                )
                capture["decay"] = (
                    -args[5].float().exp()
                    * torch.nn.functional.softplus(
                        a.float() + args[6].float()
                    )
                ).detach().float().cpu().clone()
                capture["beta"] = (
                    b.sigmoid().detach().float().cpu().clone()
                )
                capture["initial_state"] = (
                    args[7].detach().float().cpu().clone()
                )
            output = original_engine_chunk(*args, **kwargs)
            if is_layer0_prefill:
                capture["core"] = (
                    output[0].detach().float().cpu().clone()
                )
                capture["final_state"] = (
                    output[1].detach().float().cpu().clone()
                )
            return output
        ...
        def wrapped_engine_recurrent(*args, **kwargs):
            ...
            value = original_engine_recurrent(*args, **kwargs)
            if is_layer0_decode:
                engine_linear_components[name_phase][
                    "recurrent_state_after"
        ...
        engine_linear_globals["qwen35_gated_delta_chunk"] = (
            wrapped_engine_chunk
        )
    ...
    official_prefill_cache_rounding = {
        "cache_vs_decode": compare(
            official_prefill_chunk["cached_state"],
            official_linear_components["recurrent_state_before"],
        ),
        "final_vs_cache": compare(
            official_prefill_chunk["final_state"],
            official_prefill_chunk["cached_state"],
        ),
        "final_bfloat16_roundtrip_vs_decode": compare(
            official_prefill_chunk["final_state"]
            .to(torch.bfloat16).float(),
            official_linear_components["recurrent_state_before"],
        ),
        "final_float16_roundtrip_vs_decode": compare(
            official_prefill_chunk["final_state"]
            .to(torch.float16).float(),
            official_linear_components["recurrent_state_before"],
        ),
        "cached_state_dtype": (
            official_prefill_chunk["cached_state_dtype"]
        ),
    }
    restore_suffix_state = {
        "initial_vs_prefix_snapshot_roundtrip_unknown": {
            "shape": list(
                engine_prefill_chunk["restore"]["initial_state"].shape
            ),
        },
        "final_vs_decode": compare(
            engine_prefill_chunk["restore"]["final_state"],
            engine_linear_components["restore"][
                "recurrent_state_before"
            ],
        ),
        "final_bfloat16_roundtrip_vs_decode": compare(
        ...
        "final_bfloat16_roundtrip_vs_official_decode": compare(
            engine_prefill_chunk["restore"]["final_state"]
            .to(torch.bfloat16).float(),
            official_linear_components["recurrent_state_before"],
        ),
        "final_vs_official_decode": compare(
            engine_prefill_chunk["restore"]["final_state"],
            official_linear_components["recurrent_state_before"],
        ),
    }
    prefill_to_decode_state = {
        ...
        "recompute": compare(
            engine_prefill_chunk["recompute"]["final_state"],
            engine_linear_components["recompute"][
                "recurrent_state_before"
            ],
        ),
    }
    ...
    official_vs_recompute_prefill_chunk = []
    for name in prefill_chunk_names:
        if (
            name not in official_prefill_chunk
            or name not in engine_prefill_chunk["recompute"]
        ):
            raise RuntimeError(
                f"missing layer0 prefill chunk capture: {name}"
            )
        official_vs_recompute_prefill_chunk.append({
            "name": name,
            **compare(
                official_prefill_chunk[name],
                engine_prefill_chunk["recompute"][name],
            ),
        })
    ...
    result = {
        "restore_suffix_state": restore_suffix_state,
        "official_prefill_cache_rounding": (
            official_prefill_cache_rounding
        ),
        "prefill_to_decode_state": prefill_to_decode_state,
        "official_vs_recompute_prefill_chunk": (
            official_vs_recompute_prefill_chunk
        ),
        "official_vs_recompute_first_prefill_chunk_mismatch": (
        ...
        "schema_version": "qwen35.tp4-restore-suffix-state.v1",
