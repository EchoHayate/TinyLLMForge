...
def main():
            ...
            **kwargs,
            ...
            if official_rotary_scope["active"] and query.shape[2] == 1:
                ...
                full_scores = torch.matmul(
                    query,
                    full_repeated_key.transpose(2, 3),
                ) * scaling
                if attention_mask is not None:
                ...
                full_replay_output = torch.matmul(
                    full_probabilities,
                    full_repeated_value,
                )
                full_contiguous_replay_output = torch.matmul(
                    full_probabilities.contiguous(),
                    full_repeated_value.contiguous(),
                )
                official_rank1_head2.update({
                    ...
                    "full_replay_output": (
                        full_replay_output[0, 2, 0]
                        .detach().float().cpu().clone()
                    ),
                    "full_probabilities": (
                        full_probabilities.detach().cpu().clone()
                    ),
                    "full_scores": full_scores.detach().cpu().clone(),
                    "full_value": (
                        full_repeated_value.detach().cpu().clone()
                    ),
                    "full_contiguous_replay_output": (
                        full_contiguous_replay_output[0, 2, 0]
                        .detach().float().cpu().clone()
                    ),
                    "full_value_stride": list(
                        full_repeated_value.stride()
                    ),
                    "pre_gate": (
    ...
    rank1_fixture_path = Path('/tmp/qwen35_attempt52_r436_softmax_layer7_step19_core.pt')
    ...
    engine_rank1_core_names = (
        ...
        "probabilities",
        "local_replay_output",
        "global_replay_output",
        "expanded_global_replay_output",
        "pre_gate",
        "gate",
        "post_gate",
    )
    for name in engine_rank1_core_names:
    ...
    torch.save(
        {
            "schema_version": (
                "qwen35.layer7-step19-full-softmax-reduction.v3"
            ),
            ...
            "global_head": 2,
            "full_probabilities": (
                official_rank1_head2["full_probabilities"]
            ),
            "full_scores": official_rank1_head2["full_scores"],
            "full_value": official_rank1_head2["full_value"],
            "expected_output": official_rank1_head2["pre_gate"],
            "engine_output": rank1_fixture["pre_gate"],
        },
        output / "full_softmax_reduction_fixture.pt",
    )
    ...
    result = {
        ...
        "official_actual_vs_official_full_replay": compare(
            official_rank1_head2["pre_gate"],
            official_rank1_head2["full_replay_output"],
        ),
        "official_actual_vs_official_full_contiguous_replay": compare(
            official_rank1_head2["pre_gate"],
            official_rank1_head2[
                "full_contiguous_replay_output"
            ],
        ),
        "official_actual_vs_engine_expanded_global_replay": compare(
            official_rank1_head2["pre_gate"],
            rank1_fixture["expanded_global_replay_output"],
        ),
        "official_full_value_stride": (
            official_rank1_head2["full_value_stride"]
        ),
        ...
        "schema_version": "qwen35.tp4-cached-decode-step19-rank1-head2-batch-geometry.v8",
