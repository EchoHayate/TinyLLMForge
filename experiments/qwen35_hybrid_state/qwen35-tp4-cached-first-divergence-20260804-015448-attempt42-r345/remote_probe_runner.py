...
def main():
    ...
    try:
        ...
        local_query_heads = official_attention.num_attention_heads // 4
        local_kv_heads = max(
            1,
            official_attention.num_key_value_heads // 4,
        )
        ...
        def save_official_q_projection(_module, _args, value):
            paired = value[:, -1].reshape(
                official_attention.num_attention_heads,
        ...
        def save_official_kv_projection(name):
            def hook(_module, _args, value):
                projected = value[:, -1].reshape(
                    official_attention.num_key_value_heads,
    ...
    try:
        ...
        def save_engine_rotary(_module, _args, value):
            query, key = value
            if query.shape[0] == 1:
                store_engine("rotary_query", query[-1])
                store_engine("rotary_key", key[-1])
            return value
    ...
    result = {
        ...
        "schema_version": "qwen35.tp4-cached-decode-step1-layer3-components.v1",
