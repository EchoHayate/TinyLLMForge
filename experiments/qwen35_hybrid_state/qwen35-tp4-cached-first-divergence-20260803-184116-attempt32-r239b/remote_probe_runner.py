...
def main():
    ...
    official_rows = {}
    official_native = {}
    official_attention_head_batch_variants = {}
        ...
        def wrapped_official_eager_attention(
            ...
            **kwargs,
            ...
            if module is attention:
                if query.shape[-2] == 1024:
                    local_query = query[:, :local_query_heads]
                    local_key = key[:, :local_kv_heads].repeat_interleave(
                    ...
                    def local_replay(*, additive_mask):
                        local_scores = torch.matmul(
                            local_query,
                            local_key.transpose(2, 3),
                        ) * kwargs["scaling"]
                    ...
                    official_attention_head_batch_variants[
                        "local_two_head_additive_mask"
                    ] = (
                        local_replay(additive_mask=True)[:, -64:]
                        .reshape(64, -1)
                        .detach().float().cpu().clone()
                    )
                    official_attention_head_batch_variants[
                        "local_two_head_masked_fill"
                    ] = (
                        local_replay(additive_mask=False)[:, -64:]
                        .reshape(64, -1)
                        .detach().float().cpu().clone()
                    )
    ...
    native = official_attention_head_batch_variants[
        "global_eight_head_native"
    ]
    official_global_vs_local_attention_variants = []
    for name in (
        "local_two_head_additive_mask",
        "local_two_head_masked_fill",
    ):
        official_global_vs_local_attention_variants.append({
            "name": name,
            **compare(
                native,
                official_attention_head_batch_variants[name],
            ),
        })
    result = {
        "schema_version": "qwen35.tp4-official-attention-head-batch.v1",
        "official_global_vs_local_attention_variants": (
            official_global_vs_local_attention_variants
        ),
        "claim_boundary": (
            "official attention head-batch diagnostic only; no "
            "cached-continuation correctness, Engine correctness, "
            "performance, cache, memory, compression, quality, or "
            "accuracy claim"
        ),
    }
