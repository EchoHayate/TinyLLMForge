...
def main():
    ...
    try:
        ...
        official_lm_head_input = {"value": None}
        official_lm_head_weight = (
            model.lm_head.weight.detach().cpu().clone()
        )

        def save_official_lm_head_input(_module, args):
            value = tensor(args)
            official_lm_head_input["value"] = (
    ...
    try:
        ...
        engine_lm_head_inputs = {
            "recompute": None,
            "restore": None,
        }

        def save_engine_lm_head_input(_module, args):
            name_phase = phase["name"]
            if name_phase in engine_lm_head_inputs:
                value = tensor(args)
                engine_lm_head_inputs[name_phase] = (
                    value.detach().cpu().clone()
                )
        ...
        if official_lm_head_input["value"] is None:
            raise RuntimeError("official lm_head input was not captured")
        if any(
            engine_lm_head_inputs[name_phase] is None
            for name_phase in ("recompute", "restore")
        ):
            raise RuntimeError("Engine lm_head input was not captured")

        lm_device = model.lm_head.weight.device
        engine_hidden = engine_lm_head_inputs["recompute"].to(lm_device)
        official_hidden = official_lm_head_input["value"].to(lm_device)
        full_weight = official_lm_head_weight.to(lm_device)
        shard_width = model.lm_head.num_embeddings_per_partition
        full_vocab = model.lm_head.num_embeddings
        if full_weight.shape[0] != full_vocab:
            raise RuntimeError(
                "official lm_head weight does not cover full vocabulary"
            )
        if full_vocab != shard_width * model.lm_head.tp_size:
            raise RuntimeError("unexpected lm_head partition geometry")

        with torch.inference_mode():
            official_hidden_full_replay = torch.nn.functional.linear(
                official_hidden,
                full_weight,
            )
            engine_hidden_full_replay = torch.nn.functional.linear(
                engine_hidden,
                full_weight,
            )
            engine_hidden_shard_replays = [
                torch.nn.functional.linear(
                    engine_hidden,
                    full_weight.narrow(
                        0,
                        shard_index * shard_width,
                        shard_width,
                    ),
                )
                for shard_index in range(model.lm_head.tp_size)
            ]
            engine_hidden_sliced_replay = torch.cat(
                engine_hidden_shard_replays,
                dim=-1,
            )
            rank0_local_replay = torch.nn.functional.linear(
                engine_hidden,
        ...
        lm_head_probe = {
            "official_hidden": official_hidden.detach().float().cpu(),
            "engine_hidden": engine_hidden.detach().float().cpu(),
            "official_hidden_full_replay": (
                official_hidden_full_replay.reshape(
                    -1,
                    official_hidden_full_replay.shape[-1],
                )[-1].detach().float().cpu()
            ),
            "engine_hidden_full_replay": (
                engine_hidden_full_replay.reshape(
                    -1,
                    engine_hidden_full_replay.shape[-1],
                )[-1].detach().float().cpu()
            ),
            "engine_hidden_sliced_replay": (
                engine_hidden_sliced_replay.reshape(
                    -1,
                    engine_hidden_sliced_replay.shape[-1],
                )[-1].detach().float().cpu()
            ),
            "rank0_local_replay": (
                rank0_local_replay.reshape(
                    -1,
                    rank0_local_replay.shape[-1],
                )[-1].detach().float().cpu()
            ),
            "rank0_weight_equal": bool(torch.equal(
                model.lm_head.weight,
                full_weight.narrow(0, 0, shard_width),
            )),
            "rank0_weight_max_abs_diff": float(
                (
                    model.lm_head.weight.float()
                    - full_weight.narrow(0, 0, shard_width).float()
                ).abs().max().item()
            ),
            "shard_width": shard_width,
            "tp_size": model.lm_head.tp_size,
        }
        del full_weight
    finally:
    ...
    official_logits = official_postprocess["logits"]
    engine_logits = engine_postprocess["recompute"]["logits"]
    shard_width = lm_head_probe["shard_width"]
    shard_comparisons = []
    for shard_index in range(lm_head_probe["tp_size"]):
        ...
        shard_comparisons.append({
            ...
            "start": start,
            "end": end,
            "official_vs_engine_gathered": compare(
                official_logits[start:end],
                engine_logits[start:end],
            ),
            "official_vs_sliced_replay": compare(
                official_logits[start:end],
                lm_head_probe["engine_hidden_sliced_replay"][start:end],
            ),
            "engine_gathered_vs_sliced_replay": compare(
                engine_logits[start:end],
                lm_head_probe["engine_hidden_sliced_replay"][start:end],
            ),
        })
    ...
    result = {
        "lm_head_probe": {
            "rank0_weight_equal": lm_head_probe["rank0_weight_equal"],
            "rank0_weight_max_abs_diff": (
                lm_head_probe["rank0_weight_max_abs_diff"]
            ),
            "tp_size": lm_head_probe["tp_size"],
            "shard_width": lm_head_probe["shard_width"],
            "official_vs_engine_hidden": compare(
                lm_head_probe["official_hidden"],
                lm_head_probe["engine_hidden"],
            ),
            "official_logits_vs_official_hidden_full_replay": compare(
                official_logits,
                lm_head_probe["official_hidden_full_replay"],
            ),
            "official_logits_vs_engine_hidden_full_replay": compare(
                official_logits,
                lm_head_probe["engine_hidden_full_replay"],
            ),
            "official_logits_vs_engine_hidden_sliced_replay": compare(
                official_logits,
                lm_head_probe["engine_hidden_sliced_replay"],
            ),
            "engine_gathered_vs_engine_hidden_full_replay": compare(
                engine_logits,
                lm_head_probe["engine_hidden_full_replay"],
            ),
            "engine_gathered_vs_engine_hidden_sliced_replay": compare(
                engine_logits,
                lm_head_probe["engine_hidden_sliced_replay"],
            ),
            "rank0_gathered_vs_rank0_local_replay": compare(
                engine_logits[:shard_width],
                lm_head_probe["rank0_local_replay"],
            ),
        ...
        "schema_version": "qwen35.tp4-cached-decode-step1-lm-head.v4",
