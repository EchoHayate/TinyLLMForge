...
def main():
    ...
    try:
        ...
        official_output_projection = {}

        def save_official_output_input(_module, args):
            value = tensor(args)[:, -1]
            official_output_projection["input"] = (
                value.detach().float().cpu().clone()
            )
            store_official(
                "output_input",
        ...
        def save_official_output_projection(_module, _args, value):
            original_value = value
            value = tensor(value)[:, -1]
            official_output_projection["output"] = (
                value.detach().float().cpu().clone()
            )
        ...
        hooks.append(
            official_attention.o_proj.register_forward_hook(
                save_official_output_projection
            )
        )
        hooks.append(
    ...
    try:
        ...
        engine_output_projection = {
            "recompute": {},
            "restore": {},
        }
        original_engine_output_projection_forward = (
            engine_attention.output_projection.forward
        )

        def wrapped_engine_output_projection(x):
            projection = engine_attention.output_projection
            if projection.tp_size > 1:
                gathered_parts = [
                    torch.empty_like(x)
                    for _ in range(projection.tp_size)
                ]
                torch.distributed.all_gather(gathered_parts, x)
                gathered_input = torch.cat(gathered_parts, dim=-1)
            else:
                gathered_input = x
            output = projection._linear_forward_unpartitioned(
                gathered_input.unsqueeze(0),
                projection.bias,
            ).squeeze(0)
            name_phase = phase["name"]
            if x.shape[0] == 1 and name_phase in engine_output_projection:
                replay = projection._linear_forward_unpartitioned(
                    gathered_input.unsqueeze(0),
                    projection.bias,
                ).squeeze(0)
                engine_output_projection[name_phase] = {
                    "local_input": (
                        x.detach().float().cpu().clone()
                    ),
                    "gathered_input": (
                        gathered_input.detach().float().cpu().clone()
                    ),
                    "output": (
                        output.detach().float().cpu().clone()
                    ),
                    "replay": (
                        replay.detach().float().cpu().clone()
                    ),
                }
            return output

        engine_attention.output_projection.forward = (
            wrapped_engine_output_projection
        )
        hooks.append(
        ...
        for hook in hooks:
            hook.remove()
        if "original_engine_output_projection_forward" in locals():
            engine_attention.output_projection.forward = (
                original_engine_output_projection_forward
            )
        cleanup = engine.exit()
    ...
    if any(
        name not in official_output_projection
        for name in ("input", "output")
    ):
        raise RuntimeError("official output projection was not captured")
    for name_phase in ("recompute", "restore"):
        if any(
            name not in engine_output_projection[name_phase]
            for name in (
                "local_input",
                "gathered_input",
                "output",
            ...
            raise RuntimeError(
                f"Engine {name_phase} output projection was not captured"
            )
    ...
    result = {
        ...
        "official_vs_recompute_output_projection_input": compare(
            official_output_projection["input"],
            engine_output_projection["recompute"]["gathered_input"],
        ),
        "official_vs_recompute_output_projection_segments": [
            {
                "tp_rank": tp_rank,
                "head_index": head_index,
                **compare(
                    official_output_projection["input"].reshape(
                        8,
                        256,
                    )[head_index],
                    engine_output_projection["recompute"][
                        "gathered_input"
                    ].reshape(8, 256)[head_index],
                ),
                "first_nonzero_index": (
                    None
                    if not torch.any(
            ...
            for head_index in range(tp_rank * 2, tp_rank * 2 + 2)
        ],
        "official_vs_recompute_output_projection_weight": compare(
            official_attention.o_proj.weight.detach().float().cpu(),
            engine_attention.output_projection.weight.detach().float().cpu(),
        ),
        "official_vs_recompute_output_projection_output": compare(
            official_output_projection["output"],
            engine_output_projection["recompute"]["output"],
        ),
        "engine_output_projection_output_vs_replay": compare(
            engine_output_projection["recompute"]["output"],
            engine_output_projection["recompute"]["replay"],
        ),
        "recompute_vs_restore_output_projection_input": compare(
            engine_output_projection["recompute"]["gathered_input"],
            engine_output_projection["restore"]["gathered_input"],
        ),
        "recompute_vs_restore_output_projection_output": compare(
            engine_output_projection["recompute"]["output"],
            engine_output_projection["restore"]["output"],
        ),
        "official_vs_recompute_layer7_components": (
            official_vs_recompute_layer7_components
        ),
        ...
        "schema_version": "qwen35.tp4-cached-decode-step19-layer7-projection-segments.v6",
