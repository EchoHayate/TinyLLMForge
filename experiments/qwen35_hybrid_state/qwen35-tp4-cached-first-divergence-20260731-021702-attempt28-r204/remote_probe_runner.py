...
def main():
    ...
    try:
        ...
        original_attention_forward = attention.attention_backend.forward

        def full_query_attention_forward(query, key, value):
            context = engine_get_context()
            query_rows = query.shape[0]
            target_rows = int(getattr(context, "max_seqlen_k", 0))
            if (
                getattr(context, "is_prefill", False)
                and target_rows > query_rows
                and getattr(context, "cu_seqlens_k", None) is not None
            ):
                ...
                original_cu_seqlens_q = context.cu_seqlens_q
                original_max_seqlen_q = context.max_seqlen_q
                context.cu_seqlens_q = context.cu_seqlens_k
                context.max_seqlen_q = context.max_seqlen_k
                try:
                    output = original_attention_forward(
                        padded_query,
                        key,
                        value,
                    )
                finally:
                    context.cu_seqlens_q = original_cu_seqlens_q
                    context.max_seqlen_q = original_max_seqlen_q
                return output[-query_rows:]
            return original_attention_forward(query, key, value)

        attention.attention_backend.forward = full_query_attention_forward
        ...
        for hook in hooks:
            hook.remove()
        attention.attention_backend.forward = original_attention_forward
        cleanup = engine.exit()
