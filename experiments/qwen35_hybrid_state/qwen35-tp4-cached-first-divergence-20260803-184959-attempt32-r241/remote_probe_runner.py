...
def main():
    ...
    try:
        ...
        def wrapped_official_eager_attention(
            ...
            **kwargs,
            ...
            if module is attention:
                if (
                    official_prefix_capture_active[0]
                    and query.shape[-2] == 1024
                ):
                    official_prefix_attention_inputs["rotary_query"] = (
                        query[:, :local_query_heads]
                        .transpose(1, 2).squeeze(0)
                        .detach().float().cpu().clone()
                    )
                    official_prefix_attention_inputs["rotary_key"] = (
                        key[:, :local_kv_heads]
                        .transpose(1, 2).squeeze(0)
                        .detach().float().cpu().clone()
                    )
                    official_prefix_attention_inputs["value"] = (
                        value[:, :local_kv_heads]
                        .transpose(1, 2).squeeze(0)
                        .detach().float().cpu().clone()
                    )
                    if attention_mask is None:
                        official_attention_mask_metadata.update({
                            "is_none": True,
                        })
                    else:
                        official_attention_mask_metadata.update({
                            ...
                            "minimum": float(attention_mask.min().item()),
                            "maximum": float(attention_mask.max().item()),
                            "negative_infinity_count": int(
                                torch.isneginf(attention_mask).sum().item()
                            ),
                            "zero_count": int(
                                torch.count_nonzero(
                                    attention_mask == 0
        ...
        official_prefix_attention_inputs = {}
        official_attention_mask_metadata = {}
    ...
    engine_recompute_prefix_attention_inputs = {}
    source_capture = {
    ...
    try:
        ...
        def save_engine_rotary(_module, args, value):
            ...
            if name_phase in engine_rows:
                ...
                if (
                    name_phase == "recompute"
                    and query.shape[0] == 1024
                    and "rotary_query"
                    not in engine_recompute_prefix_attention_inputs
                ):
                    engine_recompute_prefix_attention_inputs.update({
                        "rotary_query": (
                            query.reshape(
                                1024,
                                attention.local_query_heads,
        ...
        def save_engine_value_projection(_module, _args, value):
            name_phase = phase["name"]
            projected = tensor(value)
            if (
                name_phase == "recompute"
                and projected.shape[0] == 1024
                and "value" not in engine_recompute_prefix_attention_inputs
            ):
                engine_recompute_prefix_attention_inputs["value"] = (
                    projected.reshape(
                        1024,
                        attention.local_kv_heads,
                        attention.head_dim,
    ...
    official_prefix_vs_engine_full_attention_inputs = []
    for name in ("rotary_query", "rotary_key", "value"):
        if name not in official_prefix_attention_inputs:
            raise RuntimeError(
                f"missing official full attention input: {name}"
            )
        if name not in engine_recompute_prefix_attention_inputs:
            raise RuntimeError(
                f"missing Engine full attention input: {name}"
            )
        official_prefix_vs_engine_full_attention_inputs.append({
            "name": name,
            **compare(
                official_prefix_attention_inputs[name],
                engine_recompute_prefix_attention_inputs[name],
            ),
        })

    official_prefix_vs_engine_first_full_attention_input_mismatch = (
        first_mismatch(
            official_prefix_vs_engine_full_attention_inputs
        )
    )
    ...
    result = {
        ...
        "schema_version": "qwen35.tp4-layer3-prefix-attention-inputs.v1",
    ...
    print(json.dumps({
        ...
        "official_prefix_vs_engine_full_attention_inputs": (
            result["official_prefix_vs_engine_full_attention_inputs"]
        ),
        "official_attention_mask_metadata": (
            result["official_attention_mask_metadata"]
        ),
        "official_prefix_vs_recompute_first_layer3_component_mismatch": (
            result[
                "official_prefix_vs_recompute_first_layer3_component_mismatch"
