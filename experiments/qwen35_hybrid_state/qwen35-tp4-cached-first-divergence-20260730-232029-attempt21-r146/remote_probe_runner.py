...
def _install_model_capture(model, rank, receipt_dir):
    if getattr(model, "_r146_capture_installed", False):
        return
    ...
    def wrapped_gated(core, gate, weight, **kwargs):
        ...
        if weight.data_ptr() == linear.norm_weight.data_ptr():
            if not pending:
                raise RuntimeError("r146 gated capture lacks matching core")
    ...
    model._r146_capture_installed = True
...
def main():
    ...
    sitecustomize.write_text(
        "\n".join([
            ...
            ")",
            "if path:",
            "    name = '_tinyvllm_r146_rank_receipt_runner'",
    ...
    result = {
        "schema_version": "qwen35.tp4-cached-rank-input-receipts.v1",
