...
def main():
    ...
    try:
        ...
        def wrapped_official_chunk(*args, **kwargs):
            ...
            core = value[0]
            official_rows["layer0_core_full"] = (
                core[:, -1, :]
                .reshape(1, -1)
                .detach()
                .float()
        ...
        hooks.append(
            linear.out_proj.register_forward_hook(
                lambda _module, _args, value: (
                    official_rows.__setitem__(
                        "layer0_out_proj_current",
                        last_row(value),
                    )
                    or official_rows.__setitem__(
                        "layer0_out_proj_2d",
                        last_row(value),
                    )
                    or value
        ...
        hooks.append(
            text.layers[1].input_layernorm.register_forward_pre_hook(
    ...
    try:
        ...
        engine_original_out_linear = (
            linear.out_proj._linear_forward_unpartitioned
        )
        engine_local_core = {}
            ...
            def wrapped(
                ...
                **kwargs,
                ...
                if (
                    ...
                    engine_local_core[name_phase] = (
                        result[0][-1:].reshape(1, -1).detach().clone()
                    )
                return result
            return wrapped
        ...
        def wrapped_engine_out_linear(
            ...
            name_phase = phase["name"]
            if name_phase in engine_rows:
                local_core = engine_local_core.get(name_phase)
                if local_core is None:
                    raise RuntimeError(
                        f"missing layer0 local core for {name_phase}"
                    )
                gathered_core = [
                    torch.empty_like(local_core)
                ...
                engine_rows[name_phase]["layer0_core_full"] = last_row(
                    torch.cat(gathered_core, dim=-1)
                )
                engine_rows[name_phase]["layer0_gated_full"] = last_row(
                    gathered
                )
                ...
                engine_rows[name_phase]["layer0_out_proj_2d"] = (
                    last_row(alternative)
                )
            return engine_original_out_linear(gathered, bias)

        linear.out_proj._linear_forward_unpartitioned = types.MethodType(
            wrapped_engine_out_linear,
            linear.out_proj,
        )
        ...
        hooks.append(
            linear.out_proj.register_forward_hook(
                save_engine("layer0_out_proj_current")
            )
        )
        hooks.append(
            layer.post_attention_layernorm.register_forward_pre_hook(
    ...
    def compare(left, right):
        ...
        if left.ndim == 2 and left.shape[1] % 4 == 0:
            ...
            for shard_index in range(4):
                ...
                shard_nonzero = torch.nonzero(
                    shard_difference.reshape(-1),
                    as_tuple=False,
                )
                shard_first = (
                    None
                    if shard_nonzero.numel() == 0
                    else int(shard_nonzero[0].item())
                )
                shard = {
                ...
                if shard_first is not None:
                    global_index = start + shard_first
                    shard.update({
                        "first_nonzero_local_index": shard_first,
                        "first_nonzero_global_index": global_index,
                        "first_nonzero_left": float(
                            left.reshape(-1)[global_index].item()
                        ),
                        "first_nonzero_right": float(
                            right.reshape(-1)[global_index].item()
                        ),
                    })
                comparison["quarter_shards"].append(shard)
        return comparison
    ...
    result = {
        "schema_version": "qwen35.tp4-cached-core-gated-probe.v1",
