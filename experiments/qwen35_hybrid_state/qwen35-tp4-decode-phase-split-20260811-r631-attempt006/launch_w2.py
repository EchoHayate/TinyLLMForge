...
PREREQUISITES_DIRECTORY = PREREQUISITES.parent
COMMAND_TEMPLATE = (
    ROOT
    / "experiments/qwen35_hybrid_state/"
    "qwen35-tp4-decode-row-parallel-fp32-20260811-r630-attempt001/"
...
def build_prerequisites_archive(output: Path) -> None:
    with tarfile.open(output, "w") as handle:
        for path in sorted(PREREQUISITES_DIRECTORY.rglob("*")):
            if path.is_file() and not path.is_symlink():
                handle.add(
                    path,
                    arcname=path.relative_to(PREREQUISITES_DIRECTORY),
                    recursive=False,
                )
...
def main() -> int:
    ...
    prerequisites_archive = inputs / "prerequisites_bundle.tar"
    build_prerequisites_archive(prerequisites_archive)
    ...
    try:
        ...
        uploads = [
            ...
            (
                prerequisites_archive,
                f"{runner.SSH_TARGET}:{REMOTE}/prerequisites_bundle.tar",
            ),
            (
                workload,
        ...
        ssh(
            " && ".join([
                ...
                (
                    f"tar -xf "
                    f"{shlex.quote(REMOTE + '/prerequisites_bundle.tar')} "
                    f"-C {shlex.quote(REMOTE + '/prerequisites')}"
                ),
                (
                    f"test $(sha256sum "
