...
def main():
    ...
    try:
        ...
        original_output_projection_forward = (
            attention.output_projection.forward
        )
        prefix_projection_input = {}

        def wrapped_output_projection_forward(value):
            output_value = original_output_projection_forward(value)
            if phase["name"] == "recompute":
                prefix_count = len(prompt) - 64
                if value.shape[0] == prefix_count:
                    prefix_projection_input["value"] = value.detach().clone()
                    padded = torch.cat(
                        (
                            value,
                            value.new_zeros(64, value.shape[1]),
                        ),
                        dim=0,
                    )
                    engine_projection_replays["zero_padded_1088"] = (
                        original_output_projection_forward(padded)[
                            :prefix_count
                        ].detach().float().cpu().clone()
                    )
                elif (
                    value.shape[0] == 64
                    and "value" in prefix_projection_input
                ):
                    reconstructed = torch.cat(
                        (prefix_projection_input["value"], value),
                        dim=0,
                    )
                    engine_projection_replays[
                        "actual_reconstructed_1088"
                    ] = (
                        original_output_projection_forward(reconstructed)[
                            :prefix_count
                        ].detach().float().cpu().clone()
                    )
            return output_value
        ...
        attention.output_projection.forward = (
            wrapped_output_projection_forward
        )
    ...
    for replay_name in (
        ...
        if replay_name not in engine_projection_replays:
            raise RuntimeError(
                f"missing output projection replay: {replay_name}"
            )
    output_projection_replay_comparisons = {
        "official_vs_engine_normal_1024": compare(
            official_prefix_rows["layer3_attention_output"],
            engine_prefix_rows["layer3_attention_output"],
        ),
        "official_vs_zero_padded_engine_1088": compare(
            official_prefix_rows["layer3_attention_output"],
            engine_projection_replays["zero_padded_1088"],
        ),
        "official_vs_actual_reconstructed_engine_1088": compare(
            official_prefix_rows["layer3_attention_output"],
            engine_projection_replays["actual_reconstructed_1088"],
        ),
        "zero_padded_vs_actual_reconstructed_engine_1088": compare(
            engine_projection_replays["zero_padded_1088"],
            engine_projection_replays["actual_reconstructed_1088"],
        ),
    }
    ...
    result = {
        ...
        "schema_version": "qwen35.tp4-cached-layer3-output-projection-replay.v1",
    ...
    print(json.dumps({
        "output_projection_replay_comparisons": (
            result["output_projection_replay_comparisons"]
        ),
