...
def main():
    ...
    for name in (
        ...
        if name not in cross_gpu_rows:
            raise RuntimeError(
                f"missing cross-GPU prefix capture: {name}"
            )
    ...
    cross_gpu_prefix_comparisons = {
        "official_gpu2_full_input_vs_gpu4_prefix_input": compare(
            official_projection_input[:prefix_count],
            cross_gpu_rows["layer3_output_input_full"],
        ),
        "official_gpu2_rank1_slice_vs_gpu4_rank1_slice": compare(
        ...
        "official_gpu4_rank1_slice_vs_engine_rank1": compare(
            cross_gpu_rows["layer3_output_input_full"][
                :, local_width:2 * local_width
            ],
            engine_projection_gather["rank_inputs"][1],
        ),
        "official_gpu2_attention_output_vs_gpu4_prefix": compare(
            official_native["layer3_attention_output_full"][
                :, :prefix_count
            ].squeeze(0).detach().float().cpu(),
            cross_gpu_rows["layer3_attention_output"],
    ...
    result = {
        ...
        "schema_version": "qwen35.tp4-cached-layer3-cross-gpu-prefix.v1",
