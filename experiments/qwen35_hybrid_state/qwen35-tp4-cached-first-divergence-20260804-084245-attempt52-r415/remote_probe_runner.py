...
def main():
    ...
    full_attention_path = (
        source / "tinyvllm" / "layers" / "qwen35_full_attention.py"
    )
    full_attention_source = full_attention_path.read_text(
        encoding="utf-8"
    )
    old_query_gate = '''        gated_output = qwen35_apply_query_gate(
            attention_output,
            query_gate,
        )
'''
    new_query_gate = '''        if cached_decode_eager:
            local_heads = self.local_query_heads
            ...
            global_attention = torch.zeros(
                (token_count, global_heads, self.head_dim),
            ...
            global_attention[:, head_start:head_end] = (
            ...
            global_gate[:, head_start:head_end] = query_gate.view(
                token_count,
                local_heads,
                self.head_dim,
            )
            gated_output = qwen35_apply_query_gate(
                global_attention,
                global_gate,
            )[:, head_start:head_end].reshape_as(attention_output)
        else:
            gated_output = qwen35_apply_query_gate(
                attention_output,
                query_gate,
...
'''
    if full_attention_source.count(old_query_gate) != 1:
        raise RuntimeError("remote query gate source anchor changed")
    full_attention_path.write_text(
        full_attention_source.replace(
            old_query_gate,
            new_query_gate,
            1,
        ),
    ...
    result = {
        ...
        "schema_version": "qwen35.tp4-cached-decode-step19-query-gate-source-candidate.v5",
