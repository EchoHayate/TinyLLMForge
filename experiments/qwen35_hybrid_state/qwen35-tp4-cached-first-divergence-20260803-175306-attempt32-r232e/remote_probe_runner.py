...
def main():
    ...
    try:
        ...
        def save_engine_layer4(name):
            def hook(_module, _args, value):
                ...
                if (
                    ...
                    source_capture[name] = tail(value)
        ...
        def wrapped_linear_conv(*args, **kwargs):
            ...
            if layer4_active[0] and (
                ...
                if name_phase == "source":
                    source_capture["layer4_conv_state"] = (
                        conv_state.detach().float().cpu().clone()
                    )
                    source_capture["layer4_conv_candidate_state"] = (
    ...
    source_projection_tail4 = source_capture[
        "layer4_projected_qkv"
    ][-4:]
    source_state_comparisons = {
        "source_candidate_state_vs_source_projection_tail4": compare(
            source_capture["layer4_conv_candidate_state"],
            source_projection_tail4.transpose(0, 1).contiguous(),
        ),
        "restore_input_state_vs_source_candidate_state": compare(
            engine_rows["restore"]["layer4_conv_state"],
            source_capture["layer4_conv_candidate_state"],
        ),
        "source_projection_tail4_vs_official_prefix_tail4": compare(
            source_projection_tail4,
            official_prefix[-4:],
        ),
        "source_projection_tail3_vs_official_prefix_tail3": compare(
            source_projection_tail4[-3:],
            official_prefix[-3:],
        ),
    }
    ...
    result = {
        ...
        "schema_version": "qwen35.tp4-cached-layer4-source-state.v1",
