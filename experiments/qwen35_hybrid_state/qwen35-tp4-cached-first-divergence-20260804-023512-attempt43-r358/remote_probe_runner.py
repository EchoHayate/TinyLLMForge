...
def main():
            ...
            **kwargs,
            ...
            if official_rotary_scope["active"] and query.shape[2] == 1:
                ...
                official_attention_core["key"] = (
                    key[0, :local_kv_heads]
                    .transpose(0, 1).detach().float().cpu().clone()
                )
                official_attention_core["value"] = (
                    value[0, :local_kv_heads]
                    .transpose(0, 1).detach().float().cpu().clone()
                )
                ...
                official_attention_core["scaling"] = float(scaling)
            return output_value
    ...
    try:
        ...
        def save_engine_attention_core(_module, args, value):
            ...
            cached_value = engine_attention.v_cache[block_ids].reshape(
                -1,
                engine_attention.local_kv_heads,
                engine_attention.head_dim,
            )[:context_length]
            ...
            scores = torch.matmul(
                query_heads.unsqueeze(1),
                repeated_key.transpose(0, 1).transpose(1, 2),
            ) * (engine_attention.head_dim ** -0.5)
            probabilities = torch.softmax(
                scores,
                dim=-1,
                dtype=torch.float32,
            ).to(query.dtype)
            eager_output = torch.matmul(
                probabilities,
                repeated_value.transpose(0, 1),
            ).squeeze(1)
            engine_attention_core[name_phase] = {
                "query": query_heads.detach().float().cpu().clone(),
                "key": key.detach().float().cpu().clone(),
                "value": cached_value.detach().float().cpu().clone(),
                "flash_output": (
                ...
                "scaling": float(engine_attention.head_dim ** -0.5),
            }
            return value
    ...
    if any(
        ...
        for name in required_attention_core
    ):
        raise RuntimeError("official decode attention core was not captured")
    for name_phase in ("recompute", "restore"):
        if any(
            ...
            raise RuntimeError(
                f"Engine {name_phase} attention core was not captured"
            )
    ...
    result = {
        ...
        "official_vs_recompute_attention_key_cache": compare(
            official_attention_core["key"],
            engine_attention_core["recompute"]["key"],
        ),
        "official_vs_recompute_attention_value_cache": compare(
            official_attention_core["value"],
        ...
        "recompute_vs_restore_attention_key_cache": compare(
            engine_attention_core["recompute"]["key"],
            engine_attention_core["restore"]["key"],
        ),
        "recompute_vs_restore_attention_value_cache": compare(
            engine_attention_core["recompute"]["value"],
        ...
        "schema_version": "qwen35.tp4-cached-decode-step1-layer3-attention-core.v3",
