...
def main():
    ...
    try:
        ...
        original_down_forward = layer.mlp.down_proj.forward

        def padded_down_forward(hidden_states):
            context = get_context()
            if not context.is_prefill:
                return original_down_forward(hidden_states)
            ...
            if (
                ...
                for index in range(len(q_offsets) - 1):
                    ...
                    outputs.append(
                        original_down_forward(segment)[-q_length:]
                    )
            ...
            return original_down_forward(padded)[-hidden_states.shape[0]:]

        layer.mlp.down_proj.forward = padded_down_forward
    ...
    result = {
        "schema_version": "qwen35.tp4-cached-layer0-probe.v1",
