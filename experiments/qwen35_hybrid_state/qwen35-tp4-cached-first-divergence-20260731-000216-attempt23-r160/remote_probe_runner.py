    ...
    try:
        ...
        def full_head_engine_chunk(
            ...
            **kwargs,
            ...
            global_heads = local_heads * linear.in_proj_z.tp_size
            rank = linear.in_proj_z.tp_rank
            ...
            def pad_tokens(tensor):
                ...
                padded[:, start:start + local_heads] = tensor
                return padded

            def pad_heads(tensor):
                padded = tensor.new_zeros(
                    global_heads,
                    *tensor.shape[1:],
                )
            ...
            output, candidate = engine_original_chunk_local(
                pad_tokens(query),
                pad_tokens(key),
                pad_tokens(value),
                pad_tokens(a.unsqueeze(-1)).squeeze(-1),
                pad_tokens(b.unsqueeze(-1)).squeeze(-1),
                pad_heads(A_log),
                pad_heads(dt_bias),
                pad_heads(recurrent_state),
                **kwargs,
            )
            return (
                output[:, start:start + local_heads],
                candidate[start:start + local_heads],
            )

        engine_original_chunk = full_head_engine_chunk
