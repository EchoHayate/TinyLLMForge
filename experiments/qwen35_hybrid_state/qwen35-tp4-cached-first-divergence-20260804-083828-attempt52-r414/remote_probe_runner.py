...
def main():
    ...
    try:
        ...
        engine_query_gate_scope = {"active": False}
        original_engine_query_gate = (
            engine_full_module.qwen35_apply_query_gate
        )
        engine_query_gate_candidate = {
            "recompute": {},
            "restore": {},
        }

        def enter_engine_query_gate_scope(_module, _args):
            engine_query_gate_scope["active"] = True

        def exit_engine_query_gate_scope(_module, _args, value):
            engine_query_gate_scope["active"] = False
            return value

        def wrapped_engine_query_gate(attention_output, query_gate):
            if (
                not engine_query_gate_scope["active"]
                or attention_output.shape[0] != 1
            ):
                return original_engine_query_gate(
                    attention_output,
                    query_gate,
                )
            local_heads = engine_attention.local_query_heads
            head_dim = engine_attention.head_dim
            world_size = torch.distributed.get_world_size()
            rank = torch.distributed.get_rank()
            global_heads = local_heads * world_size
            local_attention = attention_output.reshape(
                1,
                local_heads,
                head_dim,
            )
            local_gate = query_gate.reshape(
                1,
                local_heads,
                head_dim,
            )
            global_attention = torch.zeros(
                (1, global_heads, head_dim),
                dtype=attention_output.dtype,
                device=attention_output.device,
            )
            global_gate = torch.zeros(
                (1, global_heads, head_dim),
                dtype=query_gate.dtype,
                device=query_gate.device,
            )
            head_start = rank * local_heads
            head_end = head_start + local_heads
            global_attention[:, head_start:head_end] = local_attention
            global_gate[:, head_start:head_end] = local_gate
            global_post_gate = original_engine_query_gate(
                global_attention,
                global_gate,
            )
            candidate = global_post_gate[
                :, head_start:head_end
            ].reshape_as(attention_output)
            name_phase = phase["name"]
            if name_phase in engine_query_gate_candidate:
                baseline = original_engine_query_gate(
                    attention_output,
                    query_gate,
                )
                engine_query_gate_candidate[name_phase] = {
                    "baseline": (
                        baseline.detach().float().cpu().clone()
                    ),
                    "candidate": (
                        candidate.detach().float().cpu().clone()
                    ),
                    "rank": rank,
        ...
        hooks.append(
            engine_attention.register_forward_pre_hook(
                enter_engine_query_gate_scope
            )
        )
        hooks.append(
            engine_attention.register_forward_hook(
                exit_engine_query_gate_scope
            )
        )
        engine_full_module.qwen35_apply_query_gate = (
            wrapped_engine_query_gate
        )
        ...
        for hook in hooks:
            hook.remove()
        if "original_engine_query_gate" in locals():
            engine_full_module.qwen35_apply_query_gate = (
                original_engine_query_gate
            )
        cleanup = engine.exit()
    ...
    for name_phase in ("recompute", "restore"):
        if not engine_query_gate_candidate[name_phase]:
            raise RuntimeError(
                f"Engine {name_phase} query gate candidate missing"
            )
    ...
    result = {
        ...
        "engine_query_gate_candidate": {
            "baseline_vs_candidate": compare(
                engine_query_gate_candidate["recompute"]["baseline"],
                engine_query_gate_candidate["recompute"]["candidate"],
            ),
            "rank": engine_query_gate_candidate["recompute"]["rank"],
            "global_heads": engine_query_gate_candidate[
                "recompute"
            ]["global_heads"],
        },
        "engine_query_gate_candidate_restore": {
            "baseline_vs_candidate": compare(
                engine_query_gate_candidate["restore"]["baseline"],
                engine_query_gate_candidate["restore"]["candidate"],
            ),
            "rank": engine_query_gate_candidate["restore"]["rank"],
            "global_heads": engine_query_gate_candidate[
                "restore"
            ]["global_heads"],
        },
        ...
        "schema_version": "qwen35.tp4-cached-decode-step19-query-gate-global-geometry.v5",
