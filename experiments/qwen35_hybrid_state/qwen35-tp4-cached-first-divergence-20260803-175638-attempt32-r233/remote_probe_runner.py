...
def main():
    ...
    if len(source_projection_calls) != 2 or len(source_candidate_calls) != 2:
        raise RuntimeError(
            "expected exactly two source prefill layer4 conv calls"
        )
    source_first_projection_tail4 = source_projection_calls[0][-4:]
    source_second_projection_tail4 = source_projection_calls[1][-4:]
    source_state_comparisons = {
        ...
        "source_second_candidate_vs_source_second_projection_tail4": compare(
            source_candidate_calls[1],
            source_second_projection_tail4.transpose(0, 1).contiguous(),
        ),
    }
    ...
    result = {
        "source_call_token_counts": [
            int(rows.shape[0]) for rows in source_projection_calls
        ],
        ...
        "schema_version": "qwen35.tp4-cached-layer4-publication-boundary.v1",
