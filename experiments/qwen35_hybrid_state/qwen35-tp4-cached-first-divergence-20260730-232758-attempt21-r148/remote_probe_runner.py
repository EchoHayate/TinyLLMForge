...
def _install_model_capture(model, rank, receipt_dir):
    if getattr(model, "_r148_capture_installed", False):
        return
    ...
    original_in_proj_z_forward = linear.in_proj_z.forward
    counters = {"core": 0, "gate": 0, "weight": 0, "gated": 0}
    pending = []

    def padded_in_proj_z_forward(hidden_states):
        from tinyvllm.utils.context import get_context

        context = get_context()
        target_rows = int(context.max_seqlen_k)
        current_rows = int(hidden_states.shape[0])
        if (
            hidden_states.ndim == 2
            and context.is_prefill
            and target_rows > current_rows
        ):
            padded = torch.cat(
                (
                    torch.zeros(
            ...
            full_output = linear.in_proj_z._linear_forward_unpartitioned(
                padded,
                linear.in_proj_z.bias,
            )
            local_output = full_output.narrow(
                -1,
                linear.in_proj_z.tp_rank
                * linear.in_proj_z.output_size_per_partition,
                linear.in_proj_z.output_size_per_partition,
            )
            return local_output[-current_rows:]
        return original_in_proj_z_forward(hidden_states)

    linear.in_proj_z.forward = padded_in_proj_z_forward
    ...
    def wrapped_gated(core, gate, weight, **kwargs):
        ...
        if weight.data_ptr() == linear.norm_weight.data_ptr():
            if not pending:
                raise RuntimeError("r148 gated capture lacks matching core")
    ...
    model._r148_capture_installed = True
...
def main():
    ...
    sitecustomize.write_text(
        "\n".join([
            ...
            ")",
            "if path:",
            "    name = '_tinyvllm_r148_rank_receipt_runner'",
    ...
    result = {
        "schema_version": "qwen35.tp4-cached-rank-input-padding-ab.v1",
