...
def main():
    ...
    try:
        ...
        def summarize_token_major(tensor_value, block_size):
            ...
            token_float = tensor_value.float()
            token_stats = torch.stack(
                (
                    token_float.sum(dim=-1),
                    token_float.abs().sum(dim=-1),
                    token_float.square().sum(dim=-1),
                    token_float.amax(dim=-1),
                    token_float.amin(dim=-1),
                ),
                dim=-1,
            )
            return {
    ...
    try:
        ...
        def save_engine_attention_core(_module, args, value):
            ...
            block_size = engine_attention.attention_backend.k_cache.shape[1]
            ...
            key = engine_attention.attention_backend.k_cache[block_ids].reshape(
                -1,
                engine_attention.local_kv_heads,
                engine_attention.head_dim,
            )[:context_length]
            cached_value = engine_attention.attention_backend.v_cache[
                block_ids
            ].reshape(
                -1,
                engine_attention.local_kv_heads,
            ...
            repeated_value = cached_value.repeat_interleave(
                repeats,
                dim=1,
            ).transpose(0, 1).unsqueeze(0)
            row_query = query_heads.unsqueeze(0).unsqueeze(2)
            bf16_scores = torch.matmul(
                row_query,
                repeated_key.transpose(2, 3),
            ) * scale
            bf16_probabilities = torch.softmax(
                bf16_scores,
                dim=-1,
                dtype=torch.float32,
            ).to(query.dtype)
            bf16_eager_output = torch.matmul(
                bf16_probabilities,
                repeated_value,
            ).squeeze(0).squeeze(1)
            fp32_scores = torch.matmul(
                row_query.float(),
                repeated_key.float().transpose(2, 3),
            ) * scale
            fp32_probabilities = torch.softmax(
                fp32_scores,
                dim=-1,
                dtype=torch.float32,
            )
            fp32_eager_output = torch.matmul(
                fp32_probabilities,
            ...
            engine_attention_core[name_phase] = {
                ...
                "bf16_eager_output": (
                    bf16_eager_output.detach().float().cpu().clone()
                ),
                "fp32_eager_output": (
                    fp32_eager_output.detach().cpu().clone()
                ),
                "score_maxima": (
    ...
    result = {
        ...
        "official_vs_recompute_key_token_stats": compare(
            official_attention_core["key_summary"]["token_stats"],
            engine_attention_core["recompute"][
                "key_summary"
            ]["token_stats"],
        ),
        ...
        "official_vs_recompute_value_token_stats": compare(
            official_attention_core["value_summary"]["token_stats"],
            engine_attention_core["recompute"][
                "value_summary"
            ]["token_stats"],
        ),
        ...
        "official_vs_recompute_attention_bf16_eager": compare(
            official_attention_core["output"],
            engine_attention_core["recompute"]["bf16_eager_output"],
        ),
        "official_vs_recompute_attention_fp32_eager": compare(
            official_attention_core["output"],
            engine_attention_core["recompute"]["fp32_eager_output"],
        ),
        "engine_flash_vs_eager_attention_output": compare(
            engine_attention_core["recompute"]["flash_output"],
            engine_attention_core["recompute"]["eager_output"],
        ),
        "engine_flash_vs_bf16_eager_attention_output": compare(
            engine_attention_core["recompute"]["flash_output"],
            engine_attention_core["recompute"]["bf16_eager_output"],
        ),
        "engine_bf16_vs_fp32_eager_attention_output": compare(
            engine_attention_core["recompute"]["bf16_eager_output"],
            engine_attention_core["recompute"]["fp32_eager_output"],
        ),
        ...
        "recompute_vs_restore_key_token_stats": compare(
            engine_attention_core["recompute"][
                "key_summary"
            ]["token_stats"],
            engine_attention_core["restore"][
        ...
        "recompute_vs_restore_value_token_stats": compare(
            engine_attention_core["recompute"][
                "value_summary"
            ]["token_stats"],
            engine_attention_core["restore"][
        ...
        "recompute_vs_restore_attention_bf16_eager_output": compare(
            engine_attention_core["recompute"]["bf16_eager_output"],
            engine_attention_core["restore"]["bf16_eager_output"],
        ),
        "recompute_vs_restore_attention_fp32_eager_output": compare(
            engine_attention_core["recompute"]["fp32_eager_output"],
            engine_attention_core["restore"]["fp32_eager_output"],
        ),
        "official_vs_recompute_layer3_components": (
            official_vs_recompute_layer3_components
        ),
        ...
        "schema_version": "qwen35.tp4-cached-decode-step1-layer3-attention-dtype.v6",
