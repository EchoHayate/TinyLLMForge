...
def main():
    ...
    try:
        ...
        def save_official_layer20(name):
            def hook(_module, _args, value):
        ...
        def save_official_layer20_input(name):
        ...
        hooks.append(
            official_layer20.input_layernorm.register_forward_pre_hook(
                save_official_layer20_input("input")
            )
        )
        hooks.append(
            official_layer20.linear_attn.register_forward_hook(
                save_official_layer20("linear_output")
            )
        )
        hooks.append(
            official_layer20.post_attention_layernorm
            .register_forward_pre_hook(
                save_official_layer20_input("attention_residual")
            )
        )
        hooks.append(
            official_layer20.post_attention_layernorm.register_forward_hook(
                save_official_layer20("post_norm")
            )
        )
        hooks.append(
            official_layer20.mlp.register_forward_hook(
                save_official_layer20("mlp")
            )
        )
    ...
    try:
        ...
        def save_engine_layer20(name):
            def hook(_module, _args, value):
                name_phase = phase["name"]
                if name_phase in engine_layer20_components:
                    tensor_value = tensor(value)
                    engine_layer20_components[name_phase][name] = (
                        tensor_value.reshape(
                            -1,
                            tensor_value.shape[-1],
                        )[-1].detach().float().cpu().clone()
        ...
        def save_engine_layer20_input(name):
            def hook(_module, args):
                name_phase = phase["name"]
                if name_phase in engine_layer20_components:
                    tensor_value = tensor(args)
        ...
        hooks.append(
            engine_layer20.input_layernorm.register_forward_pre_hook(
                save_engine_layer20_input("input")
            )
        )
        hooks.append(
            engine_layer20.linear_attention.register_forward_hook(
                save_engine_layer20("linear_output")
            )
        )
        hooks.append(
            engine_layer20.post_attention_layernorm
            .register_forward_pre_hook(
                save_engine_layer20_input("attention_residual")
            )
        )
        hooks.append(
            engine_layer20.post_attention_layernorm.register_forward_hook(
                save_engine_layer20("post_norm")
            )
        )
        hooks.append(
            engine_layer20.mlp.register_forward_hook(
                save_engine_layer20("mlp")
            )
        )
        engine_postprocess = {
            "recompute": {},
    ...
    for name in layer20_names:
        if name not in official_layer20_components:
            raise RuntimeError(f"missing official layer20 capture: {name}")
        for name_phase in ("recompute", "restore"):
            if name not in engine_layer20_components[name_phase]:
                raise RuntimeError(
                    f"missing Engine {name_phase} layer20 capture: {name}"
                )
    ...
    result = {
        ...
        "schema_version": "qwen35.tp4-cached-request1-step38-layer20-linear.v11",
