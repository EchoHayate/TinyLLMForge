...
def main():
    ...
    try:
        ...
        def canonical_official_conv_storage(value):
            if value.ndim == 3:
                value = value.squeeze(0)
            tp_rank = 0
            tp_size = len(configuration.gpu_indices)
            ...
            for global_width in (
                ...
                global_offset += global_width
            return torch.cat(pieces, dim=0).detach().float().cpu().clone()

        def canonical_engine_conv_storage(value):
            return value.detach().float().cpu().clone()

        def wrapped_official_conv_update(*args, **kwargs):
            mixed = args[0] if args else kwargs["hidden_states"]
            state = args[1] if len(args) > 1 else kwargs["conv_state"]
            weight = args[2] if len(args) > 2 else kwargs["weight"]
            is_decode = mixed.shape[-1] == 1
            if is_decode:
                official_conv_raw["conv_state_before"] = (
                    state.squeeze(0).detach().float().cpu().clone()
                )
                official_conv_raw["conv_weight"] = (
                    weight.detach().float().cpu().clone()
                )
                official_linear_components["conv_state_before"] = (
                    canonical_official_conv_storage(state.clone())
                )
                official_linear_components["conv_weight"] = (
                    canonical_official_conv_storage(weight)
                )
            value = original_official_conv_update(*args, **kwargs)
            if is_decode:
                official_conv_raw["conv_state_after"] = (
                    state.squeeze(0).detach().float().cpu().clone()
                )
                official_linear_components["conv_state_after"] = (
                    canonical_official_conv_storage(state)
                )
        ...
        def wrapped_engine_conv(*args, **kwargs):
            ...
            if is_decode:
                engine_linear_components[name_phase][
                    "conv_state_before"
                ] = canonical_engine_conv_storage(state)
                engine_linear_components[name_phase][
                    "conv_weight"
                ] = canonical_engine_conv_storage(weight)
            value = original_engine_conv(*args, **kwargs)
            if is_decode:
                engine_linear_components[name_phase][
                    "conv_state_after"
                ] = canonical_engine_conv_storage(value[1])
    ...
    def official_conv_layout_candidates(name):
        raw = official_conv_raw[name]
        tp_rank = int(engine_linear.in_proj_qkv.tp_rank)
        tp_size = int(engine_linear.in_proj_qkv.tp_size)
        local_rows = raw.shape[0] // tp_size
        contiguous = raw.narrow(
            0,
            tp_rank * local_rows,
            local_rows,
        )
        segmented = canonical_official_conv_storage(raw)
        return {
            "segmented": segmented,
            "segmented_kernel_flip": segmented.flip(-1),
            "contiguous": contiguous,
            "contiguous_kernel_flip": contiguous.flip(-1),
        }
    ...
    def tensor_fingerprint(value):
        ...
        return {
            ...
            "first_values": [
                float(item)
                for item in flat[:8].tolist()
            ],
        }

    official_weight_rank_comparisons = {}
    raw_weight = official_conv_raw["conv_weight"]
    tp_size = int(engine_linear.in_proj_qkv.tp_size)
    engine_weight = engine_linear_components["recompute"]["conv_weight"]
    key_width = official_linear.key_dim
    value_width = official_linear.value_dim
    for candidate_rank in range(tp_size):
        pieces = []
        global_offset = 0
        for global_width in (
            key_width,
            ...
            local_width = global_width // tp_size
            pieces.append(raw_weight.narrow(
                0,
                global_offset + candidate_rank * local_width,
                local_width,
            ))
            global_offset += global_width
        official_weight_rank_comparisons[str(candidate_rank)] = compare(
            torch.cat(pieces, dim=0),
            engine_weight,
        )

    conv_weight_fingerprints = {
        "official_full": tensor_fingerprint(raw_weight),
        "official_rank0_segmented": tensor_fingerprint(
            canonical_official_conv_storage(raw_weight)
        ),
        "engine_rank0": tensor_fingerprint(engine_weight),
        "engine_buffer_direct": tensor_fingerprint(
            engine_linear.conv_weight.detach().float().cpu()
        ),
    }

    conv_layout_comparisons = {}
    for name in (
        "conv_state_before",
        "conv_weight",
        "conv_state_after",
    ):
        right = engine_linear_components["recompute"][name]
        conv_layout_comparisons[name] = {
            layout: compare(left, right)
            for layout, left in official_conv_layout_candidates(name).items()
        }
    ...
    result = {
        ...
        "conv_weight_fingerprints": conv_weight_fingerprints,
        "conv_layout_comparisons": conv_layout_comparisons,
        "official_vs_recompute_linear_components": (
            official_vs_recompute_linear_components
        ),
        "recompute_vs_restore_linear_components": (
        ...
        "schema_version": "qwen35.tp4-cached-decode-step1-layer0-conv-weight-identity.v1",
