...
def main():
    ...
    try:
        ...
        def wrapped_engine_chunk(*args, **kwargs):
            ...
            name_phase = phase["name"]
            is_layer0_prefill = (
                name_phase in engine_prefill_chunk
                and query.shape[0] > 1
    ...
    for name in prefill_chunk_names:
        ...
        recompute_vs_restore_prefill_chunk.append({
            "name": name,
            **compare(
                engine_prefill_chunk["recompute"][name],
                engine_prefill_chunk["restore"][name],
            ),
        })
    ...
    result = {
        ...
        "recompute_vs_restore_first_prefill_chunk_mismatch": (
            first_component_mismatch(
                recompute_vs_restore_prefill_chunk
            )
        ),
