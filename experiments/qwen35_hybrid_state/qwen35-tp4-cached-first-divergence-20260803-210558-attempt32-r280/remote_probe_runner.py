...
def main():
    ...
    import tinyvllm.layers.linear as engine_linear_primitives
    ...
    try:
        ...
        def save_layer_output(layer_index):
            def hook(_module, _args, value):
                ...
                full_value = tensor(value)
                official_prefix_layer_outputs[str(layer_index)] = (
                    full_value[:, :-64].squeeze(0)
        ...
        def official_prefix(value):
            value = tensor(value)
            if value.ndim < 3 or value.shape[0] != 1:
                raise RuntimeError(
                    f"unexpected official prefix tensor shape: {value.shape}"
                )
            return (
                value[:, :-64].squeeze(0)
                .detach().float().cpu().clone()
        ...
        def save(name):
            def hook(_module, _args, value):
                ...
                official_prefix_rows[name] = official_prefix(value)
                return value
            return hook
        ...
        def save_input(name):
            def hook(_module, args):
                ...
                official_prefix_rows[name] = official_prefix(args)
            return hook
        ...
        def save_q_projection(_module, _args, value):
            ...
            prefix = value[:, :-64].reshape(
                len(prompt) - 64,
                attention.config.num_attention_heads,
                2 * attention.head_dim,
            )[:, :local_query_heads]
            official_prefix_rows["layer3_q_projection_local"] = (
                prefix.reshape(len(prompt) - 64, -1)
                .detach().float().cpu().clone()
            )
            official_prefix_rows["layer3_query_gate_local"] = (
                prefix[..., attention.head_dim:]
                .reshape(len(prompt) - 64, -1)
                .detach().float().cpu().clone()
            )
            return value
        ...
        def save_kv_projection(name):
            def hook(_module, _args, value):
                ...
                official_prefix_rows[name] = (
                    prefix[:, :local_kv_heads]
                    .reshape(len(prompt) - 64, -1)
                    .detach().float().cpu().clone()
                )
                return value
        ...
        def save_q_norm(_module, _args, value):
            ...
            official_prefix_rows["layer3_q_norm_local"] = (
                value[:, :-64, :local_query_heads]
                .squeeze(0)
                .detach().float().cpu().clone()
            )
        ...
        def save_k_norm(_module, _args, value):
            ...
            official_prefix_rows["layer3_k_norm_local"] = (
                value[:, :-64, :local_kv_heads]
                .squeeze(0)
                .detach().float().cpu().clone()
            )
        ...
        def save_output_input(_module, args):
            official_native["layer3_output_input_full"] = (
                tensor(args).detach().cpu().clone()
            )
            ...
            prefix_value = tensor(args)[:, :-64].reshape(
                len(prompt) - 64,
                attention.config.num_attention_heads,
                attention.head_dim,
            )
            prefix_gated = (
                prefix_value[:, :local_query_heads]
                .reshape(len(prompt) - 64, -1)
                .detach().float().cpu().clone()
            )
            official_prefix_rows["layer3_output_input_local"] = prefix_gated
            prefix_gate = official_prefix_rows["layer3_query_gate_local"]
            official_prefix_rows[
                "layer3_attention_raw_inferred_local"
            ] = prefix_gated / torch.sigmoid(prefix_gate)
        ...
        def wrapped_official_eager_attention(
            ...
            **kwargs,
            ...
            if module is attention:
                ...
                official_prefix_rows["layer3_attention_raw_local"] = (
                    output[:, :-64, :local_query_heads]
                    .reshape(len(prompt) - 64, -1)
                    .detach().float().cpu().clone()
                )
        ...
        def wrapped_apply_rotary(query, key, *args, **kwargs):
            ...
            if rotary_capture_pending[0]:
                ...
                official_prefix_rows["layer3_rotary_query_local"] = (
                    rotated_query[:, :local_query_heads, :-64, :]
                    .transpose(1, 2)
                    .squeeze(0)
                    .detach().float().cpu().clone()
                )
                official_prefix_rows["layer3_rotary_key_local"] = (
                    rotated_key[:, :local_kv_heads, :-64, :]
                    .transpose(1, 2)
                    .squeeze(0)
                    .detach().float().cpu().clone()
        ...
        def save_official_attention_output(_module, _args, value):
            official_rows["layer3_attention_output"] = tail(value)
            official_prefix_rows["layer3_attention_output"] = (
                tensor(value)[:, :-64].squeeze(0)
                .detach().float().cpu().clone()
            )
            official_native["layer3_attention_output_full"] = (
        ...
        hooks.append(
            attention.o_proj.register_forward_hook(
                save_official_attention_output
            )
        )
        hooks.append(
            ...
            *,
            ...
            if query.shape[1] == len(prompt):
                ...
                official_delta.update({
                    "full_query": query.detach().clone(),
                    "full_key": key.detach().clone(),
                    "full_value": value.detach().clone(),
                    "full_g": g.detach().clone(),
                    "full_beta": beta.detach().clone(),
    ...
    cross_gpu_model = None
    cross_gpu_rows = {}
    cross_gpu_phase = {"name": None}
    cross_gpu_hooks = []
    cross_gpu_attention_globals = None
    cross_gpu_original_eager_attention = None
    try:
        with official.torch_custom_op_annotation_compatibility():
            cross_gpu_model = AutoModelForCausalLM.from_pretrained(
        ...
        cross_gpu_model = cross_gpu_model.to(
            device=torch.device("cuda:1")
        )
        cross_gpu_model.eval()
        cross_gpu_attention = (
            cross_gpu_model.model.layers[3].self_attn
        )
        cross_gpu_attention_globals = (
            cross_gpu_attention.forward.__func__.__globals__
        )
        cross_gpu_original_eager_attention = (
            cross_gpu_attention_globals["eager_attention_forward"]
        )

        def zero_padded_eager_attention(
            module,
            query,
            key,
            value,
            ...
            **kwargs,
            ...
            target_count = len(prompt)
            if token_count != target_count - 64:
                return cross_gpu_original_eager_attention(
                    module,
                    query,
                    key,
                    value,
            ...
            padding = target_count - token_count
            ...
            padded_mask = torch.zeros(
                ...
                causal.view(1, 1, target_count, target_count),
                torch.finfo(query.dtype).min,
            )
            output, weights = cross_gpu_original_eager_attention(
                module,
                padded_query,
                padded_key,
                padded_value,
            ...
            return output[:, :token_count], (
                None
                if weights is None
                else weights[..., :token_count, :token_count]
            )

        def capture_cross_gpu_projection_input(_module, args):
            name = cross_gpu_phase["name"]
            if name is not None:
                cross_gpu_rows[
                    f"{name}_layer3_output_input_full"
                ] = (
                    tensor(args).squeeze(0)
                    .detach().float().cpu().clone()
                )

        def capture_cross_gpu_attention_output(_module, _args, value):
            name = cross_gpu_phase["name"]
            if name is not None:
                cross_gpu_rows[
                    f"{name}_layer3_attention_output"
                ] = (
        ...
        cross_gpu_hooks.append(
            cross_gpu_attention.o_proj.register_forward_pre_hook(
                capture_cross_gpu_projection_input
            )
        )
        cross_gpu_hooks.append(
            cross_gpu_attention.o_proj.register_forward_hook(
                capture_cross_gpu_attention_output
            )
        )
        with torch.inference_mode():
            for name, token_ids, eager_attention in (
                (
                    "full",
                    prompt,
                    cross_gpu_original_eager_attention,
                ...
                (
                    "zero_padded_prefix",
                    prompt[:-64],
                    zero_padded_eager_attention,
                ),
            ):
                cross_gpu_phase["name"] = name
                cross_gpu_attention_globals[
                    "eager_attention_forward"
                ] = eager_attention
                cross_gpu_input_ids = torch.tensor(
                    [token_ids],
                    dtype=torch.int64,
                    device=torch.device("cuda:1"),
                )
                cross_gpu_model(
                    input_ids=cross_gpu_input_ids,
                    use_cache=False,
                    return_dict=True,
                )
            cross_gpu_phase["name"] = None
        ...
        if (
            cross_gpu_attention_globals is not None
            and cross_gpu_original_eager_attention is not None
        ):
            cross_gpu_attention_globals[
                "eager_attention_forward"
            ] = cross_gpu_original_eager_attention
        for hook in cross_gpu_hooks:
            hook.remove()
        cross_gpu_hooks.clear()
        if "cross_gpu_attention" in locals():
            cross_gpu_attention = None
        cross_gpu_model = None
        gc.collect()
    ...
    engine_prefix_rows = {}
    engine_projection_gather = {}
    engine_delta = {"recompute": {}, "restore": {}}
    engine_delta_calls = []
    engine_layer4_prefix_input = {}
    ...
    try:
        ...
        def capture_engine_layer_output(layer, value):
            ...
            if name_phase in engine_layer_outputs:
                ...
                value_tensor = tensor(value)
                if (
                    name_phase == "recompute"
                    and value_tensor.shape[0] == len(prompt) - 64
                ):
                    engine_prefix_layer_outputs[str(layer_index)] = (
                        value_tensor.detach().float().cpu().clone()
                    )
        ...
        projection_gather_active = [False]
        original_dist_all_gather = (
            engine_linear_primitives.dist.all_gather
        )

        def wrapped_dist_all_gather(output_tensors, input_tensor, *args, **kwargs):
            result = original_dist_all_gather(
                output_tensors,
                input_tensor,
                *args,
                **kwargs,
            ...
            if (
                projection_gather_active[0]
                and phase["name"] == "recompute"
                and input_tensor.shape[0] == len(prompt) - 64
            ):
                engine_projection_gather["local_input"] = (
                    input_tensor.detach().float().cpu().clone()
                )
                engine_projection_gather["rank_inputs"] = [
                    value.detach().float().cpu().clone()
                    for value in output_tensors
                ]
                engine_projection_gather["gathered_input"] = (
                    torch.cat(output_tensors, dim=-1)
                    .detach().float().cpu().clone()
                )
            return result

        engine_linear_primitives.dist.all_gather = (
            wrapped_dist_all_gather
        )

        def projection_pre(_module, _args):
            projection_gather_active[0] = True

        def projection_post(_module, _args, value):
            projection_gather_active[0] = False
            return value
        ...
        full_projection_input = official_native[
            "layer3_output_input_full"
        ].to(
            device=attention.output_projection.weight.device,
            dtype=attention.output_projection.weight.dtype,
        )
        projection_weight = attention.output_projection.weight
        projection_bias = attention.output_projection.bias
        prefix_count = len(prompt) - 64
        rank0_projection_replays = {
            "full_1088": F.linear(
                full_projection_input,
                projection_weight,
                projection_bias,
            ).squeeze(0).detach().float().cpu().clone(),
            "prefix_1024": F.linear(
                full_projection_input[:, :prefix_count],
                projection_weight,
                projection_bias,
            ).squeeze(0).detach().float().cpu().clone(),
            "zero_padded_1088": F.linear(
                torch.cat(
                    (
                        full_projection_input[:, :prefix_count],
                        full_projection_input.new_zeros(
                            1,
                            64,
                            full_projection_input.shape[-1],
                        ),
                    ),
                    dim=1,
                ),
                projection_weight,
                projection_bias,
            )[:, :prefix_count].squeeze(0).detach().float().cpu().clone(),
        }
        ...
        def capture_layer4_prefix_input(_module, args):
            hidden_states = args[0]
            if (
                phase["name"] == "recompute"
                and hidden_states.shape[0] == len(prompt) - 64
            ):
                engine_layer4_prefix_input.update({
                    "hidden_states": hidden_states.detach().clone(),
                    "convolution_state": args[1].detach().clone(),
                    "recurrent_state": args[2].detach().clone(),
                })

        hooks.append(
            linear4.register_forward_pre_hook(
                capture_layer4_prefix_input
            )
        )
        ...
        def engine_prefix(value):
            value = tensor(value)
            if (
                phase["name"] == "recompute"
                and value.ndim >= 2
                and value.shape[0] == len(prompt) - 64
            ):
                return value.detach().float().cpu().clone()
            return None
        ...
        def save(name):
            def hook(_module, _args, value):
                ...
                if name_phase in engine_rows:
                    engine_rows[name_phase][name] = tail(value)
                    prefix_value = engine_prefix(value)
                    if prefix_value is not None:
                        engine_prefix_rows[name] = prefix_value
                return value
            return hook
        ...
        def save_input(name):
            def hook(_module, args):
                ...
                if name_phase in engine_rows:
                    engine_rows[name_phase][name] = tail(args)
                    prefix_value = engine_prefix(args)
                    if prefix_value is not None:
                        engine_prefix_rows[name] = prefix_value
            return hook
        ...
        def save_engine_query_projection(_module, _args, value):
            ...
            if name_phase in engine_rows:
                ...
                if value.shape[0] == len(prompt) - 64:
                    prefix = value.reshape(
                        len(prompt) - 64,
                        attention.local_query_heads,
                        2 * attention.head_dim,
                    )
                    engine_prefix_rows["layer3_q_projection_local"] = (
                        prefix.reshape(len(prompt) - 64, -1)
                        .detach().float().cpu().clone()
                    )
                    engine_prefix_rows["layer3_query_gate_local"] = (
                        prefix[..., attention.head_dim:]
                        .reshape(len(prompt) - 64, -1)
                        .detach().float().cpu().clone()
                    )
        ...
        def save_engine_rotary(_module, args, value):
            ...
            if name_phase in engine_rows:
                ...
                if query.shape[0] == len(prompt) - 64:
                    engine_prefix_rows["layer3_rotary_query_local"] = (
                        query.reshape(
                            len(prompt) - 64,
                            attention.local_query_heads,
                            attention.head_dim,
                    ...
                    engine_prefix_rows["layer3_rotary_key_local"] = (
                        key.reshape(
                            len(prompt) - 64,
                            attention.local_kv_heads,
                            attention.head_dim,
        ...
        def save_gate_up(_module, _args, value):
            ...
            if name_phase in engine_rows:
                ...
                if value.shape[0] == len(prompt) - 64:
                    engine_prefix_rows["layer3_gate_proj"] = (
                        gate.detach().float().cpu().clone()
                    )
                    engine_prefix_rows["layer3_up_proj"] = (
                        up.detach().float().cpu().clone()
                    )
            return value
        ...
        def save_engine_output_input(_module, args):
            save_input("layer3_output_input_local")(_module, args)
            value = tensor(args)
            if (
                phase["name"] == "recompute"
                and value.shape[0] == len(prompt) - 64
            ):
                gate = engine_prefix_rows["layer3_query_gate_local"]
                gated = value.detach().float().cpu().clone()
                engine_prefix_rows[
                    "layer3_attention_raw_inferred_local"
                ] = gated / torch.sigmoid(gate)
    ...
    official_vs_engine_prefix = []
    for name in prefix_names:
        if (
            name not in official_prefix_rows
            or name not in engine_prefix_rows
        ):
            raise RuntimeError(f"missing prefix boundary capture: {name}")
        official_vs_engine_prefix.append({
            "name": name,
            **compare(
                official_prefix_rows[name],
                engine_prefix_rows[name],
            ),
        })
    ...
    prefix_layer_comparisons = []
    for layer_index in range(24):
        key = str(layer_index)
        if (
            key not in official_prefix_layer_outputs
            or key not in engine_prefix_layer_outputs
        ):
            raise RuntimeError(
                f"missing prefix layer output capture: {layer_index}"
            )
        prefix_layer_comparisons.append({
            "layer_index": layer_index,
            **compare(
                official_prefix_layer_outputs[key],
                engine_prefix_layer_outputs[key],
            ),
        })

    engine_recompute = engine_delta["recompute"]
    prefix_token_count = len(prompt) - 64
    engine_prefix_candidates = [
        call for call in engine_delta_calls
        if (
            call["phase"] == "recompute"
            and call["query"].shape[0] == prefix_token_count
        )
    ]
    if len(engine_prefix_candidates) != 1:
        raise RuntimeError(
            "expected one Engine prefix delta call; got "
            f"{[(call['phase'], call['query'].shape[0]) for call in engine_delta_calls]}"
        )
    engine_prefix = engine_prefix_candidates[0]
    engine_full_candidates = [
        call for call in engine_delta_calls
        if (
            call["phase"] == "recompute"
            and call["query"].shape[0] == len(prompt)
        )
    ]
    if len(engine_full_candidates) != 1:
        raise RuntimeError(
            "expected one Engine full recompute delta call; got "
            f"{[(call['phase'], call['query'].shape[0]) for call in engine_delta_calls]}"
        )
    engine_full = engine_full_candidates[0]
    official_prefix_query = official_delta["full_query"][
        :, :-64, :linear4.local_value_heads
    ]
    official_prefix_key = official_delta["full_key"][
        :, :-64, :linear4.local_value_heads
    ]
    official_prefix_value = official_delta["full_value"][
        :, :-64, :linear4.local_value_heads
    ]
    official_prefix_g = official_delta["full_g"][
        :, :-64, :linear4.local_value_heads
    ]
    official_prefix_beta = official_delta["full_beta"][
        :, :-64, :linear4.local_value_heads
    ]
    _, official_four_head_prefix_state = original_official_chunk(
        official_prefix_query,
        official_prefix_key,
        official_prefix_value,
        g=official_prefix_g,
        beta=official_prefix_beta,
        initial_state=None,
        output_final_state=True,
        use_qk_l2norm_in_kernel=True,
    )
    engine_prefix_g = (
        -torch.exp(engine_prefix["A_log"].float())[None, None, :]
        * torch.nn.functional.softplus(
            engine_prefix["a"].float()
            + engine_prefix["dt_bias"].float()[None, :]
        )[None, :, :]
    )
    engine_prefix_beta = torch.sigmoid(
        engine_prefix["b"]
    ).unsqueeze(0)
    _, official_on_engine_prefix_inputs_state = original_official_chunk(
        engine_prefix["query"].unsqueeze(0),
        engine_prefix["key"].unsqueeze(0),
        engine_prefix["value"].unsqueeze(0),
        g=engine_prefix_g,
        beta=engine_prefix_beta,
        initial_state=None,
        output_final_state=True,
        use_qk_l2norm_in_kernel=True,
    )
    engine_full_prefix_core, engine_full_prefix_state = (
        original_linear_chunk(
            engine_full["query"][:-64],
            engine_full["key"][:-64],
            engine_full["value"][:-64],
            engine_full["a"][:-64],
            engine_full["b"][:-64],
            engine_full["A_log"],
            engine_full["dt_bias"],
            torch.zeros_like(engine_full["initial_state"]),
        )
    )
    padded_prefix_hidden = torch.cat((
        engine_layer4_prefix_input["hidden_states"],
        engine_layer4_prefix_input["hidden_states"].new_zeros(
            64,
            engine_layer4_prefix_input["hidden_states"].shape[1],
        ),
    ))
    padded_projected_qkv = linear4.in_proj_qkv(
        padded_prefix_hidden
    )[:prefix_token_count]
    padded_projected_a = linear4.in_proj_a(
        padded_prefix_hidden
    )[:prefix_token_count]
    padded_projected_b = linear4.in_proj_b(
        padded_prefix_hidden
    )[:prefix_token_count]
    padded_convolved, _ = original_linear_conv(
        padded_projected_qkv,
        engine_layer4_prefix_input["convolution_state"],
        linear4.conv_weight,
    )
    padded_query, padded_key, padded_value = padded_convolved.split(
        (
            linear4.local_key_heads * linear4.key_head_dim,
            linear4.local_key_heads * linear4.key_head_dim,
            linear4.local_value_heads * linear4.value_head_dim,
        ),
        dim=-1,
    )
    padded_query = padded_query.reshape(
        prefix_token_count,
        linear4.local_key_heads,
        linear4.key_head_dim,
    )
    padded_key = padded_key.reshape(
        prefix_token_count,
        linear4.local_key_heads,
        linear4.key_head_dim,
    )
    padded_value = padded_value.reshape(
        prefix_token_count,
        linear4.local_value_heads,
        linear4.value_head_dim,
    )
    padded_repeat = (
        linear4.local_value_heads // linear4.local_key_heads
    )
    if padded_repeat > 1:
        padded_query = padded_query.repeat_interleave(
            padded_repeat, dim=1
        )
        padded_key = padded_key.repeat_interleave(
            padded_repeat, dim=1
        )
    with torch.inference_mode():
        _, padded_prefix_state = original_linear_chunk(
            padded_query,
            padded_key,
            padded_value,
            padded_projected_a,
            ...
            linear4.A_log,
            linear4.dt_bias,
            engine_layer4_prefix_input["recurrent_state"],
        )
    ...
    cross_replay_comparisons = {
        "official_16head_vs_official_4head_prefix_state": compare(
            official_delta["prefix_state"][
                0, :linear4.local_value_heads
            ].detach().float().cpu(),
            official_four_head_prefix_state[
                0
            ].detach().float().cpu(),
        ),
        "official_4head_vs_engine_prefix_final_state": compare(
            official_four_head_prefix_state[
                0
            ].transpose(-1, -2).detach().float().cpu(),
            engine_prefix["final_state"].detach().float().cpu(),
        ),
        "official_4head_vs_official_on_engine_prefix_inputs_state": compare(
            official_four_head_prefix_state[
                0
            ].detach().float().cpu(),
            official_on_engine_prefix_inputs_state[
                0
            ].detach().float().cpu(),
        ),
        "official_vs_engine_prefix_query": compare(
            official_prefix_query.squeeze(0).detach().float().cpu(),
            engine_prefix["query"].detach().float().cpu(),
        ),
        "official_vs_engine_prefix_key": compare(
            official_prefix_key.squeeze(0).detach().float().cpu(),
            engine_prefix["key"].detach().float().cpu(),
        ),
        "official_vs_engine_prefix_value": compare(
            official_prefix_value.squeeze(0).detach().float().cpu(),
            engine_prefix["value"].detach().float().cpu(),
        ),
        "official_vs_engine_prefix_g": compare(
            official_prefix_g.detach().float().cpu(),
            engine_prefix_g.detach().float().cpu(),
        ),
        "official_vs_engine_prefix_beta": compare(
            official_prefix_beta.detach().float().cpu(),
            engine_prefix_beta.detach().float().cpu(),
        ),
        "official_vs_engine_full_prefix_query": compare(
            official_prefix_query.squeeze(0).detach().float().cpu(),
            engine_full["query"][:-64].detach().float().cpu(),
        ),
        "official_vs_engine_full_prefix_key": compare(
            official_prefix_key.squeeze(0).detach().float().cpu(),
            engine_full["key"][:-64].detach().float().cpu(),
        ),
        "official_vs_engine_full_prefix_value": compare(
            official_prefix_value.squeeze(0).detach().float().cpu(),
            engine_full["value"][:-64].detach().float().cpu(),
        ),
        "engine_standalone_vs_full_prefix_query": compare(
            engine_prefix["query"].detach().float().cpu(),
            engine_full["query"][:-64].detach().float().cpu(),
        ),
        "engine_standalone_vs_full_prefix_key": compare(
            engine_prefix["key"].detach().float().cpu(),
            engine_full["key"][:-64].detach().float().cpu(),
        ),
        "engine_standalone_vs_full_prefix_value": compare(
            engine_prefix["value"].detach().float().cpu(),
            engine_full["value"][:-64].detach().float().cpu(),
        ),
        "official_prefix_state_vs_engine_full_prefix_state": compare(
            official_prefix_state_v_k.detach().float().cpu(),
            engine_full_prefix_state.detach().float().cpu(),
        ),
        "engine_standalone_vs_full_prefix_state": compare(
            engine_prefix["final_state"].detach().float().cpu(),
            engine_full_prefix_state.detach().float().cpu(),
        ),
        "official_vs_padded_engine_prefix_query": compare(
            official_prefix_query.squeeze(0).detach().float().cpu(),
            padded_query.detach().float().cpu(),
        ),
        "official_vs_padded_engine_prefix_key": compare(
            official_prefix_key.squeeze(0).detach().float().cpu(),
            padded_key.detach().float().cpu(),
        ),
        "official_vs_padded_engine_prefix_value": compare(
            official_prefix_value.squeeze(0).detach().float().cpu(),
            padded_value.detach().float().cpu(),
        ),
        "official_prefix_state_vs_padded_engine_prefix_state": compare(
            official_prefix_state_v_k.detach().float().cpu(),
            padded_prefix_state.detach().float().cpu(),
        ),
    ...
    if "gathered_input" not in engine_projection_gather:
        raise RuntimeError("missing output projection gathered input")
    for phase_name in (
        "full",
        "prefix",
        "zero_padded_prefix",
        ...
        for name in (
            ...
            key = f"{phase_name}_{name}"
            if key not in cross_gpu_rows:
                raise RuntimeError(
                    f"missing same-GPU shape capture: {key}"
                )
    official_projection_input = official_native[
        "layer3_output_input_full"
    ].squeeze(0).detach().float().cpu()
    prefix_count = len(prompt) - 64
    local_width = attention.local_query_heads * attention.head_dim
    cross_gpu_prefix_comparisons = {
        "official_gpu2_full_input_vs_gpu4_full_input": compare(
            official_projection_input,
            cross_gpu_rows["full_layer3_output_input_full"],
        ),
        "official_gpu2_prefix_vs_gpu4_full_prefix_input": compare(
            official_projection_input[:prefix_count],
            cross_gpu_rows["full_layer3_output_input_full"][
                :prefix_count
            ],
        ),
        "gpu4_full_prefix_vs_gpu4_standalone_prefix_input": compare(
            cross_gpu_rows["full_layer3_output_input_full"][
                :prefix_count
            ],
            cross_gpu_rows["prefix_layer3_output_input_full"],
        ),
        "gpu4_full_prefix_vs_zero_padded_prefix_input": compare(
            cross_gpu_rows["full_layer3_output_input_full"][
                :prefix_count
            ],
            cross_gpu_rows[
                "zero_padded_prefix_layer3_output_input_full"
            ],
        ),
        "gpu4_full_rank1_prefix_vs_engine_rank1": compare(
            cross_gpu_rows["full_layer3_output_input_full"][
                :prefix_count, local_width:2 * local_width
            ],
            engine_projection_gather["rank_inputs"][1],
        ),
        "gpu4_standalone_rank1_prefix_vs_engine_rank1": compare(
            cross_gpu_rows["prefix_layer3_output_input_full"][
                :, local_width:2 * local_width
            ],
            engine_projection_gather["rank_inputs"][1],
        ),
        "official_gpu2_attention_output_vs_gpu4_full": compare(
            official_native["layer3_attention_output_full"]
            .squeeze(0).detach().float().cpu(),
            cross_gpu_rows["full_layer3_attention_output"],
        ),
        "gpu4_full_prefix_vs_gpu4_standalone_attention_output": compare(
            cross_gpu_rows["full_layer3_attention_output"][
                :prefix_count
            ],
            cross_gpu_rows["prefix_layer3_attention_output"],
        ),
        "gpu4_full_prefix_vs_zero_padded_attention_output": compare(
            cross_gpu_rows["full_layer3_attention_output"][
                :prefix_count
            ],
            cross_gpu_rows[
                "zero_padded_prefix_layer3_attention_output"
            ],
        ),
    }

    projection_gather_comparisons = {
        "official_prefix_vs_engine_gathered": compare(
            official_projection_input[:prefix_count],
            engine_projection_gather["gathered_input"],
        ),
        "official_rank0_slice_vs_engine_local_input": compare(
            official_projection_input[
                :prefix_count, :local_width
            ],
            engine_projection_gather["local_input"],
        ),
    }
    for rank, rank_input in enumerate(
        engine_projection_gather["rank_inputs"]
    ):
        projection_gather_comparisons[
            f"official_rank{rank}_slice_vs_engine_rank{rank}"
        ] = compare(
            official_projection_input[
                :prefix_count,
    ...
    official_projection_full = official_native[
        "layer3_attention_output_full"
    ].squeeze(0).detach().float().cpu()
    output_projection_rank0_replay_comparisons = {
        "official_full_vs_rank0_full_1088": compare(
            official_projection_full,
            rank0_projection_replays["full_1088"],
        ),
        "official_prefix_vs_rank0_prefix_1024": compare(
            official_projection_full[:len(prompt) - 64],
            rank0_projection_replays["prefix_1024"],
        ),
        "official_prefix_vs_rank0_zero_padded_1088": compare(
            official_projection_full[:len(prompt) - 64],
            rank0_projection_replays["zero_padded_1088"],
        ),
        "rank0_prefix_1024_vs_zero_padded_1088": compare(
            rank0_projection_replays["prefix_1024"],
            rank0_projection_replays["zero_padded_1088"],
        ),
        "engine_normal_1024_vs_rank0_prefix_1024": compare(
            engine_prefix_rows["layer3_attention_output"],
            rank0_projection_replays["prefix_1024"],
        ),
    }
    ...
    result = {
        ...
        "prefix_component_comparisons": official_vs_engine_prefix,
        "prefix_first_component_mismatch": first_mismatch(
            official_vs_engine_prefix
        ),
        "prefix_layer_comparisons": prefix_layer_comparisons,
        "prefix_first_layer_mismatch": first_layer_mismatch(
            prefix_layer_comparisons
        ),
        ...
        "cross_replay_shapes": {
            "engine_delta_calls": [
                {
                    "phase": call["phase"],
                    "tokens": call["query"].shape[0],
                }
                for call in engine_delta_calls
            ],
        ...
        "schema_version": "qwen35.tp4-cached-layer3-zero-padded-attention.v1",
    ...
    print(json.dumps({
        "cross_gpu_prefix_comparisons": (
            result["cross_gpu_prefix_comparisons"]
        ),
        "projection_gather_comparisons": (
            result["projection_gather_comparisons"]
        ),
        "output_projection_rank0_replay_comparisons": (
            result["output_projection_rank0_replay_comparisons"]
        ),
        "prefix_first_component_mismatch": (
            result["prefix_first_component_mismatch"]
        ),
        "prefix_component_comparisons": (
            result["prefix_component_comparisons"]
        ),
        "prefix_first_layer_mismatch": (
            result["prefix_first_layer_mismatch"]
        ),
        "cross_replay_comparisons": (
