...
def verify_run(
    ...
    try:
        ...
        if manifest["schema_version"] != gate.SCHEMA_VERSION:
        ...
        for field in (
            ...
            if manifest[field] != result[field]:
                raise ValueError(
                    f"source manifest {field} mismatch"
                )
        artifacts = manifest["artifacts"]
        if (
            not isinstance(artifacts, dict)
            or set(artifacts) != {"result.json", "status.json"}
        ):
            raise ValueError(
                "source manifest artifact inventory mismatch"
            )
        for name in ("result.json", "status.json"):
            if artifacts[name] != gate.sha256_file(run_dir / name):
