...
def _validate_model_identity(value: object) -> dict:
    ...
    if value["architectures"] != ["Qwen3_5ForCausalLM"]:
...
def _validate_cache_snapshot(
    ...
    *,
    ...
    if (
        ...
        or len(cache_transactions) != len(transactions)
    ):
        raise ValueError("proposal KV transaction inventory mismatch")
    normalized_transactions = []
    for expected, row in zip(transactions, cache_transactions):
        _exact_keys(
            row,
            {
                "transaction_id",
        ...
        if (
            row["transaction_id"] != expected["transaction_id"]
            or row["sequence_id"] != expected["sequence_id"]
            or row["sequence_epoch"] != 0
    ...
    if not isinstance(tickets, list) or len(tickets) != len(transactions):
        raise ValueError("proposal KV ticket inventory mismatch")
    normalized_tickets = []
    for transaction, ticket in zip(transactions, tickets):
        _exact_keys(
            ticket,
            {
                "ticket_id",
        ...
        if (
            ...
            or ticket["transaction_id"]
            != transaction["transaction_id"]
            or ticket["commit_entry_count"] <= 0
            or ticket["release_entry_count"] <= 0
