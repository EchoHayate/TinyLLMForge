...
class Context:
    ...
    force_attention_backend: bool = False
...
@contextmanager
def temporary_context(**changes):
    global _CONTEXT
    previous = _CONTEXT
    _CONTEXT = replace(previous, **changes)
    try:
        yield _CONTEXT
    finally:
        _CONTEXT = previous
...
def set_context(is_prefill=None, cu_seqlens_q=None, cu_seqlens_k=None, max_seqlen_q=0, max_seqlen_k=0,
                ...
                kv_offload_prefill_chunk_ends: list[int] | None = None,
                prefill_attention_reference_lens: tuple[int, ...] | None = None,
                spec_verify_query_lens: tuple[int, ...] | list[int] = (),
    ...
    spec_verify_query_lens = tuple(
        int(length)
        for length in spec_verify_query_lens
    )
    _CONTEXT = Context(
        mode=resolved_mode,
        ...
        spec_verify_query_lens=spec_verify_query_lens,
        force_attention_backend=bool(force_attention_backend),
        ...
        prefill_attention_reference_lens=prefill_attention_reference_lens,
    )
