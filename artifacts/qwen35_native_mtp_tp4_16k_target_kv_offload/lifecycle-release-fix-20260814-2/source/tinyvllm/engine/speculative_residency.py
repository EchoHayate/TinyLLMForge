...
class SpeculativeResidencyParticipant:
    ...
    def prepare_batch(
        ...
        *,
        ...
        reserved_materialized_blocks = []
        seen_original_read_blocks = set()
        seen_reserved_materialized_blocks = set()
        try:
            for row in rows:
                ...
                row_original_read_blocks = (
                    original_blocks
                    if stage_all_original_blocks
                    else materialized_original
                )
                for block_id in row_original_read_blocks:
                    if block_id in seen_original_read_blocks:
                        continue
                    original_read_blocks.append(block_id)
                    seen_original_read_blocks.add(block_id)
                for block_id in reserved_materialized:
                    if block_id in seen_reserved_materialized_blocks:
                        continue
                    reserved_materialized_blocks.append(block_id)
                    seen_reserved_materialized_blocks.add(block_id)
            ...
            if reserved_materialized_blocks:
                self.manager.ensure_resident(
                    reserved_materialized_blocks,
                    require_valid=False,
                    protected_logical_blocks=protected,
    ...
    def ensure_materialized_for(
        ...
        for row in ticket.rows:
            if row.payload.sequence_id not in requested:
                continue
            original = {
                block_id
                for block_id, _ in (
                    row.payload.original_block_identities
                )
            }
            for block_id, _ in row.materialized_block_identities:
                if block_id in original:
                    if block_id not in seen_original:
                        original_blocks.append(block_id)
                        seen_original.add(block_id)
                elif block_id not in seen_reserved:
                    reserved_blocks.append(block_id)
                    seen_reserved.add(block_id)
        protected = set(original_blocks + reserved_blocks)
        if original_blocks:
            self.manager.ensure_resident(
                original_blocks,
                require_valid=True,
