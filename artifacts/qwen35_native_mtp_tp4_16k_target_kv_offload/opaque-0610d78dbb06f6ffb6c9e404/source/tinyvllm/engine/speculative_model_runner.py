...
def prepare_model_runner_proposal_finalize_batch(
    ...
    *,
    ...
    dispatch = model_runner.call if dispatch is None else dispatch
    if not callable(dispatch):
        raise ValueError("proposal lifecycle dispatch must be callable")
    ticket_id = dispatch(
...
def release_model_runner_proposal_sequence(
    model_runner,
    descriptor,
    sequence_id: int,
    sequence_epoch: int,
    *,
    ...
    _validate_model_runner(model_runner)
    executor_id = _validate_lifecycle_descriptor(descriptor)
    normalized_sequence_id = _validate_integer(
        sequence_id,
        "proposal release sequence ID",
    )
    if normalized_sequence_id < 0:
        raise ValueError(
            "proposal release sequence ID must be non-negative"
        )
    normalized_sequence_epoch = _validate_integer(
        sequence_epoch,
        "proposal release sequence epoch",
    )
    if normalized_sequence_epoch < 0:
        raise ValueError(
    ...
    acknowledgement = dispatch(
        ...
        normalized_sequence_epoch,
    )
    assert_tensor_free(
        acknowledgement,
        name="proposal release acknowledgement",
    )
    if acknowledgement is not None:
        raise ValueError(
