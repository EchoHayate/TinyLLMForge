...
def model_runner_proposal_token_context(
    sequence: object,
    capabilities: DraftCapabilities,
) -> tuple[tuple[int, ...], int]:
    token_count = _validate_integer(
        getattr(sequence, "num_tokens", None),
        "proposal sequence token count",
    )
    if token_count <= 0:
        raise ValueError(
            "proposal sequence token count must be positive"
        )
    raw_token_ids = getattr(sequence, "token_ids", None)
    if raw_token_ids is None:
        if capabilities.requires_full_token_history:
            raise RuntimeError(
                "proposal executor requires full token history"
            )
        return (), token_count
    token_ids = tuple(
        _validate_integer(token_id, "proposal sequence token")
        for token_id in raw_token_ids
    )
    if len(token_ids) != token_count:
        raise ValueError(
            "proposal sequence token inventory mismatch"
        )
    return token_ids, token_count


def proposal_input_context_token_count(
    ...
    token_count = input_row.context_token_count
    if token_count is None:
        token_count = len(token_ids)
    token_count = _validate_integer(
        token_count,
        "proposal input context token count",
    )
    if token_count < len(token_ids):
        raise ValueError(
            "proposal input context token count is smaller "
            "than token inventory"
        )
    return token_count
...
def _validate_inputs(
    ...
    for input_row in inputs:
        ...
        if (
            ...
            and context_token_count != len(input_row.token_ids)
        ):
            raise ValueError(
                "proposal executor requires full token history"
            )
...
class ModelRunnerProposalExecutorRegistry:
    ...
    def release_sequence(
        ...
        sequence_id = _validate_integer(
            sequence_id,
            "proposal release sequence ID",
        )
        if sequence_id < 0:
            raise ValueError(
                "proposal release sequence ID must be non-negative"
            )
        sequence_epoch = _validate_integer(
            sequence_epoch,
            "proposal release sequence epoch",
        )
        if sequence_epoch < 0:
            raise ValueError(
                "proposal release sequence epoch must be non-negative"
            )
        acknowledgement = executor.release_sequence(
            sequence_id,
            sequence_epoch=sequence_epoch,
        )
        if acknowledgement is not None:
            raise ValueError(
                "proposal release acknowledgement must be None"
            )
