...
def _validate_rank_snapshots(
    ...
    *,
    ...
    all_accepted = (
        ...
        and all(
            isinstance(row, dict)
            and row.get("rejected_draft_tokens") == 0
            and row.get("proposed_tokens", 0) > 0
            and row.get("accepted_draft_tokens")
            == row.get("proposed_tokens")
            for row in value
        )
    )
    batch_native_splits = (
        ...
        and all(
            isinstance(row, dict)
            and isinstance(row.get("proposal_rows"), int)
            and not isinstance(row.get("proposal_rows"), bool)
            and isinstance(row.get("verify_callbacks"), int)
            and not isinstance(row.get("verify_callbacks"), bool)
            and isinstance(
                row.get("verify_target_forwards"),
                int,
            )
            and not isinstance(
                row.get("verify_target_forwards"),
            ...
            and 0 < row["verify_callbacks"]
            <= row["verify_target_forwards"]
            <= row["proposal_rows"]
            for row in value
        )
    )
    if not all_accepted and not batch_native_splits:
        return _rank_gate._validate_rank_snapshots(
            value,
            policy=policy,
        ...
        if (
            ...
            executor = {
                **executor,
                "release_rows": sorted(
                    release_rows,
                    key=lambda release_row: (
                        release_row["sequence_id"]
                    ),
                ),
            }
        compatibility_rows.append({
            **row,
            ...
            "rejected_draft_tokens": (
                1
                if all_accepted
                else row["rejected_draft_tokens"]
            ),
            "verify_target_forwards": (
                row["verify_callbacks"]
                if batch_native_splits
                else row["verify_target_forwards"]
            ),
        })
    normalized = _rank_gate._validate_rank_snapshots(
        compatibility_rows,
        policy=policy,
        batch_size=batch_size,
    )
    for index, row in enumerate(normalized):
        if all_accepted:
            row["rejected_draft_tokens"] = 0
        if batch_native_splits:
            row["verify_target_forwards"] = value[index][
                "verify_target_forwards"
            ]
    return normalized
...
def _require_native_campaign_rejection(
    cells: dict[str, dict],
) -> None:
    rejected = sum(
        cells[cell_key("native_mtp", batch_size)][
            "rank_snapshots"
        ][0]["rejected_draft_tokens"]
        for batch_size in BATCH_SIZES
    )
    if rejected <= 0:
        raise ValueError(
            "native campaign requires rejected draft tokens"
        )
...
def validate_result(value: object) -> dict:
    ...
    _require_native_campaign_rejection(normalized_cells)
