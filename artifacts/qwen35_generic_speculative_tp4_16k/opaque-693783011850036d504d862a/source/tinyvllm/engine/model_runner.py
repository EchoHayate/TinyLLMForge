...
def _qwen35_mtp_registration_dependencies():
    ...
    def build_physical_store(config, module):
        backend = (
        ...
        return Qwen35MTPPhysicalSlotStore(
            capacity,
    ...
    return SimpleNamespace(
        ...
        build_physical_store=build_physical_store,
...
class ModelRunner:
    ...
    def _maybe_register_qwen35_mtp_executor(
        ...
        *,
        ...
        if (
            self.world_size != 1
            or getattr(config, "tensor_parallel_size", None) != 1
        ...
        try:
            ...
            physical_store = dependencies.build_physical_store(
                config,
                module,
            )
            attention_backend = (
    ...
    def prepare_spec_verify_batch(
        ...
        if self.config.kv_offload_mvp0:
            manager = self.speculative_residency.manager
            mapped_rows = []
            for (
                sequence_id,
                batch_index,
    ...
    def run_spec_first_target_and_proposal_batch(
        ...
        try:
            if self.world_size != 1:
                raise RuntimeError(
                    "ModelRunner proposal execution supports TP1 only"
                )
            if getattr(
                self.config,
                "kv_offload_mvp0",
                ...
                raise RuntimeError(
                    "ModelRunner proposal execution requires "
                    "KV offload disabled"
                )
            executor_id = getattr(
                descriptor,
