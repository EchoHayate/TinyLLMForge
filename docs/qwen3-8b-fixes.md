...
7. 已落地：DFlash feasibility spike 已写入 `docs/dflash-feasibility.md`，只做接口/接入点预研，不直接实现完整 DFlash；结论是先抽象通用 `verify_and_commit_block()`，保持 n-gram 行为不变，再考虑 toy block draft model。
