...
33. 部分落地：next-window hint 已扩展为 capacity-bounded multi-window future hint。新增 `_blockwise_prefill_future_hint_blocks()`，按 `gpu_blocks - 当前窗口 unique blocks - write_blocks` 计算 spare future budget，把可容纳的后续 prefix blocks 加入 `future_logical_blocks`；这些 blocks 不进入 protected/capacity 集合，因此仍只影响 `lru_cost` eviction scoring，不改变 correctness 边界。
   - TDD RED：新增 `test_blockwise_prefill_read_windows_hint_capacity_bounded_future_prefix_blocks`，远程旧实现按预期 `AssertionError`，因为只 hint 下一个 prefix window。
   - 远程目标测试通过：`tools/test_blockwise_attention_planning.py`、`tools/test_chunked_prefill.py`、`tools/test_ngram_speculative.py`。
   - 集成 smoke：`SMOKE_TAG=20260709_prefill_multi_window_hint RUN_PREFLIGHT=0 CUDA_VISIBLE_DEVICES=4 TINYVLLM_DIST_PORT=34771 MASTER_PORT=34771 tools/smoke_blockwise_prefill_remote.sh`。
   ...
   - GPU blocks matrix：`SMOKE_TAG=20260709_prefill_multi_window_hint_matrix RUN_PREFLIGHT=0 RUN_MATH_SMOKE=0 RUN_REAL_SMOKE=1 RUN_GPU_BLOCKS_MATRIX=1 MATRIX_REQUIRE_PASS=0 CUDA_VISIBLE_DEVICES=4 TINYVLLM_DIST_PORT=34772 MASTER_PORT=34772 tools/smoke_blockwise_prefill_remote.sh`。
     - `gpu_blocks=1`：预期容量失败，仍报 `required=2, gpu_blocks=1`。
     - `gpu_blocks=2`：`gate_pass=true`、`elapsed_s=29.661339823156595`、`h2d_copies=391`、`d2h_copies=6`、`evictions=395`、`resident_blocks=2`。
     - `gpu_blocks=4`：`gate_pass=true`、`elapsed_s=29.4657124876976`、`h2d_copies=166`、`d2h_copies=6`、`evictions=168`、`resident_blocks=4`。
   - 结论：`gpu_blocks=4` 相对 next-window hint 的 H2D 193→166、evict 195→168；相对 wait/enqueue fast-path matrix 的 H2D 249→166、evict 251→168。`gpu_blocks=2` 仍无改善，符合无 spare lookahead 容量的预期。
34. 部分落地：capacity-bounded future hint 已泛化到 decode read-window。新增 `_blockwise_read_window_future_hint_blocks()`，`_blockwise_online_decode_attention()` 每个 read window 也传入后续可容纳 logical blocks 作为 `future_logical_blocks`，但只等待当前窗口 blocks，且不扩大 protected/capacity 集合。
   - TDD RED：新增 `test_blockwise_decode_read_windows_hint_capacity_bounded_future_blocks`，远程旧实现按预期 `AssertionError`。
   - 远程目标测试通过：`tools/test_blockwise_attention_planning.py`、`tools/test_chunked_prefill.py`、`tools/test_ngram_speculative.py`。
   - Decode 集成 smoke：`SMOKE_TAG=20260709_decode_read_window_hint RUN_PREFLIGHT=0 RUN_MATH_SMOKE=0 MAX_OUTPUT_LEN=4 CUDA_VISIBLE_DEVICES=4 TINYVLLM_DIST_PORT=34781 MASTER_PORT=34781 tools/smoke_blockwise_prefill_remote.sh`。
   ...
   - 结论：decode path 已由真实模型多步 decode 覆盖且无 correctness 回归；低 staging 容量下 reload 仍很重，后续应把 read-window planner 显式化或补更宽 `gpu_blocks` 的 decode matrix。
35. 部分落地：blockwise decode/prefill 的 GQA window attention 不再 materialize repeated K/V heads。新增 `_gqa_scores_decode()`、`_gqa_weighted_values_decode()`、`_gqa_scores_prefill()`、`_gqa_weighted_values_prefill()`；`num_kv_heads < num_heads` 时通过 `(num_kv_heads, group_size)` grouped einsum 直接计算，避免每个 window 对 K/V 做 `repeat_interleave`。
   - TDD RED：新增 `test_blockwise_decode_gqa_does_not_materialize_repeated_kv_heads`、`test_blockwise_prefill_gqa_does_not_materialize_repeated_kv_heads`，远程旧实现按预期触发 `AssertionError("repeat_kv_for_gqa called")`。
   - 新增等价性测试：`test_gqa_grouped_helpers_match_repeated_kv_reference`，确认 grouped helpers 与旧 `repeat_interleave + einsum` 数值一致。
   - 远程目标测试通过：`tools/test_blockwise_attention_planning.py`、`tools/test_chunked_prefill.py`、`tools/test_ngram_speculative.py`。
   - 集成 smoke：`SMOKE_TAG=20260709_gqa_no_repeat_window RUN_PREFLIGHT=0 MAX_OUTPUT_LEN=4 CUDA_VISIBLE_DEVICES=4 TINYVLLM_DIST_PORT=34791 MASTER_PORT=34791 tools/smoke_blockwise_prefill_remote.sh`。
   ...
   - GPU blocks matrix：第一次 `SMOKE_TAG=20260709_gqa_no_repeat_matrix` 遇到端口 `EADDRINUSE`；换 `TINYVLLM_DIST_PORT=34801 MASTER_PORT=34801 SMOKE_TAG=20260709_gqa_no_repeat_matrix_r2` 后通过。
     - `gpu_blocks=1`：预期容量失败，仍报 `required=2, gpu_blocks=1`。
   ...
   - 结论：copy/evict counters 不变是预期；本轮收益点是减少每个 blockwise window 的 K/V head-expanded 临时张量和 `repeat_interleave`。后续更大收益仍在 read-window planner/减少 H2D reload，或把 grouped GQA helper 下沉成更高效 kernel。
36. 部分落地：blockwise prefill prefix dense K/V buffer 不再 zero-fill。`_blockwise_online_prefill_attention()` 的 prefix read window 会按 `window_len` 完整 copy staged KV，因此把 `q.new_zeros(...)` 改为 `q.new_empty(...)`；decode batch window 仍保留 `new_zeros`，因为存在 padding/不同 `window_lens`。
   - TDD RED：新增 `test_blockwise_prefill_prefix_windows_do_not_zero_fill_dense_buffers`，远程旧实现按预期触发 `AssertionError("new_zeros called for fully-copied prefix window")`。
   - 远程目标测试通过：`tools/test_blockwise_attention_planning.py`、`tools/test_chunked_prefill.py`、`tools/test_ngram_speculative.py`。
   - 集成 smoke：`SMOKE_TAG=20260709_prefill_empty_buffers RUN_PREFLIGHT=0 MAX_OUTPUT_LEN=4 CUDA_VISIBLE_DEVICES=4 TINYVLLM_DIST_PORT=34811 MASTER_PORT=34811 tools/smoke_blockwise_prefill_remote.sh`。
   ...
   - 结论：低风险内存带宽/allocator 微优化，不改变 KV offload staging counters；后续可继续找“完全覆盖写入”的临时张量，但不要动 decode padded dense buffer。
37. 部分落地：blockwise decode read-window plan 在同一次 forward 的多层之间复用。新增 `_build_blockwise_decode_window_plan()`，把 `window_rows/window_lens/needed_blocks/future_hint_blocks/max_window_tokens` 缓存在 `Context.kv_offload_decode_window_plan_cache`；同一个 `set_context()` 生命周期内后续 decoder layers 复用该 plan，`reset_context()` 自然清空。
   - TDD RED：新增 `test_blockwise_decode_reuses_cached_read_window_plan_across_layers`，第二次调用同一 context 时 monkeypatch `_blockwise_read_window_future_hint_blocks`，远程旧实现按预期触发 `AssertionError("decode read-window plan recomputed")`。
   - 远程目标测试通过：`tools/test_blockwise_attention_planning.py`、`tools/test_chunked_prefill.py`、`tools/test_ngram_speculative.py`。
   - Decode 集成 smoke：`SMOKE_TAG=20260709_decode_plan_cache RUN_PREFLIGHT=0 RUN_MATH_SMOKE=0 MAX_OUTPUT_LEN=4 CUDA_VISIBLE_DEVICES=4 TINYVLLM_DIST_PORT=34821 MASTER_PORT=34821 tools/smoke_blockwise_prefill_remote.sh`。
     ...
     - KV counters：`gpu_blocks=2`、`h2d_copies=811`、`d2h_copies=9`、`evictions=815`、`prefetch_plans=933`、`prefetch_read_blocks=924`、`prefetch_write_blocks=9`、`copy_waits=1626`。
   - 结论：该优化不改变 staging/copy 行为，主要减少每层重复 Python/list planning；后续继续抽象统一 read-window planner，并把重点放回降低 H2D reload。
38. 部分落地：blockwise prefill prefix-window plan 在同一次 forward 的多层之间复用。新增 `_build_blockwise_prefill_window_plan()`，把每个 row 的 `q_start/q_end/q_len/chunk_start/chunk_end/windows` 以及各 prefix window 的 `window/window_len/future_hint_blocks` 缓存在 `Context.kv_offload_prefill_window_plan_cache`；同一个 `set_context()` 生命周期内后续 decoder layers 复用该 plan，`reset_context()` 自然清空。
   - TDD RED：新增 `test_blockwise_prefill_reuses_cached_prefix_window_plan_across_layers`，第二次调用同一 context 时 monkeypatch `_blockwise_prefill_future_hint_blocks`，远程旧实现按预期触发 `AssertionError("prefill prefix-window plan recomputed")`。
   - 远程目标测试通过：`tools/test_blockwise_attention_planning.py`、`tools/test_chunked_prefill.py`、`tools/test_ngram_speculative.py`。
   - 集成 smoke：`SMOKE_TAG=20260709_prefill_plan_cache RUN_PREFLIGHT=0 MAX_OUTPUT_LEN=4 CUDA_VISIBLE_DEVICES=4 TINYVLLM_DIST_PORT=34831 MASTER_PORT=34831 tools/smoke_blockwise_prefill_remote.sh`。
     ...
     - KV counters：`gpu_blocks=2`、`h2d_copies=811`、`d2h_copies=9`、`evictions=815`、`prefetch_plans=933`、`prefetch_read_blocks=924`、`prefetch_write_blocks=9`、`copy_waits=1626`。
   - 结论：该优化不改变 staging/copy 行为，主要减少每层重复 prefix-window Python/list planning；prefill/decode 现在都有 per-forward read-window plan cache，下一步继续抽象统一 planner，并把重点放回降低低 staging 容量下的 H2D reload/clean eviction。
39. 部分落地：blockwise prefill local causal mask 的 position templates 在同一次 forward 的多层之间复用。新增 `Context.kv_offload_prefill_position_template_cache`，缓存 `(max_chunk_tokens, device, q_pos_template, k_pos_template)`；同一个 `set_context()` 生命周期内后续 decoder layers 复用，shape/device 变化时自动重建。
   - TDD RED：新增 `test_blockwise_prefill_reuses_cached_local_position_templates_across_layers`，第二次调用同一 context 时 monkeypatch `torch.arange`，远程旧实现按预期触发 `AssertionError("prefill local position templates recomputed")`。
   - 远程目标测试通过：`tools/test_blockwise_attention_planning.py`、`tools/test_chunked_prefill.py`、`tools/test_ngram_speculative.py`。
   - 集成 smoke：`SMOKE_TAG=20260709_prefill_position_template_cache RUN_PREFLIGHT=0 MAX_OUTPUT_LEN=4 CUDA_VISIBLE_DEVICES=4 TINYVLLM_DIST_PORT=34841 MASTER_PORT=34841 tools/smoke_blockwise_prefill_remote.sh`。
     ...
     - KV counters：`gpu_blocks=2`、`h2d_copies=811`、`d2h_copies=9`、`evictions=815`、`prefetch_plans=933`、`prefetch_read_blocks=924`、`prefetch_write_blocks=9`、`copy_waits=1626`。
   - 结论：低风险 host/GPU 小张量创建优化，不改变 attention 数值或 KV staging/copy 计划；收益点是每层少重建 local causal mask 的 position templates。后续可以把 decode `position_template` 也纳入同类 per-forward cache，或继续推进统一 read-window planner。
40. 部分落地：blockwise decode window mask 的 position template 在同一次 forward 的多层之间复用。新增 `Context.kv_offload_decode_position_template_cache`，缓存 `(position_template_tokens, device, position_template)`；同一个 `set_context()` 生命周期内后续 decoder layers 复用，shape/device 变化时自动重建。
   - TDD RED：新增 `test_blockwise_decode_reuses_cached_position_template_across_layers`，第二次调用同一 context 时 monkeypatch `torch.arange`，远程旧实现按预期触发 `AssertionError("decode position template recomputed")`。
   - 远程目标测试通过：`tools/test_blockwise_attention_planning.py`、`tools/test_chunked_prefill.py`、`tools/test_ngram_speculative.py`。
   - Decode 集成 smoke：`SMOKE_TAG=20260710_decode_position_template_cache RUN_PREFLIGHT=0 RUN_MATH_SMOKE=0 MAX_OUTPUT_LEN=4 CUDA_VISIBLE_DEVICES=4 TINYVLLM_DIST_PORT=34851 MASTER_PORT=34851 tools/smoke_blockwise_prefill_remote.sh`。
...
41. 部分落地：blockwise decode full-window dense K/V buffer 跳过无效清零。旧路径始终用 `q.new_zeros(...)` 分配 decode dense K/V；现在当 `all(window_len == max_window_tokens)`、即当前 window 对所有 batch row 都会被 `k_cache/v_cache` copy 完整覆盖时，改用 `q.new_empty(...)`；partial/padded window 继续保守使用 `new_zeros(...)`。
   - TDD RED：新增 `test_blockwise_decode_full_windows_do_not_zero_fill_dense_buffers`，完整 decode window 下 monkeypatch `q.new_zeros`，远程旧实现按预期触发 `AssertionError("new_zeros called for fully-copied decode window")`。
   - 远程目标测试通过：`tools/test_blockwise_attention_planning.py`、`tools/test_chunked_prefill.py`、`tools/test_ngram_speculative.py`。
   - Decode 集成 smoke：`SMOKE_TAG=20260710_decode_empty_buffers RUN_PREFLIGHT=0 RUN_MATH_SMOKE=0 MAX_OUTPUT_LEN=4 CUDA_VISIBLE_DEVICES=4 TINYVLLM_DIST_PORT=34861 MASTER_PORT=34861 tools/smoke_blockwise_prefill_remote.sh`。
...
42. 部分落地：blockwise decode window mask/valid 在同一次 forward 的多层之间复用。旧路径每层每个 read window 都通过 `_decode_window_mask()` 重建 `torch.tensor(window_lens)`、position 比较和 `valid = mask.any(dim=-1)`；现在新增 `Context.kv_offload_decode_window_mask_cache`，按 `(tuple(window_lens), max_window_tokens, device)` 缓存 `(mask, valid)`。
   - TDD RED：新增 `test_blockwise_decode_reuses_cached_window_masks_across_layers`，第二次调用同一 context 时 monkeypatch `_decode_window_mask`，远程旧实现按预期触发 `AssertionError("decode window mask recomputed")`。
   - 远程目标测试通过：`tools/test_blockwise_attention_planning.py`、`tools/test_chunked_prefill.py`、`tools/test_ngram_speculative.py`。
   - Decode 集成 smoke：`SMOKE_TAG=20260710_decode_mask_cache RUN_PREFLIGHT=0 RUN_MATH_SMOKE=0 MAX_OUTPUT_LEN=4 CUDA_VISIBLE_DEVICES=4 TINYVLLM_DIST_PORT=34871 MASTER_PORT=34871 tools/smoke_blockwise_prefill_remote.sh`。
...
43. 部分落地：blockwise decode full-window 直接跳过 mask 路径。当 `all(window_len == max_window_tokens)` 时，当前 window 对所有 batch row 都全有效，不需要 `_decode_window_mask()`、`masked_fill(~mask)`、`valid = mask.any(...)` 和对应 `torch.where`；partial/padded window 仍走原 mask/cache 路径。
   - TDD RED：新增 `test_blockwise_decode_full_windows_skip_mask_construction`，full decode window 下 monkeypatch `_decode_window_mask`，远程旧实现按预期触发 `AssertionError("full decode window should not build mask")`。
   - 远程目标测试通过：`tools/test_blockwise_attention_planning.py`、`tools/test_chunked_prefill.py`、`tools/test_ngram_speculative.py`。
   - Decode 集成 smoke：`SMOKE_TAG=20260710_decode_full_window_mask_skip RUN_PREFLIGHT=0 RUN_MATH_SMOKE=0 MAX_OUTPUT_LEN=4 CUDA_VISIBLE_DEVICES=4 TINYVLLM_DIST_PORT=34881 MASTER_PORT=34881 tools/smoke_blockwise_prefill_remote.sh`。
...
44. 部分落地：blockwise decode 奇偶层交替 read-window 顺序。针对 `gpu_blocks=2` 低 staging 容量下 decode 层间反复从同一个方向扫描 read windows、导致下一层无法复用上一层结束时仍 resident 的 tail window，现在奇数层反向遍历已缓存的 decode read-window plan，并为反向顺序单独计算 capacity-bounded reverse future hint；偶数层保持正向，`layer_idx < 0` 兼容路径保持正向。
   - TDD RED：新增 `test_blockwise_decode_odd_layers_stage_read_windows_from_tail`，旧实现按预期报 `_blockwise_online_decode_attention() got an unexpected keyword argument 'layer_idx'`；新增 `test_blockwise_decode_odd_layers_hint_reverse_future_blocks`，中间实现只反向遍历、不反向 hint 时能抓到 future hint 方向错误，并顺手修正 `layer_idx=-1` 被 Python `%` 误判为奇数的问题。
   ...
   - 远程目标测试通过：`tools/test_blockwise_attention_planning.py`、`tools/test_chunked_prefill.py`、`tools/test_ngram_speculative.py`、`tools/test_kv_offload.py`。
   - Decode 集成 smoke：`SMOKE_TAG=20260710_decode_alternating_window_order RUN_PREFLIGHT=0 RUN_MATH_SMOKE=0 MAX_OUTPUT_LEN=4 CUDA_VISIBLE_DEVICES=4 TINYVLLM_DIST_PORT=34731 MASTER_PORT=34731 tools/smoke_blockwise_prefill_remote.sh`。
   ...
   - 结论：这次改变了 decode staging 行为，不只是 mask/小张量微优化。相对上一轮 `20260710_decode_full_window_mask_skip` 的 `gpu_blocks=2` counters，H2D `811 -> 728`、evictions `815 -> 732`、copy waits `1626 -> 1543`，约减少 10% 重复 reload/eviction；wall-clock 仍按远程单次 smoke 噪声处理，不单独宣称稳定 tok/s 提升。下一步更值得继续做的是合并相邻 window 的 H2D/D2H batch，或让 planner 直接输出跨层 alternating order + future hints 的统一结构，进一步减少 `prefetch_plans=933` 和 copy waits。
45. 部分落地：KV offload `wait_for_blocks(..., clear_pending=True)` 跳过 stale H2D event。旧逻辑会对传入 block 里任何有 `h2d_done` event 的 block 调 `current_stream.wait_event()`，即使该 block 已不在 `pending_wait_blocks`；现在 `clear_pending=True` 时先做 `blocks &= pending_wait_blocks`，避免 blockwise decode 反复 staging/reuse 时重复等待旧 event。
   - TDD RED：新增 `test_wait_for_blocks_clear_pending_skips_non_pending_stale_events_without_cuda_stream`，构造 `h2d_done={0,1}` 但 `pending_wait_blocks={1}`，调用 `wait_for_blocks([0], clear_pending=True)`；旧实现会触发 `torch.cuda.current_stream()`，按预期失败。
   ...
   - Decode 集成 smoke：`SMOKE_TAG=20260710_skip_stale_h2d_waits RUN_PREFLIGHT=0 RUN_MATH_SMOKE=0 MAX_OUTPUT_LEN=4 CUDA_VISIBLE_DEVICES=4 TINYVLLM_DIST_PORT=34901 MASTER_PORT=34901 tools/smoke_blockwise_prefill_remote.sh`。
...
46. 部分落地：blockwise read-window staging 跳过 resident 且非 pending 的窗口。`_stage_blockwise_read_window()` 现在遇到当前 read window 的所有 logical blocks 已 resident、且不在 `pending_wait_blocks` 时直接返回 unique blocks，不再增加 `prefetch_plans/prefetch_read_blocks`，也不再调用 `ensure_resident()` 和 `wait_for_blocks()`；为保持 eviction policy 正确，fast-path 仍会 `_touch(slot)` 刷新 LRU recency。
   - TDD RED：新增 `test_stage_blockwise_read_window_skips_resident_non_pending_window`，旧实现会把 already-resident window 仍计入 `prefetch_plans` 并调用 staging/wait hooks，按预期失败。
   - 中间负分支：第一次 fast-path 没有 `_touch()`，真实 smoke correctness 通过但 H2D `728 -> 729`、evictions `732 -> 733`，说明 resident blocks 没刷新 LRU 会轻微恶化 eviction；补 `_touch()` 后复测恢复。
   - 本地验证通过：`PYTHONPYCACHEPREFIX=/private/tmp/tinyllmforge_pycache python3 -m py_compile tinyvllm/layers/attention.py tools/test_blockwise_attention_planning.py`、`git diff --check`。
   ...
   - Decode 集成 smoke：`SMOKE_TAG=20260710_skip_resident_read_stage_touch RUN_PREFLIGHT=0 RUN_MATH_SMOKE=0 MAX_OUTPUT_LEN=4 CUDA_VISIBLE_DEVICES=4 TINYVLLM_DIST_PORT=34921 MASTER_PORT=34921 tools/smoke_blockwise_prefill_remote.sh`。
...
47. 部分落地：batched H2D event 的 pending 清理语义已修正。`wait_for_blocks(..., clear_pending=True)` 等待某个 H2D event 后，会清掉所有仍在 `pending_wait_blocks` 且共享同一个 `h2d_done` event 的 logical blocks；旧逻辑只清 requested blocks，在 `_enqueue_h2d_pairs()` 把多个 blocks 合并到同一个 CUDA event 时，会让 sibling blocks 继续误留 pending，阻止 resident fast-path。
   - TDD RED：新增 `test_wait_for_blocks_clear_pending_clears_all_blocks_sharing_waited_event_without_cuda`，构造 `h2d_done[0] is h2d_done[1]` 且 `pending_wait_blocks={0,1,2}`，调用 `wait_for_blocks([0], clear_pending=True)`；旧实现只清 `0`，按预期失败。
   ...
   - 远程目标测试通过：`tools/test_kv_offload.py`、`tools/test_blockwise_attention_planning.py`、`tools/test_chunked_prefill.py`、`tools/test_ngram_speculative.py`。
   - Decode 集成 smoke：`SMOKE_TAG=20260710_clear_shared_h2d_pending RUN_PREFLIGHT=0 RUN_MATH_SMOKE=0 MAX_OUTPUT_LEN=4 CUDA_VISIBLE_DEVICES=4 TINYVLLM_DIST_PORT=34931 MASTER_PORT=34931 tools/smoke_blockwise_prefill_remote.sh`。
...
48. 部分落地：批量 missing block reload 的 H2D slot 分配更利于 coalescing。`ensure_resident()` 现在先选出一组 victim slots，再把 missing logical blocks 与 selected slots 分别排序后配对，避免 LRU victim 顺序和物理 slot 顺序相反时把连续 logical blocks 映射成反向 slots，从而把本可连续的 H2D span 拆碎。
   - TDD RED：新增 `test_ensure_resident_assigns_contiguous_missing_blocks_to_contiguous_slots_for_coalesced_h2d`，构造 `slot_last_used=[3,2,1,0]`；旧实现会把 logical `4,5,6,7` 映射到 slots `3,2,1,0`，`_coalesce_copy_pairs()` 无法得到 `(4,0,4)`，按预期失败。
   - GREEN：同批 missing blocks sorted logical -> sorted slot 配对，测试中 H2D pairs 变为 `[(4,0),(5,1),(6,2),(7,3)]`，可 coalesce 成单 span；dirty D2H、clean eviction、LRU `_touch()` 语义保持不变。
   - 本地验证通过：`PYTHONPYCACHEPREFIX=/private/tmp/tinyllmforge_pycache python3 -m py_compile tinyvllm/engine/model_runner.py tools/test_kv_offload.py`、`git diff --check`。本机 `tools/test_kv_offload.py` 因缺 `torch` 未本地运行。
   - 远程目标测试通过：`tools/test_kv_offload.py`、`tools/test_blockwise_attention_planning.py`、`tools/test_chunked_prefill.py`、`tools/test_ngram_speculative.py`。
   - 默认 decode smoke：`SMOKE_TAG=20260710_contiguous_h2d_slots RUN_PREFLIGHT=0 RUN_MATH_SMOKE=0 MAX_OUTPUT_LEN=4 CUDA_VISIBLE_DEVICES=4 TINYVLLM_DIST_PORT=34841 MASTER_PORT=34841 tools/smoke_blockwise_prefill_remote.sh`。
     ...
     - KV counters 与上一轮一致：`gpu_blocks=2`、`kv_offload_blockwise_blocks=1`、`h2d_copies=728`、`d2h_copies=9`、`evictions=732`、`prefetch_plans=737`、`prefetch_read_blocks=728`、`copy_waits=1460`、`h2d_batches=728`、`h2d_batch_spans=728`；该配置每次 H2D 基本只有单 block，不能体现 slot coalescing 收益。
   - 多块窗口 decode smoke：`SMOKE_TAG=20260710_contiguous_h2d_slots_w2g4 RUN_PREFLIGHT=0 RUN_MATH_SMOKE=0 MAX_OUTPUT_LEN=4 CUDA_VISIBLE_DEVICES=4 TINYVLLM_DIST_PORT=34843 MASTER_PORT=34843 KV_OFFLOAD_GPU_BLOCKS=4 KV_OFFLOAD_BLOCKWISE_BLOCKS=2 tools/smoke_blockwise_prefill_remote.sh`。
   ...
   - 结论：这轮不是默认 `blockwise_blocks=1/gpu_blocks=2` 的直接收益，而是为更大 staging / 多块窗口 / batch-copy 场景减少 H2D span fragmentation。后续更值得继续做的是让 read-window planner 主动输出可合并的连续 logical windows，或把 `_enqueue_h2d_pairs()` 从“按调用批次合并”提升到跨相邻窗口的显式 batch staging。
49. 部分落地：write-block staging 已抽成 `_stage_kv_offload_write_blocks()` 并同时复用于 blockwise prefill/decode；下一步继续抽象统一的 KV block access planner，让 prefill/decode 共享 `plan_read_blocks()`、`stage_blocks()`、`evict_blocks()`、`commit_write_blocks()` 语义。
50. 已落地：DFlash feasibility spike 已写入 `docs/dflash-feasibility.md`，只做接口/接入点预研，不直接实现完整 DFlash；已完成 Phase 1，把 n-gram target verify/commit 包装为通用 `verify_and_commit_block()` 并保留 n-gram 行为不变。远程 Qwen3-0.6B candidate-only smoke 已验证 `commit_event.draft_source="ngram"`。Phase 2 plumbing 已验证：新增 `--draft-source {ngram,dflash-toy,dflash-toy-ngram-or-repeat}`、`--allow-zero-accept`、deterministic `repeat_recent_tokens` toy block draft model，以及记录 zero-accept attempts 的 `verify_events`；远程 `dflash-toy` smoke `gate_pass=true`、`zero_accept_events=3`、`verify_events[].draft_source="dflash-toy"`。
