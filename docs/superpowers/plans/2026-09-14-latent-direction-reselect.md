...
## 附录二（2026-09-15）：graph 路径 wall sweep 已跑完，容量轴基本可以结案

上面附录一说"下一步是把 wall sweep 在 graph 路径上重跑"。跑完了，见
`2026-09-15-graph-path-kv-baseline.md`。结论比预期更不利，而且不利的方向和附录一的预期
相反：

- **容量轴的全部剩余空间只有 ~1.5×**。graph 路径、bf16 KV、pin 死 640 block（163,840
  token）的基线：L=2048 在 B=70 撞墙，1744 seq/s；拟合 `step_ms = 12.839 + 0.383*B`，
  渐近吞吐 = 1000/0.383 = 2611 seq/s，也就是**即使 KV 压缩到无限小，也只有 1.50×**；
  现实的 2× 压缩只值 **1.21×**。L=8192 同理：上界 1.47×，2× 压缩值 1.20×。
- 这个拟合被独立验证过：用 640-block 那次的拟合去预测另一台机器、1193 block（容量大 1.9×）
  那次 sweep 的 cell，6 个点里 5 个误差 <4%（B=128 预测 61.86 ms vs 实测 62.89 ms）。
  常数和斜率是模型+硬件的性质，不是 pool 大小的性质。
- **注意修正的方向**：把引擎开销修对（40 ms → 12 ms）之后，容量轴的奖品是**变小**而不是变大。
  常数大才显得"加并发很值"，因为并发在摊常数。常数降到 12 ms，per-sequence 斜率立刻主导，
  容量轴很早就饱和。所以作废的 3.59× 里有一部分本身就是"引擎慢"造出来的假收益。

再修一次定价表：

| 结论 | 现状 |
|---|---|
| 容量轴收益上界 ~3.5×（正文） | 作废；附录一说的"重跑后可能更大"也**错了**，实测是 **≤1.5×**，2× 压缩只值 1.2× |
| KV 字节这条线值得优先做（附录一） | **保留但被卡住**：per-sequence 斜率的 89%(L=2048)~97%(L=8192) 确实是 per-token KV 项，钱在这儿；但本引擎现在**没有可用的 KV 字节机制** |
| `kv_quant_bits=8/4` 是低成本强基线（附录一） | **证伪**。同一 pinned pool 下 KV8 斜率反而恶化 **8.8×**，吞吐掉到 0.16×。原因是本引擎 KV4/KV8 decode 会把命中块 dequant 成完整 fp16 瞬态 buffer（代码注释自己写了，C4 曾实测反慢 5.4×），读 int8 再写一份 bf16 再读回来，比不量化更费带宽 |
| 优先 Cartridges（压 token 数）（正文） | **重新上浮**。容量轴 ≤1.5× 之后，压 token 数是攻 `c1*L` 里的 `L`，不需要新 kernel |

**方向层面的真结论**：现在要决定的不是"选哪个压缩想法"，而是**"值不值得写一个 fused
dequant-in-attention 的 decode kernel"**。在那个 kernel 存在之前，任何低秩/MLA/latent KV
方案只要是"先物化成 fp16 再 attend"，测出来都是那个瞬态 buffer 的开销，跟想法本身好不好无关。
这个坏消息现在拿到很便宜——要是先训完一个 latent KV 投影再撞上，代价就大了。

下一步（从便宜到贵）：① KV8 + Quest（引擎自带的"只 dequant top-k block"缓解手段），同一 pinned
pool 一轮 sweep 就能判断选择性 dequant 能不能救回来；② 救不回来就评估 fused int8 KV decode
kernel，这是整条 KV 字节线的真 gate；③ 有了不自伤的 KV 字节机制之后，才谈低秩 latent 对比
bf16；④ 与此并行，重新看 token 数轴（Cartridges 那条），它不依赖新 kernel。
