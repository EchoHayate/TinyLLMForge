#!/usr/bin/env python3
"""Independent-verifier contracts for ragged coalescing evidence."""
...
from tools.exact_burst_ragged_coalescing_gate import produce_gate
from tools.exact_burst_ragged_coalescing_verify import verify_bundle
from tools.test_exact_burst_ragged_coalescing_gate import (
...
def test_verifier_is_independent_and_reconstructs_gate() -> None:
    source = (
        REPO_ROOT
        / "tools/exact_burst_ragged_coalescing_verify.py"
    ).read_text(encoding="utf-8")
    assert "exact_burst_ragged_coalescing_gate import" not in source
    assert "profile_exact_burst_ragged_coalescing import" not in source
    with TemporaryDirectory() as temporary:
        ...
        assert result["performance_row_count"] == 45
        assert result["correctness_row_count"] == 36
...
def test_verifier_rejects_metric_classification_and_manifest_tamper() -> None:
    with TemporaryDirectory() as temporary:
        run_dir, repo_root = _ready_bundle(Path(temporary))
...
def test_verifier_accepts_rounding_within_one_e_minus_nine() -> None:
    with TemporaryDirectory() as temporary:
        run_dir, repo_root = _ready_bundle(Path(temporary))
        comparison_path = run_dir / "comparison.json"
        comparison = json.loads(
        ...
        assert (
            0.0
            < result["maximum_metric_disagreement"]
            <= 1e-9
        )


def main() -> None:
    test_verifier_is_independent_and_reconstructs_gate()
    test_verifier_rejects_metric_classification_and_manifest_tamper()
    test_verifier_accepts_rounding_within_one_e_minus_nine()
    print("exact burst ragged-coalescing verifier tests passed")
