#!/usr/bin/env python3
"""Safety contracts for the greedy fast-path remote controller."""
...
from tools import run_zero_temperature_greedy_fast_path_remote as remote
...
def test_remote_paths_are_confined_to_mounted_root() -> None:
    paths = remote.remote_paths(
        "20260822-qwen3-06b-greedy-fast-r1"
    )
    assert set(paths) == {
        "staging",
        "primary",
    ...
    assert all(
        "/private/tmp" not in path for path in paths.values()
    )
    assert all(
...
def test_gpu_admission_requires_strict_clean_state() -> None:
    rows = [
        ...
        _gpu_row(
            3,
            compute_processes=[{"pid": 12, "process_name": "python"}],
        ),
    ]
...
def test_remote_preflight_uses_required_pinned_tests() -> None:
...
def test_runtime_environment_is_isolated_below_staging() -> None:
    source = (
        ...
        + "/zero-temperature-greedy-fast-path/staging/tag/source"
    )
...
def test_terminal_inventory_requires_all_primary_files() -> None:
    inventory = [
        {"path": name}
        for name in remote.REQUIRED_PRIMARY_FILES
    ]
    assert (
        remote.validate_terminal_download_inventory(inventory)
        == inventory
    )
    with pytest.raises(
        ValueError,
...
def main() -> None:
    ...
    print("zero-temperature greedy remote controller tests passed")
