...
TP1_RESULT_PATH="${TP1_RESULT_PATH:-${REPO_ROOT}/artifacts/qwen35_native_mtp_tp1_4k_engine/opaque-57a3a62810d43636b96295da/local-authority/result.json}"
TP1_AUTHORITY_SHA256="f267e49281cc12e64c176fc2294f594e7b2118897092708ce1ece3bd3b9ee9ac"
...
REMOTE_PARENT="${REMOTE_PARENT:-/data00/home/sitian/sitian-workspace01/tllm/qwen35-native-mtp-tp4-4k-engine-runs}"
LOCAL_PARENT="${REPO_ROOT}/artifacts/qwen35_native_mtp_tp4_4k_engine"
...
REMOTE_TP1_RESULT="${REMOTE_RUN}/tp1-result.json"
...
if ! klist -t -c "${KRB5CCNAME}" >/dev/null 2>&1; then
  printf \
    'Kerberos credentials are missing or expired in %s\n' \
    "${KRB5CCNAME}" >&2
  exit 2
fi

if [[ ! -f "${TP1_RESULT_PATH}" ]]; then
  printf 'frozen TP1 authority is missing: %s\n' \
    "${TP1_RESULT_PATH}" >&2
  exit 2
fi
actual_tp1_sha256="$(
  shasum -a 256 "${TP1_RESULT_PATH}" | awk '{print $1}'
)"
if [[ "${actual_tp1_sha256}" != "${TP1_AUTHORITY_SHA256}" ]]; then
  printf \
    'frozen TP1 authority digest mismatch: expected=%s actual=%s\n' \
...
spec = importlib.util.spec_from_file_location(
    'qwen35_native_mtp_tp4_preflight',
    worker_path,
)
worker = importlib.util.module_from_spec(spec)
spec.loader.exec_module(worker)
model_path = '${MODEL_PATH}'
target_actual = worker.target_model_manifest_sha256(model_path)
...
mtp_actual = worker.mtp_checkpoint_manifest_sha256(model_path)
...
python3 \
  "${REPO_ROOT}/tools/verify_qwen35_native_mtp_tp4_4k_engine_gate.py" \
  "${authority_path}" \
