#!/usr/bin/env python3
"""Source-bound remote controller for the lease-local delta-journal gate."""
...
from tools.exact_burst_lease_local_delta_journal_verify import (
...
TASK_REMOTE_ROOT = (
    APPROVED_ROOT + "/exact-burst-lease-local-delta-journal"
)
...
LOCAL_ARTIFACT_ROOT = (
    REPO_ROOT / "artifacts"
    / "exact_burst_lease_local_delta_journal"
)
...
def validate_selected_gpu_still_clean(
    ...
    }.get(selected.get("index") if isinstance(selected, dict) else None)
    if (
        latest is None
...
def _require_task_tracked_diff_clean(repo_root: Path) -> None:
    result = subprocess.run(
        ["git", "diff", "--quiet", "HEAD", "--", *TASK_TRACKED_PATHS],
        cwd=repo_root,
        check=False,
    )
    ...
    while True:
        ...
        if local_destination is not None:
            ...
            _write_json(
                ...
                {
                    "schema": (
                        "exact_burst_lease_local_delta_journal_remote_v1"
                    ),
                    "status": "MONITORING",
...
def _run_remote_preflight(*, source: str, gpu_index: int) -> None:
    ...
    command = (
        ...
        + "; "
        + f"PYTHONPATH={pytest_path} {REMOTE_PYTHON} -m pytest -q "
        + "tools/test_exact_burst_lease_local_delta_journal_gate.py "
        + "tools/test_exact_burst_lease_local_delta_journal_verify.py"
    )
    _run_remote_checked(
...
def _launch_worker(
    *,
    ...
    inner = (
        ...
        + "; code=$?; "
        + f"printf '%s\\n' \"$code\" > "
        + shlex.quote(controller + "/worker.exitcode")
    )
    launch = (
        "set -eu; "
        ...
        "< /dev/null & pid=$!; "
        f"printf '%s\\n' \"$pid\" > "
        f"{shlex.quote(controller + '/worker.pid')}; "
        f"printf '%s\\n' \"$pid\" > "
        f"{shlex.quote(controller + '/worker.pgid')}; "
        "printf '%s\\n' \"$pid\""
    )
...
def _poll_worker(
    *,
    ...
    while True:
        script = "\n".join((
            "import json,pathlib",
            f"exit_path=pathlib.Path({(controller + '/worker.exitcode')!r})",
            f"pid_path=pathlib.Path({(controller + '/worker.pid')!r})",
            f"pgid_path=pathlib.Path({(controller + '/worker.pgid')!r})",
            "if not pid_path.is_file() or not pgid_path.is_file():",
            " raise ValueError('worker ownership receipt is missing')",
            f"if int(pid_path.read_text().strip()) != {worker_pid}:",
            " raise ValueError('worker PID receipt mismatch')",
            f"if int(pgid_path.read_text().strip()) != {worker_pid}:",
...
def _download_terminal_bundle(
    *,
    ...
    _write_json(local_verify_path, local_verification)
    download_receipt = {
        "schema": (
            "exact_burst_lease_local_delta_journal_remote_v1"
        ),
