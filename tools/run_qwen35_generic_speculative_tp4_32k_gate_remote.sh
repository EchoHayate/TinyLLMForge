...
export QWEN35_TP4_32K_REPO_ROOT="${REPO_ROOT}"
...
generated_runner="$(mktemp "${TMPDIR:-/tmp}/qwen35-tp4-32k-runner.XXXXXX")"
...
text = text.replace(
    'REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"',
    'REPO_ROOT="${QWEN35_TP4_32K_REPO_ROOT:?}"',
)
text = text.replace(
...
text = text.replace(
    ...
    "qwen35-generic-speculative-tp4-32k-runs",
)
authority_sources = (
...
if missing:
    raise SystemExit(
        "derived 32K runner contract is incomplete: "
        + ", ".join(missing)
    )
if "ControlMaster=" + "yes" in text:
    raise SystemExit(
        "derived 32K runner enables persistent SSH control"
    )
