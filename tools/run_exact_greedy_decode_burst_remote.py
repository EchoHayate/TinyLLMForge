#!/usr/bin/env python3
"""Safe source-bound remote controller for the exact-burst Stage-1 gate."""
...
TASK_REMOTE_ROOT = (
    APPROVED_ROOT + "/exact-greedy-decode-burst"
)
...
def remote_paths(run_tag: str) -> dict[str, str]:
    ...
    for path in paths.values():
        if not path.startswith(TASK_REMOTE_ROOT + "/"):
            raise ValueError(
                "remote path is outside the approved task root"
            )
        if (
            path.startswith("/tmp/")
...
def validate_terminal_download_inventory(
    ...
    *,
    ...
    if (
        ...
        or set(MANIFEST_PRIMARY_FILES)
        - set(manifest_artifacts)
    ):
        raise ValueError(
            "download is incomplete: manifest inventory"
        )
    names = {
        row.get("path")
    ...
    required = (
        set(manifest_artifacts)
        | {"manifest.sha256", "independent-verification.json"}
    )
...
def _upload_source_archive(
    *,
    ...
    expected_prefix = TASK_REMOTE_ROOT + "/staging/"
...
def _create_controller_dir(
    ...
    base._require_success(result, "controller receipt upload")
...
def _write_remote_completion(
    *,
    ...
    base._require_success(result, "controller completion upload")
...
def _download_terminal_bundle(
    *,
    ...
    manifest = json.loads(
        (primary / "manifest.sha256").read_text(
            encoding="utf-8"
        )
    )
...
def run_controller(args) -> dict:
    ...
    preflight = {
        "schema_version":
            "exact-greedy-decode-burst.controller.v1",
        "status": "READY",
