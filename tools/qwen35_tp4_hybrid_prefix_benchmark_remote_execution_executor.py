...
def _execute_command(
    ...
    *,
    ...
    if name == "upload":
        ...
        return {
            "returncode": 0,
            "stdout": "\n".join(row["stdout"] for row in results),
            "stderr": "\n".join(row["stderr"] for row in results),
        }
...
def _completed_row(plan, name, result):
    return {
        "name": name,
    ...
    *,
    ...
    for row in completed:
        ...
        if (
            ...
            raise ValueError(
                "failure evidence completed command mismatch"
            )
    return {
    ...
    *,
    ...
    for name in plan["command_order"]:
        try:
            ...
            raise
        completed.append(_completed_row(plan, name, result))
...
def execute_verified_plan_file(
    *,
    ...
    extract_argv = plan["commands"]["safe_extract"]["argv"]
    local_targets = (
        Path(output_path),
        Path(failure_path),
        package,
