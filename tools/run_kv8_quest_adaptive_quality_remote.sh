#!/usr/bin/env bash
# Source-bound Qwen3-8B quality gate for adaptive KV8 + Quest activation.
#
# Runs KV8 full attention and adaptive Quest top-16 in one fresh engine on
# identical fixed prompts. Remote data stays below the approved mounted root.
set -euo pipefail
...
RUN_TAG="${RUN_TAG:-kv8-quest-adaptive-quality-$(date +%Y%m%d-%H%M%S)}"
...
SSH_SOCKET="${SSH_SOCKET:-/tmp/ssh-kv8-quest-adaptive-quality}"
...
COMMON_ARGS=(
  ...
  --out-json "${REMOTE_DIR}/kv8_adaptive_paired.json"
)
printf -v COMMON_ARGS_Q ' %q' "${COMMON_ARGS[@]}"

"${SSH_STREAM[@]}" \
  "set -o pipefail; ${REMOTE_ENV} '${REMOTE_PYTHON}' '${REMOTE_DIR}/source/tools/eval_needle.py'${COMMON_ARGS_Q} 2>&1 | tee '${REMOTE_DIR}/kv8_adaptive_paired.log'"

for artifact in kv8_adaptive_paired.json kv8_adaptive_paired.log; do
  "${SSH_STREAM[@]}" "cat '${REMOTE_DIR}/${artifact}'" > "${LOCAL_OUT}/${artifact}"
done
"${SSH[@]}" "sha256sum '${REMOTE_DIR}/kv8_adaptive_paired.json'" \
  > "${LOCAL_OUT}/remote_sha256.txt"

echo "adaptive quality artifacts written to ${LOCAL_OUT}"
