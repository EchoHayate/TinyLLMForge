...
DIST_PORT="${DIST_PORT:-29641}"
MASTER_PORT="${MASTER_PORT:-29741}"
HARD_TIMEOUT_SECONDS="${HARD_TIMEOUT_SECONDS:-5400}"
RUN_TAG="${RUN_TAG:-tp4-qwen3-b4-proposal-forward-diagnostic-$(date -u +%Y%m%dT%H%M%SZ)}"
...
"${SSH[@]}" \
  "if [[ -e '${REMOTE_RUN}' ]]; then
   ...
   mkdir -p '${REMOTE_SOURCE}' '${REMOTE_ARTIFACTS}/workers' '${REMOTE_ARTIFACTS}/logs'"
...
if [[ "${preflight_status}" -eq 0 ]]; then
  ...
  "${remote_python}" -m pytest -q \
    tools/test_autoregressive_draft_executor.py \
    >"${remote_artifacts}/executor-tests.log" 2>&1
  preflight_status=$?
fi
...
"${python_executable}" \
  tools/autoregressive_draft_performance_worker.py \
    ...
    --warmup-runs 2 \
    --measured-runs 8 \
    --out "${artifacts}/workers/target-b4.json" \
    >"${artifacts}/logs/target-b4.log" 2>&1
...
"${python_executable}" \
  tools/autoregressive_draft_performance_worker.py \
    ...
    --warmup-runs 2 \
    --measured-runs 8 \
    --out "${artifacts}/workers/learned-b4.json" \
    >"${artifacts}/logs/learned-b4.log" 2>&1
...
if [[ "${remote_status}" -ne 0 ]]; then
  printf 'remote diagnostic failed: %s\n' "${remote_status}" >&2
  for log_path in \
    "${LOCAL_RUN}/logs/target-b4.log" \
    "${LOCAL_RUN}/logs/learned-b4.log" \
    "${LOCAL_RUN}/diagnostic.log"; do
    if [[ -f "${log_path}" ]]; then
      printf '\n==> %s <==\n' "${log_path}" >&2
      tail -200 "${log_path}" >&2
    fi
