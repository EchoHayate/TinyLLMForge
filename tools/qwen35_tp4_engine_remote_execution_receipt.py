...
SCHEMA_VERSION = "qwen35.tp4-engine-remote-execution-receipt.v1"
MAX_LOG_BYTES = 64 * 1024
MIN_GPU_FREE_BYTES = 24 * 1024**3
PASS_STEPS = {"guarded_authority", "local_verify"}


def _load_resource_policy():
    module_name = (
        "qwen35_tp4_correctness_resource_policy_for_engine_receipt"
    )
...
def _validate_resource_guard(plan, step):
    payload = _parse_json_log(step["stdout"], "resource guard")
    return _validate_resource_payload(plan, payload)
...
def _validate_resource_payload(plan, payload):
    ...
    try:
        return resource_policy_module.validate_guard_payload(
            resource_policy,
            payload,
            gpu_indices=gpu_indices,
            baseline=baseline,
...
def _validate_pass_payload(step, label, *, require_inventory):
    payload = _parse_json_log(step["stdout"], label)
    expected_inventory = [
        "reference_authority",
    ...
    if require_inventory and payload["inventory"] != expected_inventory:
        raise ValueError(f"{label} PASS inventory is invalid")
    if (
        payload["classification"] != "PASS"
        or payload["reference_classification"] != "PASS"
        or payload["engine_classification"] != "PASS"
        or any(
    ...
    *,
    ...
    for name, step in zip(plan["command_order"], steps):
        ...
        if package_output and (
            not isinstance(step["output_sha256"], str)
            or len(step["output_sha256"]) != 64
            or any(
                character not in "0123456789abcdef"
                for character in step["output_sha256"]
            )
            or isinstance(step["output_size"], bool)
            or not isinstance(step["output_size"], int)
            or step["output_size"] <= 0
        ):
            raise ValueError("execution receipt package output mismatch")
        by_name[name] = step
    gpu_indices = _validate_resource_guard(
        plan,
        by_name["resource_guard"],
    )
    ...
    if len(marked) != 1 or not guarded_lines:
        raise ValueError(
            "guarded authority final resource receipt is invalid"
        )
    final_resource = _parse_json_log(
        marked[0],
        "guarded authority final resource",
    )
    final_gpu_indices = _validate_resource_payload(
        plan,
        final_resource,
    )
    preflight_resource = _parse_json_log(
        by_name["resource_guard"]["stdout"],
        "resource guard",
    )
    if [
        (row["gpu_index"], row["gpu_uuid"])
        for row in final_resource["selected"]
    ] != [
        (row["gpu_index"], row["gpu_uuid"])
        for row in preflight_resource["selected"]
    ]:
    ...
    if authority["source_tree_sha256"] != plan["source_tree_sha256"]:
    ...
    return {
        ...
        "workload_manifest_sha256": (
            authority["workload_manifest_sha256"]
        ),
        "gpu_indices": list(final_gpu_indices),
