...
REMOTE_ARTIFACT="${REMOTE_RUN_ROOT}/artifacts/qwen35_mtp_real_checkpoint_gate.json"
...
"${SSH[@]}" "${REMOTE_HOST}" \
  "cd '${REMOTE_RUN_ROOT}' && \
   ...
   TINYVLLM_DIST_PORT='${TINYVLLM_DIST_PORT:-29357}' \
...
printf 'local_artifact=%s\n' \
  "${LOCAL_RUN_ROOT}/qwen35_mtp_real_checkpoint_gate.json"
