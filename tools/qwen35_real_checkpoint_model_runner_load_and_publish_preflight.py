...
publication_gate = _load_sibling(
    "_qwen35_model_runner_load_publish_publication_method_base",
...
PUBLICATION_METHOD_ARTIFACT_SHA256 = (
...
PUBLICATION_METHOD_SOURCE_TREE_SHA256 = (
...
REMOTE_TARGET = publication_gate.REMOTE_TARGET
REMOTE_PYTHON = publication_gate.REMOTE_PYTHON
APPROVED_MODEL_DIR = publication_gate.APPROVED_MODEL_DIR
APPROVED_MODEL_MANIFEST_SHA256 = (
    publication_gate.APPROVED_MODEL_MANIFEST_SHA256
)
APPROVED_CONFIG_SHA256 = publication_gate.APPROVED_CONFIG_SHA256
APPROVED_INDEX_SHA256 = publication_gate.APPROVED_INDEX_SHA256
APPROVED_COMPOSITE_SHA256 = publication_gate.APPROVED_COMPOSITE_SHA256
APPROVED_SHARD_NAME = publication_gate.APPROVED_SHARD_NAME
APPROVED_SHARD_SIZE = publication_gate.APPROVED_SHARD_SIZE
APPROVED_SHARD_SHA256 = publication_gate.APPROVED_SHARD_SHA256
AUTHORIZATION_SHA256 = publication_gate.AUTHORIZATION_SHA256
MAX_TENSOR_BYTES = publication_gate.MAX_TENSOR_BYTES
STREAMED_STATS = dict(publication_gate.STREAMED_STATS)
MEMORY_CEILINGS_KIB = dict(publication_gate.MEMORY_CEILINGS_KIB)
LOCAL_RUN_ROOT = publication_gate.LOCAL_RUN_ROOT
REMOTE_RUN_ROOT = (
    "/data00/home/sitian/sitian-workspace01/tllm/"
    "qwen35-real-checkpoint-model-runner-load-and-publish-runs"
)
WORKER_CONTEXTS = (
    (1, 0, "success"),
...
SOURCE_FILES = (
    *publication_gate.SOURCE_FILES,
    "tools/qwen35_real_checkpoint_model_runner_load_and_publish_preflight.py",
)
_read_proc_status = publication_gate._read_proc_status
_memory_point = publication_gate._memory_point
_install_namespace_packages = publication_gate._install_namespace_packages
_source_tree_sha256 = publication_gate._source_tree_sha256
_atomic_write_json = publication_gate._atomic_write_json
validate_run_tag = publication_gate.validate_run_tag
build_ssh_command = publication_gate.build_ssh_command
_require_success = publication_gate._require_success
...
def _read_exact_json(path, expected_sha256):
    ...
    if _sha256(payload) != expected_sha256:
        raise ValueError(
            "ModelRunner load-and-publish prerequisite hash is invalid"
        )
    try:
        value = json.loads(payload)
    except (UnicodeDecodeError, json.JSONDecodeError) as error:
        raise ValueError(
            "ModelRunner load-and-publish prerequisite JSON is invalid"
        ) from error
    if not isinstance(value, dict):
        raise ValueError(
            "ModelRunner load-and-publish prerequisite must be an object"
        )
    return value
...
def _row_map(rows):
    ...
    for row in rows:
        ...
        if key in result:
            raise ValueError(
                "duplicate ModelRunner load-and-publish prerequisite row"
            )
        result[key] = MappingProxyType(row)
    if tuple(result) != PREREQUISITE_ROWS:
        raise ValueError(
            "ModelRunner load-and-publish prerequisite TP rows are invalid"
        )
    return MappingProxyType(result)


@dataclass(frozen=True)
class ModelRunnerLoadPublishPrerequisites:
    complete_artifact_sha256: str
    publication_method_artifact_sha256: str
    complete_source_tree_sha256: str
    publication_method_source_tree_sha256: str
    complete_rows: tuple[tuple[int, int], ...]
    publication_method_rows: tuple[tuple[int, int], ...]
    complete_row_map: MappingProxyType
    publication_method_row_map: MappingProxyType
    publication_method_source_file_sha256: MappingProxyType


def load_model_runner_load_publish_prerequisites(
    complete_artifact,
    publication_method_artifact,
) -> ModelRunnerLoadPublishPrerequisites:
    complete_record = (
        publication_gate.publication.ownership.loader_core
        .load_complete_gate_oracle(complete_artifact)
    )
    publication_record = _read_exact_json(
        publication_method_artifact,
        PUBLICATION_METHOD_ARTIFACT_SHA256,
    )
    publication_gate.validate_model_runner_local_publication_preflight(
        publication_record
    )
    if (
    ...
    if (
        ...
        raise ValueError(
            "publication-method prerequisite source tree is invalid"
        )
    ...
    if len({row["process_id"] for row in publication_record["rows"]}) != 6:
        raise ValueError(
            "publication-method prerequisite process IDs are invalid"
        )
    source_hashes = publication_record["source_file_sha256"]
    if not isinstance(source_hashes, dict) or len(source_hashes) != 49:
        raise ValueError(
            "publication-method prerequisite source closure is invalid"
        )
    return ModelRunnerLoadPublishPrerequisites(
        ...
        publication_method_rows=tuple(publication_rows),
        complete_row_map=complete_rows,
        publication_method_row_map=publication_rows,
        publication_method_source_file_sha256=MappingProxyType(
            source_hashes
        ),
    )
...
def select_model_runner_load_publish_prerequisite_rows(
    prerequisites,
    tensor_parallel_size,
    tensor_parallel_rank,
):
    if type(prerequisites) is not ModelRunnerLoadPublishPrerequisites:
        raise ValueError(
    ...
    try:
        return (
            ...
            prerequisites.publication_method_row_map[key],
        )
    except KeyError as error:
        raise ValueError(
            "ModelRunner load-and-publish prerequisite TP row is invalid"
        ) from error
...
def _model_runner_method_node(source):
    ...
    if len(methods) != 1:
        raise ValueError(
            "ModelRunner load-and-publish method identity is invalid"
        )
    method = methods[0]
    if [argument.arg for argument in method.args.args] != [
        ...
        raise ValueError(
            "ModelRunner load-and-publish method arguments are invalid"
        )
    names = {
        node.id
    ...
    if names - builtins_and_locals != ALLOWED_GLOBALS:
        raise ValueError(
            "ModelRunner load-and-publish globals are invalid"
        )
    calls = [
        node
        for node in ast.walk(method)
    ...
    if (
        ...
        or len(loader_calls) != 1
        or len(publish_calls) != 1
        or not (
            validator_calls[0].lineno
            < loader_calls[0].lineno
            < publish_calls[0].lineno
        )
    ):
        raise ValueError(
            "ModelRunner load-and-publish call ordering is invalid"
        )
    publish_receiver = publish_calls[0].func.value
    if (
        not isinstance(publish_receiver, ast.Attribute)
        or publish_receiver.attr
        != "qwen35_loaded_checkpoint_candidate_slot"
        or not isinstance(publish_receiver.value, ast.Name)
        or publish_receiver.value.id != "self"
        or len(publish_calls[0].args) != 1
        or not isinstance(publish_calls[0].args[0], ast.Name)
        or publish_calls[0].args[0].id != "candidate"
    ):
        raise ValueError(
            "ModelRunner load-and-publish publication call is invalid"
        )
    assignments = [
        node
        for node in ast.walk(method)
    ...
    for node in assignments:
        for target in node.targets:
            if (
                ...
                completion_lines[target.attr] = node.lineno
    if (
        set(completion_lines)
        != {
            "qwen35_checkpoint_candidate_load_request",
            "qwen35_checkpoint_candidate_load_configuration",
        }
        or any(
            line <= publish_calls[0].lineno
            for line in completion_lines.values()
        )
    ):
        raise ValueError(
            "ModelRunner load-and-publish completion ordering is invalid"
        )
    handlers = [
        handler
        for node in ast.walk(method)
    ...
    if (
        ...
        raise ValueError(
            "ModelRunner load-and-publish error boundary is invalid"
        )
    return method


def load_frozen_model_runner_load_publish_method(
    source_root,
    *,
    loaded_candidate_type,
    request_validator,
):
    if not isinstance(loaded_candidate_type, type):
        raise ValueError("loaded_candidate_type must be a type")
    if not callable(request_validator):
        raise ValueError("request_validator must be callable")
    ...
    if (
        ...
        raise ValueError(
            "ModelRunner load-and-publish method source hash is invalid"
        )
    method.decorator_list = []
    module = ast.Module(body=[method], type_ignores=[])
    namespace = {
        "Qwen35LoadedCheckpointCandidate": loaded_candidate_type,
        "validate_qwen35_checkpoint_candidate_load_request": (
            request_validator
        ),
    }
...
def execute_model_runner_load_publish_scope(
    *,
    ...
    if mode not in ("success", "injected_publication_failure"):
        raise ValueError(
            "ModelRunner load-and-publish mode is invalid"
        )
    ...
    def execute_nested_scope():
        ...
        registered = (
            publication_gate.publication.ownership.loader_core
            ._registered_tensors(model)
        )
        identity = (
            publication_gate.publication.ownership.loader_core
            ._snapshot_identity(registered)
        )
        pool_snapshot = publication_gate.publication._snapshot_pool(
            pool
        )
        selected = {}
        ...
        if any(
            ...
            raise RuntimeError(
                "ModelRunner load-and-publish destination "
                "initialization failed"
            )
        production_slot = production_slot_factory()
        if production_slot.candidate is not None:
            raise ValueError(
                "ModelRunner load-and-publish slot must start empty"
            )
        if target._consumed is not False:
            raise ValueError(
                "ModelRunner load-and-publish target must be unconsumed"
            )
        ...
        class RejectingSlot:
            ...
            def publish(self, candidate):
                ...
                self.attempted_candidate = candidate
                raise RuntimeError(
        ...
        def counted_loader(value):
            ...
            if adapter_call_count != 1:
                raise RuntimeError(
                    "ModelRunner load-and-publish loader called twice"
                )
        ...
        if mode == "success":
            ...
            proxy = RejectingSlot(production_slot)
            runner.qwen35_loaded_checkpoint_candidate_slot = proxy
        method_call_count = 1
        method_row = publication_method(runner, request)
        candidate = (
            production_slot.candidate
            if mode == "success"
            else proxy.attempted_candidate
        )
        if candidate is None:
            raise RuntimeError(
                "ModelRunner load-and-publish candidate is missing"
            )
        if target._consumed is not True:
            raise RuntimeError(
                "ModelRunner load-and-publish target was not consumed"
            )
        result = (
            publication_gate.publication.ownership
            .validate_private_loaded_candidate(
                candidate=candidate,
                target=target,
        ...
        if mode == "success":
            if method_row != expected_published_row:
                raise ValueError(
                    "ModelRunner load-and-publish success row is invalid"
                )
            if production_slot.candidate is not candidate:
                raise ValueError(
                    "ModelRunner load-and-publish candidate visibility "
                    "is invalid"
                )
            if (
                runner.qwen35_checkpoint_candidate_load_request
                is not request
                or runner.qwen35_checkpoint_candidate_load_configuration
                != configuration
            ):
                raise ValueError(
                    "ModelRunner load-and-publish completion state "
                    "is invalid"
                )
            production_publish_call_count = 1
            proxy_publish_call_count = 0
            production_slot_remained_empty = False
            completion_state_committed = True
        else:
            if method_row != expected_error_row:
                raise ValueError(
                    "ModelRunner load-and-publish failure row is invalid"
                )
            if (
                production_slot.candidate is not None
                or runner.qwen35_checkpoint_candidate_load_request
                is not None
                or runner
                .qwen35_checkpoint_candidate_load_configuration
                is not None
        ...
        try:
            ...
            if any(
                ...
                raise RuntimeError(
                    "ModelRunner load-and-publish destination clear "
                    "is incomplete"
                )
            if any(
                ...
                raise RuntimeError(
                    "ModelRunner load-and-publish non-selected tensor "
                    "changed"
                )
            if not publication_gate.publication._pool_unchanged(
                pool,
                pool_snapshot,
            ):
                raise RuntimeError(
                    "ModelRunner load-and-publish pool state changed"
                )
        except Exception as caught:
            if clear_error is None:
                clear_error = caught
        if clear_error is not None:
            raise RuntimeError(
                "ModelRunner load-and-publish scope cleanup failed"
            ) from clear_error
        result.update({
            ...
            "production_slot_remained_empty": (
                production_slot_remained_empty
            ),
            "completion_state_committed": (
                completion_state_committed
            ),
            ...
            "alias_groups": (
                publication_gate.publication.ownership.loader_core
                ._alias_groups(target.binding_plan)
            ),
    ...
    if not all(collected.values()):
        ...
        raise RuntimeError(
            "ModelRunner load-and-publish objects escaped scope: "
            + ", ".join(escaped)
        )
    result["collected_private_objects"] = collected
    result["all_private_load_publish_objects_collected"] = True
    return result
...
def _validate_memory(row):
    ...
    if not isinstance(memory, Mapping) or set(memory) != set(names):
        raise ValueError(
            "ModelRunner load-and-publish memory points are invalid"
        )
    observed = (
    ...
    if observed != recomputed:
        raise ValueError(
            "ModelRunner load-and-publish memory deltas are invalid"
        )
    ...
    if any(
        ...
        raise ValueError(
            "ModelRunner load-and-publish memory ceiling exceeded: "
            + details
        )


def validate_model_runner_load_publish_row(row):
    tp = (row.get("tp_size"), row.get("tp_rank"))
    mode = row.get("mode")
    if (
        row.get("schema_version") != ROW_SCHEMA_VERSION
        or tp not in PREREQUISITE_ROWS
        or mode not in ("success", "injected_publication_failure")
    ):
        raise ValueError(
            "ModelRunner load-and-publish row schema is invalid"
        )
    exact = {
        "status": "PASS",
        ...
        "pool_unchanged": True,
        "non_selected_tensors_unchanged": True,
        "all_private_load_publish_objects_collected": True,
    ...
    for name, value in exact.items():
        if row.get(name) != value:
            raise ValueError(
                f"ModelRunner load-and-publish {name} is invalid"
            )
    ...
    if (
        ...
        raise ValueError(
            "ModelRunner load-and-publish hash evidence is invalid"
        )
    expected_collected = {
        "runner": True,
    ...
    expected_row = {
        ...
        "detail": (
            ...
            if mode == "success"
                ...
                "RuntimeError: injected ModelRunner "
                "load-and-publish failure"
            )
        ),
        "status": "published" if mode == "success" else "error",
    }
    expected = {
        "method_row": expected_row,
        "completion_configuration": (
            expected_configuration if mode == "success" else None
        ),
        "completion_request_verified": mode == "success",
        "completion_state_committed": mode == "success",
        "production_publish_call_count": (
            1 if mode == "success" else 0
        ),
        "proxy_publish_call_count": (
            0 if mode == "success" else 1
        ),
        "production_slot_remained_empty": mode != "success",
        "production_slot_visibility_verified": mode == "success",
    }
    if mode != "success":
    ...
    if (
        ...
        raise ValueError(
            "ModelRunner load-and-publish process ID is invalid"
        )
    _validate_memory(row)
    return row


def run_model_runner_load_publish_rank_worker(
    *,
    checkpoint_dir,
    source_root,
    complete_artifact,
    ...
    prerequisites = load_model_runner_load_publish_prerequisites(
        complete_artifact,
        publication_method_artifact,
    )
    oracle_row, _publication_row = (
        select_model_runner_load_publish_prerequisite_rows(
            prerequisites,
            tensor_parallel_size,
            tensor_parallel_rank,
        )
    ...
    method = load_frozen_model_runner_load_publish_method(
        source_root,
        loaded_candidate_type=(
    ...
    def private_graph_factory():
        ...
        def provide_target():
            ...
            if diagnostics["provider_call_count"] != 1:
                raise RuntimeError(
                    "ModelRunner load-and-publish target provider "
                    "called twice"
                )
            return target
    ...
    result = execute_model_runner_load_publish_scope(
    ...
    if diagnostics["provider_call_count"] != 1:
        raise RuntimeError(
            "ModelRunner load-and-publish provider count is invalid"
        )
    result["provider_call_count"] = diagnostics["provider_call_count"]
    if mode == "success":
        result["production_slot_visibility_verified"] = True
        result["completion_request_verified"] = True
        result["completion_configuration"] = [
            APPROVED_MODEL_DIR,
            APPROVED_MODEL_MANIFEST_SHA256,
            MAX_TENSOR_BYTES,
            AUTHORIZATION_SHA256,
        ...
        result["production_slot_visibility_verified"] = False
        result["completion_request_verified"] = False
        result["completion_configuration"] = None
    ...
    validate_model_runner_load_publish_row(row)
    return row


def validate_model_runner_load_publish_preflight(record):
    exact = {
    ...
    if any(record.get(name) != value for name, value in exact.items()):
        raise ValueError(
            "ModelRunner load-and-publish preflight identity is invalid"
        )
    hashes = record.get("source_file_sha256")
    if set(hashes or {}) != set(SOURCE_FILES):
        raise ValueError(
            "ModelRunner load-and-publish source hashes are invalid"
        )
    if hashes[MODEL_RUNNER_SOURCE] != MODEL_RUNNER_FILE_SHA256:
        raise ValueError(
            "ModelRunner load-and-publish source file hash is invalid"
        )
    if record.get("source_tree_sha256") != _source_tree_sha256(
        hashes
    ):
        raise ValueError(
            "ModelRunner load-and-publish source tree is invalid"
        )
    ...
    if [
        ...
        raise ValueError(
            "ModelRunner load-and-publish worker rows are invalid"
        )
    for row in rows:
        validate_model_runner_load_publish_row(row)
    if len({row["process_id"] for row in rows}) != 6:
        raise ValueError(
            "ModelRunner load-and-publish process IDs must be unique"
        )
    return record
...
def _source_hashes(source_root):
    ...
    for name in SOURCE_FILES:
        ...
        if not path.is_file():
            raise ValueError(
                f"missing ModelRunner load-and-publish source: {name}"
            )
...
def build_source_tar(source_root):
    ...
    with tarfile.open(fileobj=stream, mode="w") as archive:
        for name in SOURCE_FILES:
            ...
            if not path.is_file():
                raise ValueError(
                    "missing ModelRunner load-and-publish source: "
                    + name
                )
...
def stage_source_and_prerequisites(
    ...
    *,
    ...
    if (
        _sha256(publication_payload)
        != PUBLICATION_METHOD_ARTIFACT_SHA256
    ):
        raise ValueError(
            "publication-method prerequisite artifact hash is invalid"
        )
    ...
    if {
        ...
        for name in publication_gate.SOURCE_FILES
    } != dict(prerequisites.publication_method_source_file_sha256):
        raise ValueError(
            "ModelRunner load-and-publish source does not match "
            "publication-method prerequisite"
        )
    if local_hashes[MODEL_RUNNER_SOURCE] != MODEL_RUNNER_FILE_SHA256:
        raise ValueError(
            "ModelRunner load-and-publish production source was modified"
        )
    ...
    _require_success(
        staged,
        "ModelRunner load-and-publish source staging",
    )
    for remote_path, payload, label in (
        (remote_complete, complete_payload, "complete"),
        (remote_publication, publication_payload, "publication-method"),
    ):
        ...
        _require_success(
            completed,
            f"ModelRunner load-and-publish {label} prerequisite staging",
        )
    script = "\n".join([
    ...
    _require_success(
        verified,
        "ModelRunner load-and-publish staged hashing",
    )
    remote = json.loads(verified.stdout)
    if remote.get("source") != local_hashes:
        raise ValueError(
            "ModelRunner load-and-publish remote source hash mismatch"
        )
    if remote.get("complete") != COMPLETE_ARTIFACT_SHA256:
        raise ValueError(
            "ModelRunner load-and-publish remote complete hash mismatch"
        )
    if (
        remote.get("publication")
        != PUBLICATION_METHOD_ARTIFACT_SHA256
    ):
        raise ValueError(
            "ModelRunner load-and-publish remote prerequisite mismatch"
        )
    if (
        remote.get("method")
        != MODEL_RUNNER_LOAD_PUBLISH_METHOD_SHA256
        ...
        raise ValueError(
            "ModelRunner load-and-publish remote method hash mismatch"
        )
    return {
        "remote_source_dir": remote_source_dir,
        "remote_complete_artifact": remote_complete,
        "remote_publication_method_artifact": remote_publication,
        ...
        "publication_method_artifact_sha256": remote["publication"],
        "model_runner_load_publish_method_sha256": remote["method"],
    }
...
def _aggregate(rows, source_root):
    ...
    validate_model_runner_load_publish_preflight(record)
    return record
...
def _source_manifest(run_tag, staged):
    return {
        ...
        "remote_publication_method_artifact": (
            staged["remote_publication_method_artifact"]
        ),
        "complete_artifact_sha256": (
            staged["complete_artifact_sha256"]
        ),
        "publication_method_artifact_sha256": (
            staged["publication_method_artifact_sha256"]
        ),
        "model_runner_file_sha256": MODEL_RUNNER_FILE_SHA256,
        "model_runner_load_publish_method_sha256": (
            staged["model_runner_load_publish_method_sha256"]
        ),
...
def run_remote_model_runner_load_publish_preflight(
    source_root,
    run_tag,
    *,
    staged,
    ...
    if destination.exists():
        raise ValueError(
            "local ModelRunner load-and-publish directory exists: "
            f"{destination}"
        )
    remote_run_dir = f"{REMOTE_RUN_ROOT}/{run_tag}"
    remote_artifact = (
    ...
    for tp_size, tp_rank, mode in WORKER_CONTEXTS:
        ...
        _require_success(
            completed,
            "ModelRunner load-and-publish rank worker",
        )
    ...
    _require_success(
        finalized,
        "ModelRunner load-and-publish finalizer",
    )
    ...
    if (
        ...
        raise ValueError(
            "ModelRunner load-and-publish source binding mismatch"
        )
    ...
    script = "\n".join([
        ...
        "record=json.loads((root/'model_runner_load_and_publish_preflight.json').read_text())",
        ...
        "result={'model_runner_load_and_publish_preflight':record,'source_manifest':json.loads((root/'source_manifest.json').read_text())}",
    ...
    round_trip = command_runner(
        ...
        input=json.dumps({
            "model_runner_load_and_publish_preflight": record,
    ...
    _require_success(
        round_trip,
        "ModelRunner load-and-publish artifact round trip",
    )
    if json.loads(round_trip.stdout) != {
        ...
        raise ValueError(
            "ModelRunner load-and-publish artifact round-trip mismatch"
        )
...
def execute_remote_model_runner_load_publish_preflight(
    source_root,
    run_tag,
    *,
    complete_artifact,
    ...
    staged = stage_source_and_prerequisites(
        ...
        publication_method_artifact=publication_method_artifact,
        command_runner=command_runner,
    )
    return run_remote_model_runner_load_publish_preflight(
        source_root,
        run_tag,
        staged=staged,
        local_run_root=local_run_root,
...
def _rank_worker_main(arguments):
    ...
    row = run_model_runner_load_publish_rank_worker(
        checkpoint_dir=arguments.checkpoint_dir,
        source_root=arguments.source_root,
        complete_artifact=arguments.complete_artifact,
        publication_method_artifact=(
            arguments.publication_method_artifact
        ),
...
def _finalize_main(arguments):
    ...
    if output.exists():
        raise ValueError(
            "ModelRunner load-and-publish output already exists"
        )
    payload = json.load(sys.stdin)
...
def main(argv=None):
    ...
    worker_parser.add_argument(
        "--attempt-mode",
        choices=("success", "injected_publication_failure"),
        required=True,
    )
    ...
    if arguments.mode == "validate":
        ...
        record = execute_remote_model_runner_load_publish_preflight(
