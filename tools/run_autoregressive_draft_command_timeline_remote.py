...
def _build_gpu_sampler_script() -> str:
    return "\n".join((
        ...
        "indices=[row.get('index') for row in inventory]",
        "uuids=[row.get('uuid') for row in inventory]",
        ...
        "nvml.nvmlDeviceGetUUID.argtypes=[ctypes.c_void_p,",
        " ctypes.POINTER(ctypes.c_char),ctypes.c_uint]",
        "nvml.nvmlDeviceGetUUID.restype=ctypes.c_int",
        "nvml.nvmlDeviceGetPerformanceState.argtypes=[",
        " ctypes.c_void_p,ctypes.POINTER(ctypes.c_int)]",
        "nvml.nvmlDeviceGetPerformanceState.restype=ctypes.c_int",
        "nvml.nvmlDeviceGetClockInfo.argtypes=[ctypes.c_void_p,",
        ...
        "nvml.nvmlDeviceGetTemperature.argtypes=[ctypes.c_void_p,",
        " ctypes.c_int,ctypes.POINTER(ctypes.c_uint)]",
        ...
        "nvml.nvmlDeviceGetCurrentClocksThrottleReasons.argtypes=[",
        " ctypes.c_void_p,ctypes.POINTER(ctypes.c_ulonglong)]",
        "nvml.nvmlDeviceGetCurrentClocksThrottleReasons.restype="
        "ctypes.c_int",
        "def check(name,result):",
        " if result==NVML_SUCCESS:",
        "  return",
        " detail=nvml.nvmlErrorString(result)",
        ...
        "initialized=False",
        "try:",
        " check('nvmlInit_v2',nvml.nvmlInit_v2())",
        " initialized=True",
        " devices=[]",
        " for index,expected_uuid in expected:",
        "  handle=ctypes.c_void_p()",
        "  check('nvmlDeviceGetHandleByIndex_v2',",
        ...
        "  check('nvmlDeviceGetUUID',nvml.nvmlDeviceGetUUID(",
        "   handle,uuid_buffer,NVML_DEVICE_UUID_BUFFER_SIZE))",
        "  observed_uuid=uuid_buffer.value.decode('utf-8')",
        "  if observed_uuid!=expected_uuid:",
        "   raise ValueError('GPU UUID inventory changed')",
        "  devices.append((index,observed_uuid,handle))",
        " next_sample_ns=time.monotonic_ns()",
        " while not stop_requested:",
        "  snapshot=[]",
        "  for index,gpu_uuid,handle in devices:",
        "   pstate=ctypes.c_int()",
        ...
        "  milliseconds=(unix_ns//1000000)%1000",
        "  timestamp=(time.strftime('%Y/%m/%d %H:%M:%S',",
        "   time.gmtime(seconds))+'.'+str(milliseconds).zfill(3))",
        "  for row in snapshot:",
        "   print(json.dumps({",
        "     **row,",
        ...
        "  if shutdown_result!=NVML_SUCCESS and not stop_requested:",
        "   check('nvmlShutdown',shutdown_result)",
    ))
...
def _start_epoch_samplers(
    *,
    ...
    if (
        len(inventory) != 4
        or len(gpu_indices) != len(gpu_uuids)
        or len({row["index"] for row in inventory}) != 4
        or len({row["uuid"] for row in inventory}) != 4
    ):
        raise ValueError("GPU inventory is invalid")
    ...
    try:
        processes.append(subprocess.Popen(
            [sys.executable, "-c", gpu_script, json.dumps(inventory)],
            stdout=handles[0],
            stderr=handles[2],
        ))
        processes.append(subprocess.Popen(
