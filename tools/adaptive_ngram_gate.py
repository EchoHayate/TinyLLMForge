...
def expand_owned_source_paths(repo_root: Path) -> tuple[str, ...]:
    return source_audit.expand_owned_source_paths(
        repo_root,
        OWNED_SOURCE_ROOTS,
    )
...
def hash_source_tree(
    ...
    return source_audit.hash_source_tree(
        source_root,
        relative_paths,
    )
...
def source_tree_sha256(files: list[dict]) -> str:
    return source_audit.source_tree_sha256(files)
...
def reconstruct_source_snapshot(
    ...
    source_audit.reconstruct_source_snapshot(
        repo_root,
        source_root,
        evidence,
        patch_path,
...
def build_source_evidence(repo_root: Path, out_dir: Path) -> dict:
    return source_audit.build_source_evidence(
        repo_root,
        out_dir,
        owned_roots=OWNED_SOURCE_ROOTS,
        ignored_untracked_prefixes=GENERATED_ARTIFACT_ROOTS,
    )
