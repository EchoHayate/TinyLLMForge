...
FROZEN_RUNNER="${REPO_ROOT}/tools/run_qwen35_native_mtp_tp4_16k_target_kv_offload_remote.sh"
export QWEN35_NATIVE_MTP_TP4_32K_REPO_ROOT="${REPO_ROOT}"
...
generated_runner="$(
  mktemp \
    "${TMPDIR:-/tmp}/qwen35-native-mtp-tp4-32k-runner.XXXXXX"
)"
...
text = text.replace(
    'REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"',
    'REPO_ROOT="${QWEN35_NATIVE_MTP_TP4_32K_REPO_ROOT:?}"',
)
text = text.replace(
    "qwen35_native_mtp_tp4_16k_target_kv_offload",
    "qwen35_native_mtp_tp4_32k_target_kv_offload",
)
text = text.replace(
    "qwen35-native-mtp-tp4-16k-target-kv-offload-runs",
    "qwen35-native-mtp-tp4-32k-target-kv-offload-runs",
)
authority_sources = (
...
if missing:
    raise SystemExit(
        "derived native-MTP 32K runner contract is incomplete: "
        + ", ".join(missing)
    )
if "ControlMaster=" + "yes" in text:
    raise SystemExit(
        "derived native-MTP 32K runner enables "
        "persistent SSH control"
    )
