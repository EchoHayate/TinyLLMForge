#!/usr/bin/env python3
"""Safety contracts for the continuation-epoch remote controller."""
...
from tools import run_exact_burst_continuation_epoch_remote as remote


def _raises(expected, function, *args, **kwargs) -> Exception:
    try:
...
def test_paths_runtime_and_archive_are_confined() -> None:
    paths = remote.remote_paths(
        "20260823-qwen3-06b-exact-burst-continuation-r1"
    )
    expected_root = (
...
def test_preflight_worker_and_gates_use_continuation_tools() -> None:
    ...
    assert pid == 321
    joined = "\n".join(captured)
    assert "tools/profile_exact_burst_continuation_epoch.py" in joined
    assert "tools/exact_burst_continuation_epoch_gate.py" in joined
    assert "tools/exact_burst_continuation_epoch_verify.py" in joined
    for required_arg in (
...
def test_gpu_admission_and_polling_contract() -> None:
    rows = [
    ...
    attempts = []
    original_run = remote.base._run_remote
    original_sleep = remote.time.sleep
    remote.base._run_remote = lambda _command: (
        attempts.append(True)
        or subprocess.CompletedProcess(
            args=[], returncode=255, stdout="", stderr="transport"
        )
    )
    remote.time.sleep = lambda _seconds: None
    try:
        _raises(
            RuntimeError,
            remote._poll_worker,
        ...
        remote.base._run_remote = original_run
        remote.time.sleep = original_sleep
    assert len(attempts) == 3


def test_controller_checks_kerberos_twice_before_launch_only() -> None:
    selected = _gpu_row(2)
    calls = []
    originals = {
    ...
    remote.validate_kerberos = lambda **kwargs: (
        calls.append(kwargs["minimum_lifetime_seconds"])
        or {"status": "PASS"}
    )
    ...
    remote._create_controller_dir = lambda **_kwargs: None
    ...
    assert calls == [5400, 5400]
    assert result["status"] == "COMPLETE"


def test_download_and_verifier_receipts_are_exact() -> None:
    artifacts = {
    ...
    changed["reconstructed_classification"] = "NO_GO_K4_MEDIAN"
    _raises(
        ValueError,
...
def test_controller_source_contains_no_destructive_command() -> None:
    source = (
        REPO_ROOT
...
def main() -> None:
    test_paths_runtime_and_archive_are_confined()
    test_preflight_worker_and_gates_use_continuation_tools()
    test_gpu_admission_and_polling_contract()
    test_controller_checks_kerberos_twice_before_launch_only()
    test_download_and_verifier_receipts_are_exact()
    test_controller_source_contains_no_destructive_command()
    print("exact burst continuation remote controller tests passed")
