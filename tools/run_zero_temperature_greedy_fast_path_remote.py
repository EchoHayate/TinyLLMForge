#!/usr/bin/env python3
"""Safe remote controller for the greedy fast-path Stage-1 gate."""
...
def remote_runtime_prelude(
    *,
    ...
    expected_prefix = (
        APPROVED_ROOT
        + "/zero-temperature-greedy-fast-path/staging/"
    )
    if (
...
def _download_terminal_bundle(
    *,
    ...
    validate_terminal_download_inventory(primary_inventory)
...
def run_controller(args) -> dict:
    ...
    preflight = {
        "schema_version":
            "zero-temperature-greedy-fast-path.controller.v1",
        "status": "READY",
    ...
    _run_remote_gates(source=source, primary=paths["primary"])
    completion = {
        "schema_version":
            "zero-temperature-greedy-fast-path.controller.v1",
        "status": "COMPLETE",
