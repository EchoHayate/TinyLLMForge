#!/usr/bin/env python3
"""Safety contracts for the split-phase K8 remote controller."""
...
from tools import run_exact_burst_split_phase_remote as remote
...
def test_paths_runtime_and_run_tags_are_strictly_confined() -> None:
    paths = remote.remote_paths(
        "20260823-qwen3-06b-split-phase-r1"
    )
    expected_root = (
        "/data00/home/sitian/tinyllmforge-workspaces/"
        "command-timeline-20260818/exact-burst-split-phase"
    )
...
def test_source_is_pushed_commit_with_explicit_clean_patch() -> None:
    commit = "a" * 40
...
def test_gpu_admission_is_strict_and_polling_auto_launches() -> None:
    rows = [
    ...
    assert [name for name, _value in calls].index("wait") < (
        [name for name, _value in calls].index("launch")
    )
    assert [
    ...
    assert (
        call_names.index("upload")
        < kerberos_positions[1]
        < call_names.index("preflight")
    )
    assert result["status"] == "COMPLETE"
    assert result["source_patch_sha256"] == remote.SOURCE_PATCH_SHA256
...
def test_preflight_worker_gate_and_verifier_are_source_bound() -> None:
    ...
    assert pid == 321
    joined = "\n".join(captured)
    profiler = joined.index("tools/profile_exact_burst_split_phase.py")
    patch = joined.index("source.patch")
    gate = joined.index("tools/exact_burst_split_phase_gate.py")
    verifier = joined.index("tools/exact_burst_split_phase_verify.py")
...
def test_terminal_bundle_requires_manifest_verifier_and_source_patch() -> None:
    artifacts = {
    ...
    assert "source.patch" in artifacts
    ...
    changed["reconstructed_classification"] = (
    ...
    _raises(
        ...
        receipt,
        changed,
        message="verification receipt disagreement",
    )


def test_controller_contains_no_refresh_kill_or_root_temp_write() -> None:
    source = (
        REPO_ROOT / "tools/run_exact_burst_split_phase_remote.py"
...
def main() -> None:
    test_paths_runtime_and_run_tags_are_strictly_confined()
    test_source_is_pushed_commit_with_explicit_clean_patch()
    test_gpu_admission_is_strict_and_polling_auto_launches()
    test_preflight_worker_gate_and_verifier_are_source_bound()
    test_terminal_bundle_requires_manifest_verifier_and_source_patch()
    test_controller_contains_no_refresh_kill_or_root_temp_write()
    print("exact burst split-phase remote controller tests passed")
