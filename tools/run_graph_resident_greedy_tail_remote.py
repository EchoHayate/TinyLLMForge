#!/usr/bin/env python3
"""Safe remote controller for the graph-tail Stage-1 gate."""
...
def remote_paths(run_tag: str) -> dict[str, str]:
    ...
    root = APPROVED_ROOT + "/graph-resident-greedy-tail"
    paths = {
    ...
    for path in paths.values():
        if (
            ...
            or any(value in path for value in forbidden)
        ):
            raise ValueError(
...
def _run_remote_gates(
    *,
    ...
    _run_remote_checked(
        remote,
        context=(
            "remote producer and independent verification"
        ),
    )
...
def run_controller(args) -> dict:
    ...
    preflight = {
        "schema_version":
            "graph-resident-greedy-tail.controller.v1",
        "status": "READY",
