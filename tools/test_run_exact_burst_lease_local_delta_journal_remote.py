...
from tools import (
    run_exact_burst_lease_local_delta_journal_remote as remote,
)
...
def test_worker_and_verifier_commands_are_source_bound(
    ...
    assert "--run-tag fresh-delta-tag" in joined
    assert (
...
def test_worker_poll_tolerates_one_transient_remote_failure(
    ...
    assert remote._poll_worker(
        controller=remote.remote_paths("poll-retry")["controller"],
        worker_pid=321,
