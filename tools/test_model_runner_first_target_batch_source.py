...
def _method(name="_run_spec_first_target_batch"):
...
def test_first_target_batch_uses_one_prepare_and_one_model_forward():
    method = _method()
    calls = [
        node
...
def test_first_target_model_forward_is_not_nested_in_iteration():
    method = _method()
...
def test_first_target_context_reset_is_in_finally_block():
    method = _method()
    try_nodes = [
        node
        for node in ast.walk(method)
...
def test_first_target_capability_flags_accept_positional_rpc_arguments():
    method = _method("run_spec_first_target_batch")
    ...
    assert positional_names[-3:] == (
        "return_hidden",
        "return_logits",
        "kv_block_identity_rows",
    )
    assert "return_hidden" not in keyword_only_names
    assert "return_logits" not in keyword_only_names
