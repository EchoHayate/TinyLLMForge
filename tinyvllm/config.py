...
class Config:
    ...
    def __post_init__(self):
        ...
        # 传递 quantization 给 hf_config，或者独立保存，这里为了方便让模型可以直接 getattr 访问
        if self.quantization:
            self.hf_config.quantization = self.quantization
            
        self.max_model_len = min(self.max_model_len, getattr(self.hf_config, "max_position_embeddings", self.max_model_len))
