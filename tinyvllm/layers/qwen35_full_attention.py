...
QWEN35_PREFILL_BLOCKWISE_THRESHOLD = 16384
QWEN35_PREFILL_BLOCK_TOKENS = 512


def qwen35_prefill_blockwise_attention(
    query: torch.Tensor,
    key: torch.Tensor,
    value: torch.Tensor,
    *,
    ...
    head_dim: int,
    scale: float,
    block_tokens: int = QWEN35_PREFILL_BLOCK_TOKENS,
) -> torch.Tensor:
    token_count = int(query.shape[0])
    block_tokens = int(block_tokens)
    if block_tokens <= 0:
        raise ValueError("prefill attention block size must be positive")
    if query.shape != (token_count, num_heads, head_dim):
        raise ValueError("prefill query shape does not match attention shape")
    if key.shape != value.shape or key.shape[0] != token_count:
        raise ValueError("prefill key and value shapes must match")
    if key.shape[2] != head_dim or num_heads % key.shape[1] != 0:
        raise ValueError("prefill key heads do not divide query heads")
    if token_count == 0:
        return query.new_empty((0, num_heads * head_dim))
    ...
    outputs = []
    for query_start in range(0, token_count, block_tokens):
        query_end = min(query_start + block_tokens, token_count)
        query_block = (
        ...
        running_max = torch.full(
            (num_heads, query_end - query_start),
            -torch.inf,
            dtype=torch.float32,
            device=query.device,
        )
        ...
        for key_start in range(0, query_end, block_tokens):
            key_end = min(key_start + block_tokens, query_end)
            key_block = (
                expanded_key[key_start:key_end]
                .transpose(0, 1)
                .to(torch.float32)
            )
            value_block = (
                expanded_value[key_start:key_end]
                .transpose(0, 1)
                .to(torch.float32)
            )
            scores = torch.matmul(
            ...
            block_weights = torch.exp(
                scores - next_max.unsqueeze(-1)
            )
            running_sum = (
            ...
            running_output = (
                ...
                + torch.matmul(block_weights, value_block)
            )
...
def qwen35_prefill_eager_attention(
    ...
    *,
    ...
    for index in range(len(offsets) - 1):
        ...
        if reference_count > blockwise_threshold:
            outputs.append(
                qwen35_prefill_blockwise_attention(
                    segment_query,
                    segment_key,
                    segment_value,
                    num_heads=num_heads,
